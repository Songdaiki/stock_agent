# Checkpoint 28A Round 140 R8 Loop 8 Platform / Content / Software / Security

## 목적

`docs/round/round_140.md`의 R8 Loop 8 내용을 별도 calibration pack으로 반영했다.

R8은 플랫폼, 콘텐츠, 소프트웨어, 보안에서 “AI 기능”, “유저 수”, “신작 기대”, “광고 회복”, “보안 수요”와 실제 반복 매출·FCF를 분리하는 라운드다. 이번 Loop 8의 핵심은 ARR, billings, bookings, ad revenue, contract value, guidance, churn, retention, workflow lock-in, OPM/FCF, operational trust, privacy/youth safety, ad quality를 더 강하게 보는 것이다.
...
- `src/e2r/sector/round140_r8_loop8_platform_content_sw_security.py`
- `src/e2r/cli/build_round140_r8_loop8_report.py`
- `tests/test_round140_r8_loop8_platform_content_sw_security.py`
- `data/e2r_case_library/cases_r8_loop8_round140.jsonl`
- `data/sector_taxonomy/score_weight_profiles_round140_r8_loop8_v8.csv`
- `output/e2r_round140_r8_loop8_platform_content_sw_security/`
...
```bash
PYTHONPATH=src python -m unittest tests/test_round140_r8_loop8_platform_content_sw_security.py -v
PYTHONPATH=src python -m e2r.cli.build_round140_r8_loop8_report
```

결과:

- Round 140 전용 테스트 14개 통과
- v8 score profile 생성
- case JSONL 생성
