좋아. **R12 Loop 6까지 끝났으니, 이번은 R13 Loop 6 — Cross-archetype RedTeam / 4B / 회계신뢰도 / 가격검증 총정리**다.

R13은 새 산업군이 아니라, **R1~R12에서 나온 모든 후보를 마지막으로 통과시키는 공통 검문소**다. Theme Tag Map 기준으로 R1~R12는 전력설비, HBM, ESS, 희토류, K푸드, 밸류업, CDMO, SaaS, 로보택시, 데이터센터, 정책이벤트, 스마트팜까지 대부분 흡수하고 있고, R13은 이 후보들이 진짜 구조적 E2R인지, 4B 과열인지, 4C 논리훼손인지, 단순 이벤트·사이클·테마인지 재분류한다.

Checkpoint 20 원칙도 R13에서 최종 강제한다. OpenDART list만으로는 부족하고, 단일판매·공급계약, 신규시설투자, 유상증자, 전환사채, 감사의견, 거래정지, 계약 해지·정정 같은 watch disclosure는 detail에서 계약금액, 계약기간, 매출 대비 계약금액, 거래상대방, OP YoY, 희석률을 실제 확인해야 한다. 없는 값을 만들면 Stage 3-Green이 가장 빨리 오염된다.

서생원식으로도 R13의 역할은 명확하다. “좋아 보이는 테마”가 아니라 **산업 구조 변화 → EPS/FCF 체급 변화 → 시장의 과거 프레임 오해 → 밸류에이션 리레이팅 → 실제 가격경로 확인**이 이어지는지만 남겨야 한다.

---

# R13 Loop 6. Cross-archetype RedTeam / 4B / 회계신뢰도 / 가격검증 총정리
...
R13은 산업군이 아니다.
R1~R12에서 나온 후보를 통과시키는 **최종 품질검사대**다.
...
5. 시장 전체 policy shock으로 price-path가 깨지는 crowded trade
```
...
| `STRUCTURAL_SUCCESS_BUT_4B_WATCH`       | 구조는 맞지만 시장이 이미 대부분 인정            | 졸업·과열 감시          |
| `SECTOR_SUCCESS_BUT_POLICY_SHOCK_WATCH` | 구조는 맞지만 정책 shock에 price-path가 취약 | 4B overlay        |
| `PRICE_ONLY_RALLY`                      | 주가만 오르고 근거 없음                    | Green 금지          |
| `EVENT_PREMIUM`                         | 공개매수·정책·재난·질병·MOU로 오른 가격         | Event로 분리         |
...
| `POLICY_MARKET_SHOCK_EVENT`             | 세금·배당·분배 발언이 crowded trade를 흔듦   | 4B/4C overlay     |
...
## 4-1. SK하이닉스 HBM / AI 메모리 — `STRUCTURAL_SUCCESS_ALIGNED + STRUCTURAL_SUCCESS_BUT_4B_WATCH`

SK하이닉스는 R13 기준으로 가장 강한 `STRUCTURAL_SUCCESS_ALIGNED` 예시다. AI 서버용 HBM과 기존 메모리 수요가 실제 영업이익·시총·가격경로를 바꿨고, Reuters는 SK하이닉스 주가가 2025년에 274%, 2026년에 200% 이상 상승해 시가총액 약 9,420억 달러에 근접했다고 보도했다. 이건 단순 “AI 테마”가 아니라 **HBM 병목 → EPS 체급 변화 → 메모리 valuation frame 전환 → 가격경로 리레이팅**이 연결된 케이스다. ([Reuters][1])

또 SK하이닉스는 2025년 4분기 영업이익 19.2조 원으로 전년 대비 137% 증가했고, HBM 점유율 61%, 12.2조 원 규모 treasury share cancellation을 발표했다. 실적·HBM 점유율·자본환원이 동시에 확인됐으므로 Stage 3 성공사례로 남길 수 있지만, 같은 기사에서 2026년 경쟁 심화와 HBM 점유율 하락 가능성도 언급됐기 때문에 4B-watch를 함께 붙여야 한다. ([Reuters][2])
...
Circle은 regulated fiat-backed stablecoin infrastructure의 좋은 reference다. 2026년 1분기 revenue/reserve income은 20% 증가한 6.94억 달러였고, USDC circulation은 전년 대비 28% 증가한 770억 달러였다. MiCA와 GENIUS Act 같은 규제 프레임워크가 stablecoin adoption에 도움을 줬지만, Circle은 미국 국채·은행예금 준비금에서 이자수익을 얻는 구조라 금리와 규제에 민감하다. ([Reuters][3])

다만 주가는 IPO 가격 31달러 대비 크게 오른 상태이고, AI-agent payment나 Arc 같은 option value가 valuation에 먼저 반영될 수 있다. 그래서 Circle은 `REGULATED_STABLECOIN_INFRA`의 성공 후보이되, reserve income sensitivity, issuer margin, user cap, redemption, de-peg history를 계속 확인해야 한다. ([Reuters][3])
...
- 준비금·상환 구조 검증 필요
- IPO 이후 valuation 과열
- AI-agent payment option이 아직 실제 fee revenue로 검증되지 않음
```
...
## 4-3. Blackstone Digital Infrastructure Trust — `AI_INFRA_REAL_ASSET_THEME_STAGE1_NOT_GREEN`

Blackstone Digital Infrastructure Trust는 AI 데이터센터 실물자산의 강한 Stage 1 후보지만, Stage 3-Green은 아니다. Reuters는 이 vehicle이 17.5억 달러 IPO를 했고, hyperscale tenant 대상 신규 데이터센터 자산 취득을 목표로 하지만, 아직은 Blackstone의 실행력과 acquisition pipeline을 사는 구조라고 보도했다. ([Reuters][4])
...
주의:
- asset_acquired_flag 확인 필요
- binding_lease_flag 확인 필요
- NOI/AFFO 확인 필요
- power/water/permitting 확인 필요
```
...
Bavarian Nordic처럼 질병 이벤트가 정부 stockpile 계약과 매출·EBITDA 가이던스 상향으로 이어지면 `EVENT_TO_CONTRACT_ESCALATION`으로 남긴다. R11에서 본 원칙 그대로, 단순 질병 뉴스는 Event지만 **정부계약 + 계약금액 + 가이던스 상향 + 반복 조달 가능성**이 붙으면 Stage 2 후보가 된다.
...
Supermicro는 R13의 핵심 반례다. AI 서버 수요로 급등했지만, Ernst & Young이 감사인에서 사임하자 주가가 30% 이상 급락했다. EY는 governance, transparency, internal control over financial reporting에 대한 우려를 제기했고, 이 사안은 annual report filing delay, Hindenburg의 회계조작 주장, DOJ 조사 보도 이후 나왔다. ([Reuters][5])

```text
case_type:
EARLY_RERATING_SUCCESS_THEN_ACCOUNTING_HARD_4C
...
CrowdStrike는 보안 SaaS가 반복매출이어도 운영 신뢰가 깨지면 어떻게 무너지는지 보여준다. 2024년 7월 flawed software update로 800만 대 이상 컴퓨터가 영향을 받았고, 이후 12일 동안 CrowdStrike 주가는 32% 하락해 약 250억 달러 시가총액이 사라졌다. Delta도 outage 비용을 두고 손해배상을 추진했다. ([Reuters][6])
...
Bluebird bio는 승인된 유전자치료제를 갖고 있었지만 severe cash crunch 속에 Carlyle·SK Capital에 주당 3달러로 비상장화되기로 했다. 이 가격은 직전 종가 대비 57.4% 할인된 수준이었고, 발표 후 주가는 36% 하락했다. Reuters는 Bluebird의 gene therapy uptake가 느렸고, 2024년 11월 기준 세 치료제 중 하나를 시작한 환자가 57명에 그쳤다고 보도했다. ([Reuters][7])
...
GLP-1은 대형 성장시장처럼 보였지만, Novo Nordisk는 2026년 sales와 operating profit이 5~13% 감소할 수 있다고 경고했고, 주가는 16% 하락하며 약 500억 달러의 시총이 사라졌다. 원인은 미국 가격 압박, 경쟁 심화, semaglutide 제품 가격 하락 등으로 설명됐다. ([Reuters][8])
...
데이터센터 REIT도 AI 수혜라는 이름만으로 Green을 줄 수 없다. Hindenburg Research는 Equinix가 maintenance capex를 expansion capex로 잘못 분류해 AFFO를 부풀렸다고 주장했고, Reuters는 해당 공개 후 Equinix 주가가 약 2% 하락했다고 보도했다. ([Reuters][9])
...
CoreWeave류 neocloud는 대형계약이 visibility를 높이지만, 공급자·투자자·고객 관계가 서로 얽히면 circular financing RedTeam을 붙여야 한다. Reuters는 Nvidia가 CoreWeave에 20억 달러를 투자했고, CoreWeave의 미판매 cloud capacity를 Nvidia가 구매하는 63억 달러 구조도 있다고 보도했다. Jensen Huang 부부 재단의 CoreWeave compute 구매·기부 건도 Nvidia와 CoreWeave의 깊은 관계를 다시 조명했다. ([Reuters][10])
...
## 5-7. Fermi — AI real asset no-revenue / no-tenant 4C-watch

Fermi는 AI power campus narrative의 hard RedTeam 사례다. Reuters는 Fermi가 2025년 IPO에서 6.825억 달러를 조달하고 124.6억 달러 valuation을 받았지만, 2025년 1월 설립된 초기 기업이고, 6월 말까지 640만 달러 손실을 냈으며, AI data-center campus는 2038년까지 11GW 전력 공급을 목표로 하는 장기 프로젝트라고 보도했다. ([Reuters][11])
...
R13 규칙:
no_revenue_flag = hard review
binding_tenant_lease_absent = Stage 3 금지
power_delivery_long_dated = Valuation 감점
water/permitting_unknown = Disclosure cap
```

AI 데이터센터 수요가 진짜여도, **자산·tenant·전력·수자원·NOI/AFFO**가 없으면 R10/R13에서 Stage 3를 막는다.

---

## 5-8. AI citizen dividend / policy shock — 구조가 맞아도 price-path는 깨질 수 있다

2026년 5월 12일 한국 증시는 AI 초과이익 또는 세수 배분성 발언 이후 코스피가 장중 5.1% 하락하고 종가 기준 2.3% 하락했다. 이후 “세율 인상”이 아니라 “늘어난 세수 활용”이라는 취지로 해명됐지만, 이미 과열된 AI·반도체·value-up rally에서는 정책 코멘트 하나도 price-path를 크게 흔들 수 있다. ([마켓워치][12])
...
이면:
Stage 자체가 깨지지 않아도
price_validation_status를 재검토한다.
```

이건 “논리 4C”라기보다 **crowded 4B 구간의 가격경로 4C-watch**다.
...
Supermicro는 이 항목의 기준 반례다. AI 서버 매출 성장으로 리레이팅됐어도, 회계 신뢰도 hard flag가 나오면 바로 4C가 된다. ([Reuters][5])
...
Novo Nordisk 사례는 GLP-1도 “거대한 시장”만으로 방어되지 않는다는 기준이다. ([Reuters][8])
...
Equinix 사례처럼 데이터센터 REIT도 capex 분류와 AFFO 무결성을 통과해야 한다. BXDC처럼 자산을 아직 취득하지 않은 AI data-center REIT vehicle도 sponsor premium만으로 Stage 3를 주면 안 된다. ([Reuters][9])
...
Circle은 regulated stablecoin infrastructure 후보지만, IPO 가격 대비 크게 오른 가격경로에서는 4B-watch가 필요하다. ([Reuters][3])
...
2026년 5월 12일 한국 증시는 AI 수익·세수 재분배성 발언 이후 코스피가 장중 5.1% 하락하고 종가 기준 2.3% 하락했다. 이건 crowded rally에서는 정책 이벤트가 thesis를 깨지 않아도 price-path를 강하게 흔들 수 있음을 보여준다. ([마켓워치][12])
...
Supermicro는 이 기준을 가장 잘 보여준다. 감사인이 사임하고 경영진·감사위원회 신뢰 문제를 제기하면, AI 서버 매출 성장은 더 이상 Green을 방어하지 못한다. ([Reuters][5])
...
CrowdStrike는 보안 SaaS의 핵심 반례다. 반복매출 기업도 운영 신뢰가 깨지면 RedTeam hard review로 내려가야 한다. ([Reuters][6])
...
이건 R2 neocloud, R7 biotech, R9 eVTOL, R10 REIT, R12 스마트팜에 공통 적용한다. CoreWeave처럼 대형계약이 있더라도 supplier-investor-customer loop와 부채·FCF를 통과해야 한다. ([Reuters][10])
...
R6 stablecoin은 반드시 regulated fiat-backed와 algorithmic을 분리해야 한다. Circle 같은 fiat-backed model은 reserve income과 circulation이 실제 숫자로 확인될 수 있지만, 그 경우에도 금리·상환·규제 리스크를 따로 본다. ([Reuters][3])
...
정책 코멘트가 실제 세금으로 확정되지 않았더라도, 주도주와 지수가 이미 4B 구간에 있으면 price-path를 깨는 강한 overlay가 된다. ([마켓워치][12])

---

# 8. 점수비중 보정표 — R13 Loop 6 / v6.0
...
| `STRUCTURAL_SUCCESS_BUT_4B_WATCH`       |      유지 |         유지 |         유지 |         약화 |        감점 | optional |   유지 | 졸업·과열 감시               |
| `SECTOR_SUCCESS_BUT_POLICY_SHOCK_WATCH` |      유지 |         유지 |         유지 |         약화 |        감점 | optional |   유지 | crowded trade overlay  |
...
R13 Loop 6부터 모든 case는 아래 stage date를 강제한다.
...
R13 Loop 6에서는 특히 `stage4b_date`와 `stage4c_date`를 더 중요하게 본다.
...
## 10-4. R13 Loop 6에서 가격검증 실패 시 처리
...
시장 전체 policy shock으로 가격경로가 훼손:
→ POLICY_MARKET_SHOCK overlay 적용
```
...
R13 Loop 6부터는 모든 case library에 아래 공통 필드를 강제한다.
...
6. 4B 과열 미포화
7. policy shock overlay 통과

이 일곱 개를 통과해야 한다.
```

**R13 Loop 6 점수정규화의 핵심 문장:**
...
이제 **R1~R13 Loop 6가 완료**됐다.
다음에 같은 지시가 오면 규칙대로 **R1 — 산업재·수주·인프라 Loop 7**로 돌아가서, 지금까지 쌓은 RedTeam과 가격검증 기준을 반영해 다시 더 촘촘하게 판다.
...
[2]: https://www.reuters.com/world/asia-pacific/sk-hynix-posts-forecast-beating-q4-profit-huge-ai-demand-2026-01-28/?utm_source=chatgpt.com "SK Hynix's record quarterly profit smashes forecasts, sees explosive memory chip demand"
...
[5]: https://www.reuters.com/technology/super-micro-computer-says-ernst-young-resigns-its-accountant-shares-tank-2024-10-30/?utm_source=chatgpt.com "Super Micro Computer says Ernst & Young resigns as auditor, shares tank"
...
[10]: https://www.reuters.com/legal/transactional/nvidia-ceos-foundation-buys-108-million-ai-computing-coreweave-donates-it-2026-05-13/?utm_source=chatgpt.com "Nvidia CEO's foundation buys $108 million of AI computing from CoreWeave, donates it to researchers"
[11]: https://www.reuters.com/business/rick-perrys-data-center-reit-raises-682-million-us-ipo-2025-09-30/?utm_source=chatgpt.com "Rick Perry-backed Fermi REIT raises $682 million in U.S. IPO amid AI data-center boom"
