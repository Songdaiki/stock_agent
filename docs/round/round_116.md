좋아. **R10 Loop 6까지 끝났으니, 이번은 R11 Loop 6 — 정책·지정학·재난·이벤트**다.

R11은 새 산업군을 찾는 라운드라기보다, R1~R12에서 나온 모든 테마가 **뉴스성 이벤트인지, 실제 계약·예산·조달·financing·매출로 승격됐는지**를 가르는 방어 라운드다. Theme Tag Map 기준으로도 남북경협, DMZ, 개성공단, 금강산 관광, 황사·마스크, 엠폭스, 코로나 진단, 초전도체, 맥신, 그래핀, 양자기술 같은 태그는 대부분 Event/Red 또는 SPECULATIVE_SCIENCE_THEME 성격이고, Stage 3-Green보다 RedTeam/4B 방어가 중요하다.

Checkpoint 20 원칙도 R11에서 특히 중요하다. 정부계약, 예산, 공사착수, stockpile 주문, 계약금액, 매출 가이던스, 진단매출 감소, 재현 실패 같은 값은 실제 확인된 증거만 써야 한다. 정책·재난·과학 테마는 “뉴스가 크다”는 이유로 없는 매출·계약·EPS를 만들어 넣으면 가장 빨리 점수체계가 오염된다.
...
정부 stockpile / 조달 취소 / 재건 financing /
전력망·기후재난 / 희토류 수출통제 /
AI windfall tax shock / 과학테마 재현실패를
...
희토류 수출통제 + 실제 대체공급 계약·price floor·offtake
...
| `STRATEGIC_SUPPLY_CHAIN_EXPORT_CONTROL_EVENT` | Event/Watch. 희토류·반도체·배터리 소재 수출통제는 macro bottleneck 증거    |
| `EXPORT_CONTROL_TO_OFFTAKE_ESCALATION`        | Watch-to-Green. 수출통제가 실제 대체공급·offtake·price floor로 연결될 때 |
...
| `TOURISM_POLICY_EVENT`                        | Event/Watch. 비자·관광정책은 실제 spend 확인 전 Green 금지             |
| `POLICY_MARKET_SHOCK_EVENT`                   | RedTeam overlay. 세금·분배 발언이 시장 전체를 흔드는 경우                 |
| `AI_WINDFALL_CITIZEN_DIVIDEND_POLICY_SHOCK`   | RedTeam overlay. AI 초과세수·시민배당 발언이 crowded rally를 흔드는 경우  |
...
Bavarian Nordic은 R11에서 보기 드문 `EVENT_TO_CONTRACT_ESCALATION` 후보로 둔다. 회사는 미국 정부로부터 Jynneos 동결건조 제형 추가 계약 옵션 9,700만 달러를 받았고, 이를 반영해 2026년 매출 가이던스를 50억~52억 덴마크크로네에서 55억~57억 덴마크크로네로, EBITDA margin 전망을 25%에서 약 28%로 올렸다. 이건 단순 엠폭스 뉴스가 아니라 **정부 stockpile 계약 + 매출 가이던스 상향 + EBITDA margin 상향**으로 승격된 사례다. ([Reuters][1])
...
## 4-2. 우크라 critical infra financing — `CRITICAL_INFRA_RECONSTRUCTION_FINANCING`

Loop 6에서 우크라 재건은 더 세밀하게 나눈다. EBRD는 우크라 민영화와 critical infrastructure 회복을 지원할 수 있다고 밝혔고, Chornomorsk 항만 40년 concession, 700MW 재생에너지 확대, critical electricity transformer 보호시설 135개 중 118개를 연말까지 확보하는 계획 등이 언급됐다. 이건 단순 재건 테마가 아니라 **항만 concession + 전력망 보호 + 재생에너지 expansion + 국제기구 financing**으로 넘어가는 축이다. ([Reuters][2])
...
```text
CRITICAL_INFRA_RECONSTRUCTION_FINANCING 강화.
...
## 4-3. 희토류 수출통제 — `STRATEGIC_SUPPLY_CHAIN_EXPORT_CONTROL_EVENT`

중국의 heavy rare earth 수출통제는 R11에서 macro bottleneck evidence로 둔다. 2026년 5월 보도 기준, 중국의 rare earth export approval은 일부 개선됐지만 yttrium 같은 핵심 소재 승인 지연이 여전히 남아 있고, 2025년 4월 수출통제 이후 heavy rare earth 수출 제한이 미국·일본·독일·항공우주·반도체 공급망에 영향을 줬다. 이건 개별 종목 Green이 아니라 **정책·지정학 event가 실제 공급망 병목을 만든 사례**다. ([Reuters][3])
...
좋은 점:
- 수출통제가 실제 공급망 병목으로 작동
- 항공우주·방산·반도체·EV·풍력 모두 영향 가능
- 대체 공급망·offtake·price floor 필요성 강화

주의:
- 개별 기업 생산능력 없으면 Green 금지
- 수출통제 완화 또는 truce 연장 시 premium 축소
- 가격 spike를 장기 EPS로 바로 연결하면 안 됨
```
...
## 4-4. 폭염 → 전력망/VPP — `CLIMATE_EVENT_TO_GRID_INFRA`

폭염 자체는 Event지만, 전력망·냉각·수요반응·VPP로 넘어가면 R1/R3/R10과 교차하는 구조증거가 된다. 독일 사례 연구는 폭염 시나리오에서 가정용 이동식 에어컨 보급률이 19%에서 35%로 올라가면 피크 전력수요가 12.9GW 이상 증가할 수 있고, 오후 피크가 태양광 출력 저하 시간과 겹쳐 전력계통 안정성 부담을 키울 수 있다고 분석했다. ([arXiv][4])

뉴욕에서는 폭염 피크 시간대 창문형 에어컨 부하를 줄이기 위해 임차인에게 plug-in battery를 제공하는 시범 프로그램이 진행 중이다. 초기 200kW flexible capacity에서 2MW 이상·1,000가구 이상으로 확대될 예정이고, 이는 VPP와 분산형 배터리 수요가 climate event에서 실제 grid program으로 넘어가는 사례다. ([AP News][5])

```text
가격경로 1차 판정:
CLIMATE_EVENT_TO_GRID_INFRA_STAGE2_REFERENCE

좋은 점:
- 폭염 → 전력피크
- 에어컨 → grid stress
- battery/VPP → 실제 프로그램
- R1 전력망, R3 ESS, R10 데이터센터 전력망과 교차 가능
...
## 4-5. 중국 단체관광 무비자 — `TOURISM_POLICY_EVENT`

한국의 중국 단체관광객 무비자 정책은 면세점, 카지노, 호텔, 화장품에 강한 Stage 1 촉매가 될 수 있다. 한국은 2025년 9월 29일부터 2026년 6월까지 중국 단체관광객에게 최대 15일 무비자 입국을 허용하기로 했고, 발표 직후 백화점·호텔·카지노·화장품 관련주가 중국 소비 기대감으로 움직였다. 하지만 R11 기준에서는 이 정책을 **관광객 수·객단가·casino drop amount·면세 매출·OPM 확인 전까지 Event/Watch**로 둬야 한다. ([Reuters][6])
...
- 정책기간 명확
- 관련주 price premium 가능

주의:
- 정책 완화 ≠ 소비재/관광 구조적 Green
- 관광객 수와 spend 확인 필요
- 면세점 객단가·카지노 drop amount가 핵심
```
...
## 5-1. Moderna BARDA funding cancellation — `PUBLIC_HEALTH_PROCUREMENT_REVERSAL`

전염병 계약은 정부가 돈을 넣는 순간 Stage 2 후보가 될 수 있지만, 정책 전환으로 뒤집히면 4C가 된다. 미국 정부는 Moderna의 mRNA 기반 H5N1 bird flu 백신 개발·구매를 위한 7.66억 달러 BARDA funding을 취소했다. AP 보도 기준 이 funding은 2024년 1.76억 달러와 2025년 5.90억 달러 지원을 포함했고, late-stage trial 지원 성격이 있었기 때문에, 취소는 공중보건 조달형 바이오·방역주에 hard RedTeam이다. ([AP News][7])
...
## 5-2. 남북경협·금강산 관광 — `NORTH_KOREA_POLICY_EVENT`

남북경협은 R11에서 기본값을 Event/Red로 유지한다. 북한은 금강산 지역의 이산가족 상봉 시설을 철거하고 있고, 한국 정부는 이를 반인도적 행위로 비판했다. Reuters는 북한이 한국을 “적대국”으로 규정했고, 남북 도로·철도 일부도 폭파한 적이 있다고 보도했다. 이런 환경에서는 금강산 관광, 개성공단, 남북철도, DMZ, 북한 광물개발 관련주는 실제 사업 재개·제재 완화·정부 승인·현금흐름 전까지 Green을 주면 안 된다. ([Reuters][8])
...
## 5-3. LK-99 — `SPECULATIVE_SCIENCE_THEME`

LK-99는 R11의 과학테마 hard counterexample이다. 독립 재현 연구는 실온·상압 초전도성을 확인하지 못했고, 한 arXiv 연구는 SQUID magnetization 측정에서도 실온 초전도 신호가 없다고 보고했다. 또 다른 연구는 원 논문에서 초전도 증거처럼 보인 저항·열용량 이상이 Cu₂S, 즉 copper sulfide 불순물의 구조전이에 의해 설명될 수 있다고 지적했다. 즉 LK-99는 **preprint·SNS·영상 기반 테마가 실제 재현·상용화·매출로 넘어가지 못한 대표 사례**다. ([arXiv][9])
...
## 5-4. 전염병 진단 one-off — `DIAGNOSTICS_INFECTIOUS_EVENT`

전염병 진단은 EPS가 폭발할 수 있지만 지속성이 약하다. Abbott의 2025년 3분기 diagnostics segment 매출은 COVID-19와 당뇨 검사 수요 약화 등으로 6.6% 감소했고, 회사의 전체 매출도 시장 예상에 소폭 못 미쳤다. 이 사례는 진단키트·전염병 수요가 정상화되면 EPS가 빠르게 꺾일 수 있음을 보여준다. ([Reuters][10])
...
## 5-5. AI citizen dividend policy shock — `AI_WINDFALL_CITIZEN_DIVIDEND_POLICY_SHOCK`

R11은 개별 테마뿐 아니라 시장 전체 정책 이벤트도 RedTeam으로 본다. 2026년 5월 12일 한국 증시는 AI 수익 일부를 국민에게 돌려야 한다는 취지의 정책성 발언 이후 코스피가 장중 5.1% 하락하고 종가 기준 2.3% 하락했다. 이후 해당 발언은 세율 인상이 아니라 늘어난 세수 활용에 관한 취지라고 해명됐지만, 이미 과열된 AI·반도체·value-up rally에서는 정책 코멘트 하나도 price-path를 크게 흔들 수 있다. ([마켓워치][11])
...
Bavarian Nordic은 정부계약과 가이던스 상향이 붙어 Stage 2 후보지만, Moderna 사례처럼 공중보건 조달은 정책 변화로 뒤집힐 수 있다. 따라서 질병 테마는 `EVENT_TO_CONTRACT`와 `PROCUREMENT_REVERSAL`을 동시에 점검해야 한다. ([Reuters][1])
...
EBRD의 우크라 항만 concession, 전력망 보호시설, 재생에너지 financing은 좋은 Stage 2 reference지만, 한국 상장주에 매핑하려면 실제 계약·수주·매출 연결이 필요하다. 재건 테마 전체를 이 사례 하나로 Green 처리하면 안 된다. ([Reuters][2])

---

## 6-3. 희토류 수출통제 4B-watch

```text
4B 조건:
- 중국 수출통제 뉴스로 희토류·전략자원 관련주 동반 급등
- 실제 제련·자석 생산능력 없는 회사도 상승
- 수출허가 지연을 장기 EPS로 바로 연결
- truce extension 또는 export approval 개선 가능성을 무시
```

희토류 수출통제는 macro bottleneck evidence다. 하지만 개별 기업은 생산능력, 고객계약, price floor, offtake, FCF를 따로 확인해야 Stage 2~3 후보가 된다. ([Reuters][3])

---

## 6-4. 폭염·전력망 테마 4B-watch
...
폭염은 전력피크와 grid stress를 만들 수 있지만, 개별 종목은 전력망 투자·수요반응·배터리/VPP 계약 등으로 연결될 때만 Stage 2 후보가 된다. 독일 AC peak-load 연구와 뉴욕 plug-in battery pilot은 구조증거 후보지만, 개별 상장사 매출 연결은 별도로 검증해야 한다. ([arXiv][4])

---

## 6-5. 관광·비자 정책 4B-watch

```text
4B 조건:
- 무비자 정책 발표만으로 면세·카지노·호텔·화장품 관련주 급등
- 실제 visitor arrivals, spend, casino drop amount 미확인
- 정책기간 종료 이후 수요 지속성을 시장이 과대평가
```

중국 단체관광 무비자는 Stage 1 촉매다. Stage 3는 관광객 수, 객단가, 면세 매출, 카지노 drop amount, RevPAR, OPM이 확인될 때만 가능하다. ([Reuters][6])

---

## 6-6. 정책 shock 4B-watch
...
AI 시민배당·세수배분성 발언으로 코스피가 장중 5.1% 하락했다는 보도는, crowded rally에서는 정책 이벤트가 sector thesis를 깨지 않아도 가격경로를 강하게 훼손할 수 있음을 보여준다. ([마켓워치][11])

---

## 6-7. 초전도체·맥신·그래핀 4B-watch
...
LK-99는 독립 재현 실패와 copper sulfide impurity explanation이 나온 뒤 speculative science theme의 핵심 4B/4C 기준 사례가 됐다. ([arXiv][9])
...
금강산 시설 철거와 남북 도로·철도 폭파는 `NORTH_KOREA_POLICY_EVENT`의 hard 4C다. ([Reuters][8])
...
LK-99는 독립 재현 실패와 Cu₂S 불순물 설명 때문에 `SPECULATIVE_SCIENCE_THEME`의 hard 4C 기준 사례다. ([arXiv][9])
...
Abbott diagnostics 매출 감소는 진단키트 one-off 구조를 보여준다. 전염병 검사 매출이 구조적 진단 플랫폼 매출로 바뀌지 않으면 Stage 3-Green은 막아야 한다. ([Reuters][10])
...
Moderna의 bird-flu vaccine funding 취소 사례는 “질병 위험이 커서 정부가 계속 돈을 넣을 것”이라는 가정이 틀릴 수 있음을 보여준다. 정부 조달형 바이오·방역주는 계약 지속성과 정책 방향을 같이 봐야 한다. ([AP News][7])
...
우크라 재건은 실제 financing·참여기업·자산이 붙으면 Stage 2가 될 수 있지만, 선언·회의·MOU만 있으면 event premium으로 둬야 한다. ([Reuters][2])
...
폭염·황사·재난복구는 실제 전력망·냉각·VPP·ESS·복구 계약으로 승격되지 않으면 one-off로 분류해야 한다. ([arXiv][4])

---

## 7-7. 전략자원 수출통제 premium unwind

```text
4C-watch:
export_control_relief
truce_extension
license_approval_recovery
price_spike_normalization
no_domestic_capacity
no_offtake_contract
```

희토류 수출통제는 강한 macro evidence지만, 수출허가가 개선되거나 truce가 연장되면 event premium이 빠질 수 있다. 그래서 개별 종목은 생산능력·offtake·price floor 없이는 Stage 3-Green을 막아야 한다. ([Reuters][3])

---

## 7-8. 정책 코멘트로 인한 price-path break
...
| `STRATEGIC_SUPPLY_CHAIN_EXPORT_CONTROL_EVENT` |      12 |         12 |         17 |         12 |         8 |       0 |    5 | 수출통제 완화, 생산능력 부재, 가격정상화             |
| `EXPORT_CONTROL_TO_OFFTAKE_ESCALATION`        |      16 |         17 |         17 |         13 |        10 |       1 |    5 | offtake 부재, price floor 부재, 실행력     |
...
| `TOURISM_POLICY_EVENT`                        |       8 |          7 |          4 |         10 |         6 |       0 |    5 | 실제 spend·drop amount·OPM 미확인        |
| `POLICY_MARKET_SHOCK_EVENT`                   |    gate |       gate |       gate |       gate |      gate |    gate | gate | 세금·분배·규제 발언 shock                   |
| `AI_WINDFALL_CITIZEN_DIVIDEND_POLICY_SHOCK`   |    gate |       gate |       gate |       gate |      gate |    gate | gate | AI 세수·시민배당 발언, crowded trade unwind |
...
```text
1. STRATEGIC_SUPPLY_CHAIN_EXPORT_CONTROL_EVENT를 추가.
   희토류 수출통제처럼 실제 supply-chain bottleneck을 만드는 정책 이벤트를 별도 관리한다.

2. EXPORT_CONTROL_TO_OFFTAKE_ESCALATION을 추가.
   수출통제가 실제 대체공급계약, price floor, offtake로 넘어가면 Stage 2~3 후보가 된다.

3. GOVERNMENT_STOCKPILE_REVENUE_GUIDANCE를 유지/강화.
   Bavarian Nordic처럼 정부 stockpile 계약이 매출·EBITDA 가이던스를 올리면 단순 질병 이벤트보다 강하다.

4. PUBLIC_HEALTH_PROCUREMENT_REVERSAL을 gate로 유지.
   Moderna BARDA funding 취소처럼 정부 조달·보조금은 정책 방향이 바뀌면 바로 4C-watch다.

5. CRITICAL_INFRA_RECONSTRUCTION_FINANCING을 강화.
   우크라 재건도 통신망·전력망·항만·변압기 보호시설처럼 실제 financing과 asset이 붙으면 Stage 2 후보가 된다.

6. CLIMATE_EVENT_TO_GRID_INFRA를 강화.
   폭염 자체는 event지만, 전력망·VPP·plug-in battery program으로 넘어가면 다른 대섹터 구조수요로 이관 가능하다.

7. TOURISM_POLICY_EVENT를 별도 분리.
   비자정책은 Stage 1이고, 실제 spend·drop amount·OPM 전까지 Green 금지다.

8. POLICY_MARKET_SHOCK_EVENT와 AI_WINDFALL_CITIZEN_DIVIDEND_POLICY_SHOCK를 gate로 유지.
   테마 자체가 틀리지 않아도 정책 발언이 가격경로를 깨뜨릴 수 있기 때문이다.

9. SPECULATIVE_SCIENCE_THEME은 Red 고정.
   LK-99가 기준 반례다.
```
...
```text
Stage 1:
수출통제, export license delay, 가격 급등, 공급망 병목 발생

Stage 2:
대체 공급원, 정부지원, 고객계약, offtake, price floor 확인

Stage 3:
실제 생산·매출·FCF와 다년 공급계약이 확인될 때

Stage 4B:
수출통제 뉴스 하나로 관련주 무차별 급등

Stage 4C:
수출허가 정상화, truce extension, 가격 급락, 개별 기업 생산능력 부재 노출
```
...
```text
Stage 1:
비자완화, 단체관광 허용, 관광정책 발표

Stage 2:
관광객 수, 객단가, 면세 매출, casino drop amount, hotel RevPAR 확인

Stage 3:
OPM과 FCF가 실제로 개선되고 정책 종료 후에도 수요가 유지될 때

Stage 4B:
관광정책 뉴스만으로 관련주 동반 급등

Stage 4C:
관광객 mix 약화, spend 부진, 정책 종료, 면세·카지노 실적 부진
```
...
SUPPLY_CHAIN_EXPORT_CONTROL_EVENT:
수출통제와 허가 지연이 macro bottleneck을 만들었지만 개별 기업 Green은 아님.

EXPORT_CONTROL_TO_OFFTAKE_ESCALATION:
수출통제 이벤트가 실제 대체공급·offtake·price floor·생산능력으로 승격.
...
TOURISM_POLICY_EVENT_ONLY:
비자·관광정책은 있으나 spend/drop amount/OPM 미확인.
...
## 이번 R11 Loop 6에서 우선 검증할 가격 case

| case_id                                      |    stage2 후보일 | 현재 1차 가격판정                                      |
| -------------------------------------------- | ------------: | ----------------------------------------------- |
| `bavarian_nordic_us_stockpile_contract_case` |    2026-05-11 | 정부계약 + 가이던스 상향, event-to-contract               |
| `moderna_barda_contract_cancel_case`         |    2025-05~08 | government funding withdrawal 4C-watch          |
| `ukraine_ebrd_power_port_concession_case`    |    2026-05-15 | critical infra financing reference              |
| `china_rare_earth_export_delay_case`         | 2026-05-13~15 | supply-chain export-control macro evidence      |
...
R11 Loop 6에서는 아래 필드를 채우게 해야 한다.
...
tourism_policy_event_only_flag
...
수출통제 이벤트가 실제 대체공급·offtake·price floor·생산능력으로 연결되는 경우
...
재건 financing 실패
기후·재난 수요 one-off
수출통제 완화로 인한 strategic material premium unwind
세금·분배·규제 정책 shock으로 인한 price-path break
예산·계약·수주·공사착수 detail 부족으로 인한 disclosure confidence cap
```

**R11 Loop 6 점수정규화의 핵심 문장:**

> 정책·지정학·재난·이벤트는 “뉴스가 크다”가 아니라 **실제 계약, 예산, 정부 주문, financing, 공사착수, 반복 매출, EPS/FCF 전환**으로 봐야 한다.
> Loop 6부터는 특히 `GOVERNMENT_STOCKPILE_REVENUE_GUIDANCE`, `PUBLIC_HEALTH_PROCUREMENT_REVERSAL`, `CRITICAL_INFRA_RECONSTRUCTION_FINANCING`, `STRATEGIC_SUPPLY_CHAIN_EXPORT_CONTROL_EVENT`, `EXPORT_CONTROL_TO_OFFTAKE_ESCALATION`, `CLIMATE_EVENT_TO_GRID_INFRA`, `TOURISM_POLICY_EVENT`, `SPECULATIVE_SCIENCE_THEME`, `AI_WINDFALL_CITIZEN_DIVIDEND_POLICY_SHOCK`, `DISCLOSURE_CONFIDENCE_CAP`을 강한 보정축으로 넣어야 한다.

다음 순서는 **R12 — 농업·생활서비스·기타 Loop 6**다.

[1]: https://www.reuters.com/business/healthcare-pharmaceuticals/denmarks-bavarian-nordic-raises-2026-forecast-2026-05-11/ "https://www.reuters.com/business/healthcare-pharmaceuticals/denmarks-bavarian-nordic-raises-2026-forecast-2026-05-11/"
[2]: https://www.reuters.com/sustainability/climate-energy/ebrd-backs-privatisation-efforts-ukraine-could-provide-funding-2026-05-15/ "https://www.reuters.com/sustainability/climate-energy/ebrd-backs-privatisation-efforts-ukraine-could-provide-funding-2026-05-15/"
[3]: https://www.reuters.com/business/aerospace-defense/china-still-drags-its-feet-rare-earths-sometimes-says-greer-2026-05-15/ "https://www.reuters.com/business/aerospace-defense/china-still-drags-its-feet-rare-earths-sometimes-says-greer-2026-05-15/"
[4]: https://arxiv.org/abs/2507.13534 "https://arxiv.org/abs/2507.13534"
[5]: https://apnews.com/article/26ebb0eafe344a661bf722a99a070371 "https://apnews.com/article/26ebb0eafe344a661bf722a99a070371"
...
[7]: https://apnews.com/article/ca14251649cf6a065342196a8b021f92 "https://apnews.com/article/ca14251649cf6a065342196a8b021f92"
[8]: https://www.reuters.com/world/asia-pacific/north-korea-dismantling-facility-near-border-separated-families-seoul-says-2025-02-13/ "https://www.reuters.com/world/asia-pacific/north-korea-dismantling-facility-near-border-separated-families-seoul-says-2025-02-13/"
[9]: https://arxiv.org/abs/2308.03544 "https://arxiv.org/abs/2308.03544"
[10]: https://www.reuters.com/business/healthcare-pharmaceuticals/abbott-posts-higher-quarterly-revenue-strong-medical-device-demand-2025-10-15/ "https://www.reuters.com/business/healthcare-pharmaceuticals/abbott-posts-higher-quarterly-revenue-strong-medical-device-demand-2025-10-15/"
