좋아. **R13 Loop 7까지 끝났으니, 규칙대로 다시 R1로 돌아와서 이번은 R1 Loop 8 — 산업재·수주·인프라**다.

이번 R1 Loop 8은 이전 R1보다 더 엄격하게 간다. R1은 전력설비, 전선·케이블, 피팅밸브, 조선, 방산, 원전, 철도, 건설기계, 우크라 재건, 네옴시티를 흡수한다. 이 대섹터는 성공 사례가 많이 나올 수 있지만, 수주 뉴스만으로는 부족하고 **계약질 + 마진 + EPS 상향 + 주가 리레이팅**이 같이 가야 한다는 기준을 유지한다.

서생원 원칙도 그대로 깐다. R1에서 찾는 건 “수주가 많은 회사”가 아니라, **산업 구조 변화로 EPS/FCF 체급이 바뀌는데 시장이 아직 과거 프레임으로 낮게 보는 회사**다. 전력망·변압기·가스터빈·방산·철도·원전 모두 같은 공식으로 본다.

공시 기반으로는 OpenDART list만 보고 Stage 3를 주면 안 된다. 단일판매·공급계약, 신규시설투자, 잠정실적, 영업실적 전망, 유상증자, CB/BW, 감사의견, 거래정지, 계약 해지·정정 같은 watch disclosure는 detail에서 계약금액, 계약기간, 상대방, 매출 대비 계약금액, OP YoY, 희석률을 실제 확인해야 한다. 없는 값은 만들지 않는다.

---

# R1 Loop 8. 산업재·수주·인프라
...
R1 = 산업재·수주·인프라
Loop 8 목표 =
전력설비·변압기·가스터빈·전력망·방산·철도·원전·조선·재건 후보를
“수주 뉴스”가 아니라
...
```text
Stage 1:
AI 데이터센터 전력수요, 전력망 부족, 방산 재무장, 철도투자, 원전 재가동, 미국 조선 재건, 재건정책 뉴스

Stage 2:
계약금액, 계약기간, 고객명, 납품스케줄, backlog, slot reservation, PPA, 정부계약, 수주공시 확인

Stage 3:
OP/EPS/FCF 상향, margin 개선, 자본환원, 수주잔고의 질 개선, 실제 가격경로 동행

Stage 4B:
모두가 전력설비·방산·원전·AI power 수혜를 인정해서 valuation이 먼저 간 구간

Stage 4C:
대규모 증자, 저마진 수주, 납기 지연, 관세·원가 shock, wind loss, grid approval delay, MoU-only, 회계·공시 문제
```

R1 Loop 8의 핵심 분리는 이거다.
...
### 4-1. GE Vernova — R1 Loop 8의 가장 강한 `stage → price-path` 성공사례

GE Vernova는 R1에서 점수표가 가장 잘 맞은 사례다. AI 데이터센터와 전력망 수요가 실제 backlog, guidance, margin, 주가 반응으로 연결됐다. 회사는 2026년 매출 전망을 445억~455억 달러로 올렸고, adjusted EBITDA margin 전망도 12~14%로 상향했다. backlog는 130억 달러 증가해 1,630억 달러가 됐으며, 2027년까지 2,000억 달러에 도달할 수 있다고 봤다. 발표 후 주가는 13% 넘게 올라 사상 최고가를 기록했다. ([Reuters][1])

GE Vernova의 핵심은 “AI 전력수요”라는 headline이 아니라, electrification과 power 사업의 숫자가 동시에 바뀐 점이다. power unit core profit은 57% 증가했고, electrification profit은 두 배 이상 증가했다. 다만 wind segment 손실, 2026년 2.5억~3.5억 달러 tariff cost는 RedTeam으로 남는다. ([Reuters][1])
...
Siemens Energy는 “수주잔고가 현금흐름과 자본환원으로 넘어가면 R1 점수가 올라간다”는 기준사례다. 회사는 2분기 pre-tax free cash flow가 42% 증가했고, AI 데이터센터 전력수요의 수혜를 받으면서 2026년 자사주 매입 규모를 기존 20억 유로에서 30억 유로로 앞당기기로 했다. 기존 전체 60억 유로 buyback 프로그램의 총액은 유지됐다. ([Reuters][2])
...
다만 해당 기사에서 당일 주가 반응은 명시되지 않으므로
...
미국 전력망은 데이터센터, EV, 공장, 재생에너지, grid modernization 때문에 변압기 병목이 심해졌다. 2019년 이후 미국 GSU transformer 수요는 274%, substation transformer 수요는 116% 증가했고, 변압기 가격은 5년간 약 80% 상승했으며, 대형 변압기 lead time은 최대 4년까지 길어졌다. ([Reuters][3])

이 macro bottleneck이 개별 계약으로 연결된 사례가 LS Electric이다. LS Electric은 2025년 11월 미국 utility와 3.12억 달러 규모 계약을 맺고, 미국 남동부 대형 데이터센터에 525kV 초고압 변압기를 2027~2029년 공급하기로 했다. ([Reuters][3])
...
하지만 이 source만으로 event 이후 MFE/MAE를 확정하지 않음.
...
현대로템은 모로코 국영철도 ONCF로부터 약 2.2조 원, 15.4억 달러 규모 이층 전동차 수주를 확보했다. 이는 현대로템 철도사업 사상 최대 수주다. ([Reuters][4])
...
Constellation은 Microsoft 데이터센터에 전력을 공급하기 위해 Three Mile Island의 Crane Clean Energy Center 재가동을 추진하고 있다. 회사는 FERC 결정이 2026년 6~7월에 나올 수 있다고 봤고, 핵심은 Eddystone gas plant의 grid injection rights를 Crane으로 이전해 2031년까지 grid connection이 밀리는 위험을 피하는 것이다. ([Reuters][5])

Barron’s 기준으로 Constellation은 Q1 조정 EPS와 매출을 모두 상회했지만, full-year guidance를 유지하자 주가는 2.3% 하락했다. 즉 **PPA와 원전 재가동 narrative는 Stage 2지만, 규제·grid rights·guidance가 가격경로를 제한**한다. ([Barron's][6])
...
한화에어로스페이스는 방산 수요와 생산능력 확대 narrative가 강하지만, 대규모 자본조달은 hard RedTeam이다. 회사가 3.6조 원 규모 증자를 발표한 뒤 금융감독원이 정정신고서 제출을 요구했고, 해당 filing이 투자자 의사결정에 필요한 정보를 충분히 제공하지 못했다고 밝혔다. 증자 발표 다음 날 주가는 13% 하락해 2016년 11월 이후 최악의 하루를 기록했다. ([Reuters][7])
...
HD Hyundai Heavy Industries와 Huntington Ingalls는 미국 해군 보조함 공동건조를 위한 memorandum of agreement를 체결했다. 양사는 미국 내 새 조선소 건설 또는 기존 시설 인수를 포함한 공동투자 가능성을 검토한다. 이 흐름은 미국 조선산업 재건과 한국의 1,500억 달러 규모 조선 투자 pledge와 연결된다. ([Reuters][8])

하지만 이건 아직 실제 vessel contract, 계약금액, 인도스케줄, yard CAPEX, margin이 확인된 Stage 3가 아니다. 관세협상 세부사항도 확정되지 않았다. ([Reuters][8])
...
### 5-3. GE Vernova도 wind loss·tariff cost가 남으면 4B/4C-watch

GE Vernova는 성공사례이지만, 완전 무위험 성공은 아니다. 같은 실적 발표에서 wind segment revenue는 23% 감소했고 손실은 3.82억 달러로 확대됐다. 또 2026년 global tariffs 비용은 2.5억~3.5억 달러로 예상된다. ([Reuters][1])
...
### 5-4. Siemens — orders는 강하지만 profit miss면 Stage 3 확정은 보류

Siemens는 데이터센터, utilities, defense 수요 덕분에 주문이 11% 증가했고 record order backlog는 1,240억 유로였다. 하지만 2분기 sales는 197.6억 유로로 예상 201.4억 유로를 밑돌았고, industrial profit은 8% 감소해 예상치를 하회했다. 주가는 early trading에서 1.7% 하락했다. ([Reuters][9])
...
### 5-5. Oklo / SMR — 규제 milestone은 있어도 no revenue면 Stage 3 금지

Oklo는 advanced nuclear/SMR narrative가 강하지만, 아직 no revenue 상태다. Q1 2026 순손실은 3,310만 달러로 전년 980만 달러보다 크게 확대됐다. Barron’s는 NRC가 Aurora powerhouse의 Principal Design Criteria를 승인한 것이 중요한 milestone이라고 보도했지만, 상업운전 목표는 2028년이고 현금·단기채 등 유동성으로 시간을 버티는 구조다. ([Barron's][10])

IBD는 Oklo가 Q1 실적 후 5.2% 하락했다고 보도했다. ([Investors.com][11])
...
Q1 이후 주가 -5.2%는 capital/cash burn gate가 필요하다는 신호.
...
R1 Loop 8부터 기본 점수표는 이렇게 재정규화한다.
...
5. valuation room / 4B 여지                 7점
6. capital discipline / 자본배분 / dilution 10점
7. disclosure confidence / RedTeam          8점
...
Siemens Energy = FCF+buyback으로 Stage 3 후보, price backfill 필요
```
...
| Siemens Energy     |      Stage 3 후보 | FCF +42%, buyback acceleration, 주가 backfill 필요 | 사업 stage 강함                   | Capital/FCF 가중치 상향             |
| LS Electric 525kV  |         Stage 2 |                계약 detail 강함, price backfill 필요 | Stage 2 cap이 맞음               | 계약 visibility 상향, EPS cap 유지   |
| Hyundai Rotem ONCF |         Stage 2 |              $1.54B 최대 철도수주, price backfill 필요 | Stage 2 cap이 맞음               | rail margin/warranty gate 강화   |
| Constellation TMI  |         Stage 2 |                             Q1 beat에도 주가 -2.3% | PPA alone 부족                  | grid approval/guidance gate 강화 |
| Hanwha Aerospace   |  Stage 4C-watch |                                      증자 후 -13% | Capital RedTeam 매우 잘 맞음       | dilution penalty 강화            |
| HD Hyundai-HII     |       Stage 1~2 |                                       MoA only | Green 차단이 맞음                  | MoU/MoA cap 강화                 |
| Siemens            |      Stage 2 실패 |                  orders +11%, but shares -1.7% | orders-only false-positive 방지 | profit/margin gate 강화          |
| Oklo               | Stage 1~2 Watch |                         Q1 후 -5.2%, no revenue | pre-revenue nuclear cap 맞음    | SMR no-revenue penalty 강화      |

---

### 6-3. R1 Loop 8 점수비중 재조정
...
| capital discipline    |        보조 |       강한 상향. Hanwha/Siemens Energy |
...
### 6-4. R1 Loop 8 archetype별 최종 stage 규칙
...
Siemens = orders +11%에도 profit miss로 주가 -1.7%
Oklo = no revenue SMR, loss 확대, 주가 -5.2%
```

**R1 Loop 8 점수정규화의 핵심 문장:**
...
> 이번 Loop 8에서는 `GE Vernova`가 Stage 2→3 포착과 주가 상승이 가장 잘 맞은 사례이고, `Hanwha Aerospace`, `Siemens`, `Oklo`, `HD Hyundai-HII MoA-only`가 R1 RedTeam이 실제 가격·사업경로와 맞는 반례다.

다음 순서는 **R2 — AI·반도체·전자부품 Loop 8**다.
...
[9]: https://www.reuters.com/business/siemens-q2-profit-misses-forecasts-orders-surge-2026-05-13/?utm_source=chatgpt.com "Siemens orders rise more than expected despite Iranian war tension"
[10]: https://www.barrons.com/articles/oklos-regulatory-nuclear-earnings-stock-8228360e?utm_source=chatgpt.com "Oklo's Losses Widen. A Major Regulatory Win Keeps the Nuclear Dream Alive."
[11]: https://www.investors.com/news/oklo-stock-earnings-july-4-criticality-goal-nuclear-stocks/?utm_source=chatgpt.com "Oklo Sinks On Earnings. Why Nuclear Startup Is Revving Up For July 4."
