좋아. **R10 Loop 4까지 끝났으니, 이번은 R11 Loop 4 — 정책·지정학·재난·이벤트**로 간다.

R11은 종목을 새로 많이 뽑는 라운드라기보다, **테마가 “진짜 계약·예산·반복매출”로 승격됐는지 걸러내는 방어 라운드**다. Theme Tag Map 기준으로도 남북경협, DMZ, 개성공단, 금강산 관광, 황사·마스크, 엠폭스, 코로나 진단, 초전도체, 맥신, 그래핀, 양자기술 같은 태그는 대부분 `Event/Red`, `Watch/Red`, `SPECULATIVE_SCIENCE_THEME` 쪽으로 분류되고, Stage 3-Green보다 RedTeam/4B 방어가 중요하다.
...
## 4-1. Bavarian Nordic — 전염병 이벤트가 정부계약·가이던스로 승격되는 경우
...
## 4-2. Bavarian Nordic 2024 mpox 가격반응 — event premium과 계약 검증의 기준

Bavarian Nordic은 2024년에도 mpox 관련 수요와 주문으로 주가가 크게 움직였다. 2024년 2분기 매출 beat와 44만 도즈 유럽 주문 발표 후 주가가 12% 뛰었고, 한 달 기준으로는 45% 이상 상승했다. 이 케이스의 순서는 **전염병 뉴스 → 단기 주가 급등 → 실제 정부 주문·가이던스·반복 stockpile 여부 확인**이다. ([Investopedia][2])
...
의미:
전염병 관련주 급등은 Stage 1 event premium.
정부계약·가이던스·반복 비축이 붙으면 Stage 2.
그렇지 않으면 one-off event.
```

---

## 4-3. 우크라이나 통신망 복구 — 재건이 실제 financing으로 승격되는 경우
...
## 4-4. 폭염·냉각·전력망 — 이벤트가 다른 대섹터 구조증거로 넘어가는 경우

폭염 자체는 `CLIMATE_DISASTER_EVENT`지만, 전력망·냉각·수요반응·VPP로 연결되면 R1/R2/R3와 교차하는 구조증거가 될 수 있다. 독일 사례 연구는 가정용 이동식 에어컨 보급률이 폭염 시나리오에서 19%에서 35%로 올라가면 피크 전력수요가 12.9GW 이상 증가할 수 있고, 오후 피크가 태양광 출력 저하 시간과 겹쳐 전력계통 안정성 부담을 키울 수 있다고 분석했다. ([arXiv][4])

뉴욕에서는 폭염 피크 시간대 창문형 에어컨 부하를 줄이기 위해 임차인에게 plug-in battery를 제공하는 시범 프로그램이 진행 중이고, 200kW flexible capacity에서 2MW 이상·1,000가구 이상으로 확장될 예정이라고 보도됐다. 이건 폭염 테마가 단순 에어컨 이벤트가 아니라 **수요반응·분산배터리·VPP**로 연결될 수 있음을 보여준다. ([AP News][5])
...
남북경협은 R11에서 기본값을 `Event/Red`로 유지한다. 북한은 금강산 지역의 이산가족 상봉 시설을 철거하고 있고, 한국 정부는 이를 반인도적 행위로 비판했다. Reuters는 북한이 한국을 “적대국”으로 규정했고, 남북 도로·철도 일부도 폭파한 적이 있다고 보도했다. 이런 환경에서는 금강산 관광, 개성공단, 남북철도, DMZ, 북한 광물개발 관련주는 실제 사업 재개·제재 완화·정부 승인·현금흐름 전까지 Green을 주면 안 된다. ([Reuters][6])
...
LK-99는 R11에서 가장 중요한 과학 테마 반례다. 독립 재현 연구는 실온·상압 초전도성을 확인하지 못했다. 한 arXiv 연구는 LK-99 샘플이 실온에서 초전도 신호를 보이지 않았고, SQUID magnetization 측정에서도 실온 초전도 신호가 없었다고 보고했다. ([arXiv][7])

또 다른 연구는 원 논문에서 초전도 증거처럼 보인 저항·열용량 이상이 Cu₂S, 즉 copper(I) sulfide의 구조전이에 의해 설명될 수 있다고 지적했다. 즉 LK-99는 **preprint·SNS·영상 기반 테마가 실제 재현·상용화·매출로 넘어가지 못한 대표 사례**다. ([arXiv][8])
...
## 5-4. 정책 코멘트 shock — 이벤트가 시장 전체를 흔드는 경우
...
Bavarian Nordic은 2024년 mpox 기대와 주문으로 주가가 단기간 크게 움직였고, 2026년에는 실제 미국 정부 계약과 가이던스 상향이 붙었다. 그래서 이 케이스는 “이벤트 급등”과 “계약 승격”을 분리하는 기준으로 써야 한다. ([Investopedia][2])
...
금강산 시설 철거와 남북 도로·철도 폭파 사례는 남북경협 테마가 언제든 4C로 전환될 수 있음을 보여준다. ([Reuters][6])

---

## 6-4. 초전도체·맥신·그래핀 4B-watch
...
LK-99는 독립 재현 실패와 copper sulfide impurity explanation이 나온 뒤 speculative science theme의 핵심 4B/4C 기준 사례가 됐다. ([arXiv][7])

---

## 6-5. 폭염·전력망 테마 4B-watch
...
금강산 시설 철거와 남북 도로·철도 폭파는 `NORTH_KOREA_POLICY_EVENT`의 hard 4C다. ([Reuters][6])
...
LK-99는 독립 재현 실패와 Cu₂S 불순물 설명 때문에 `SPECULATIVE_SCIENCE_THEME`의 hard 4C 기준 사례다. ([arXiv][7])
...
## 7-6. 정책 코멘트로 인한 price-path break
...
```text
1. REAL_RECONSTRUCTION_FINANCING을 GEOPOLITICAL_RECONSTRUCTION에서 분리.
   우크라 재건도 EBRD/IFC financing처럼 실제 자금·운영회사·자산이 붙으면 Stage 2 후보가 된다.

2. CLIMATE_EVENT_TO_GRID_INFRA를 CLIMATE_DISASTER_EVENT에서 분리.
   폭염 자체는 event지만, 전력망·VPP·battery pilot로 넘어가면 다른 대섹터 구조수요로 이관 가능하다.

3. EVENT_TO_CONTRACT_ESCALATION을 유지/강화.
   Bavarian Nordic처럼 정부계약과 가이던스 상향이 붙는 예외가 있기 때문이다.

4. NORTH_KOREA_POLICY_EVENT은 더 보수적으로 유지.
   시설철거·군사긴장·제재 리스크가 강하기 때문이다.

5. SPECULATIVE_SCIENCE_THEME은 Red 고정.
   LK-99가 기준 반례다.

6. POLICY_MARKET_SHOCK_EVENT는 gate로 유지.
...
Stage 2:
계약금액, 정부 주문, stockpile, 예산, financing, 공사착수 확인

Stage 3:
반복 조달 또는 다년 매출 visibility 확인

Stage 4B:
계약 하나를 전체 테마로 과잉 일반화

Stage 4C:
계약 반복 실패, 예산 지연, 매출 가이던스 하향
```
...
## 이번 R11 Loop 4에서 우선 검증할 가격 case

| case_id                                      |    stage2 후보일 | 현재 1차 가격판정                                        |
| -------------------------------------------- | ------------: | ------------------------------------------------- |
| `bavarian_nordic_us_stockpile_contract_case` |    2026-05-11 | 정부계약 + 가이던스 상향, event-to-contract                 |
| `bavarian_nordic_2024_mpox_order_case`       |       2024-08 | +12%, 1개월 +45%+, event premium with validation    |
...
| `lk99_cu2s_impurity_case`                    |       2023-08 | impurity explanation, hard 4C                     |
...
R11 Loop 4에서는 아래 필드를 채우게 해야 한다.
...
**R11 Loop 4 점수정규화의 핵심 문장:**

> 정책·지정학·재난·이벤트는 “뉴스가 크다”가 아니라 **실제 계약, 예산, 정부 주문, financing, 공사착수, 반복 매출, EPS/FCF 전환**으로 봐야 한다.
> 그 증거가 없으면 대부분은 `event_premium`, `price_moved_without_evidence`, `one_off_revenue`, `speculative_science_failure`, `policy_market_shock`다.

다음 순서는 **R12 — 농업·생활서비스·기타 Loop 4**다.
...
[2]: https://www.investopedia.com/bavarian-nordic-stock-jumps-on-revenue-beat-mpox-vaccine-order-8699720?utm_source=chatgpt.com "Bavarian Nordic Stock Jumps on Revenue Beat, Mpox Vaccine Order"
...
[4]: https://arxiv.org/abs/2507.13534?utm_source=chatgpt.com "The impact of heatwave-driven air conditioning adoption on electricity demand: A spatio-temporal case study for Germany"
[5]: https://apnews.com/article/26ebb0eafe344a661bf722a99a070371?utm_source=chatgpt.com "Air conditioning battery program for renters could help cities manage grid stress during heat waves"
...
[7]: https://arxiv.org/abs/2308.03544?utm_source=chatgpt.com "Absence of superconductivity in LK-99 at ambient conditions"
[8]: https://arxiv.org/abs/2308.05222?utm_source=chatgpt.com "Superionic phase transition of copper(I) sulfide and its implication for purported superconductivity of LK-99"
