좋아. **R12 Loop 4까지 끝났으니, 이번은 R13 Loop 4 — Cross-archetype RedTeam / 4B / 회계신뢰도 / 가격검증 총정리**로 간다.

R13은 새 대섹터가 아니라 **R1~R12 전체 후보를 마지막으로 걸러내는 공통 검문소**다. 지금까지 Theme Tag Map 기준으로 12개 대섹터가 전력설비·HBM·ESS·희토류·K푸드·밸류업·CDMO·SaaS·로보택시·데이터센터·정책이벤트·스마트팜까지 대부분의 테마를 흡수했고, R13은 그 후보들이 진짜 구조적 E2R인지, 이미 4B 졸업 구간인지, 아니면 테마·사이클·이벤트·회계 리스크인지 분류하는 층이다.

또 Checkpoint 20 원칙상 계약금액, 계약기간, 매출 대비 계약금액, OP YoY, dilution, 감사의견, 거래정지, 공시 detail 값은 **실제 확인된 필드만 써야 하고 없는 값을 만들면 안 된다.** R13은 이 원칙을 모든 섹터에 강제하는 마지막 방어막이다.

서생원식으로도 R13의 역할은 명확하다. “좋아 보이는 테마”가 아니라 **산업 구조 변화 → EPS/FCF 체급 변화 → 시장의 과거 프레임 오해 → 밸류에이션 리레이팅**이 실제로 연결되는지만 남겨야 한다.

---

# R13 Loop 4. Cross-archetype RedTeam / 4B / 회계신뢰도 / 가격검증 총정리
...
| `SECTOR_SUCCESS_BUT_4B_WATCH`      | 구조는 맞지만 시장이 이미 대부분 인정한 상태      | 비중축소·졸업 감시        |
...
| `DISCLOSURE_CONFIDENCE_CAP`        | 계약값·고객명·용도·마진 미공개              | Stage 3 제한        |
...
SK하이닉스는 R13 기준으로 가장 강한 `STRUCTURAL_SUCCESS_ALIGNED` 예시다. AI 서버용 HBM과 전통 메모리 수요가 동시에 붙으면서 SK하이닉스 주가는 2025년에 274%, 2026년에 200% 이상 상승했고, 시가총액은 약 9,420억 달러까지 올라 1조 달러에 근접했다. 이건 “AI 테마”가 아니라 **HBM 병목 → EPS/FCF 체급 변화 → 시장 프레임 전환 → 가격경로 리레이팅**이 연결된 케이스다. ([Reuters][1])
...
## 4-2. Circle / USDC — `REGULATED_STABLECOIN_INFRA`

Circle은 regulated fiat-backed stablecoin infrastructure의 좋은 reference다. 2026년 1분기 revenue/reserve income은 20% 증가한 6.94억 달러였고, USDC circulation은 전년 대비 28% 증가한 770억 달러였다. 다만 Circle은 미국 국채·은행예금 준비금에서 이자수익을 얻기 때문에 금리와 규제에 민감하고, IPO 가격 31달러 대비 주가가 3배 이상 오른 상태라 4B-watch도 같이 붙는다. ([Reuters][2])
...
## 4-3. `EVENT_TO_CONTRACT_ESCALATION` — 이벤트가 실제 계약으로 승격되는 경우
...
## 4-4. `REAL_ASSET_WITH_CASHFLOW_PROOF` — 데이터센터·콜드체인·전력설비의 진짜 승격 조건

AI 데이터센터, 콜드체인, 전력설비 같은 real asset 테마는 수요가 커 보이기 쉽다. 그러나 R13에서는 **자산 보유 → tenant/계약 → power/water/permits → NOI/AFFO/FCF → 가격경로** 순서가 확인되어야만 Stage 3 후보가 된다.

```text
Stage 1:
AI data-center, cold-chain, grid infrastructure narrative

Stage 2:
자산 취득, tenant lease, 계약, power/water secured, 수주잔고

Stage 3:
NOI/AFFO/FCF, 배당 커버리지, OP/EPS 상향, 가격경로 alignment
```

이 원칙은 R1 전력설비, R10 데이터센터 REIT, R3 ESS, R9 satellite connectivity에 공통 적용한다.
...
Supermicro는 R13의 핵심 반례다. AI 서버 수요로 급등했지만, Ernst & Young이 Supermicro 감사인에서 사임하자 주가가 30% 이상 급락했다. EY는 governance와 financial reporting control에 대한 우려를 제기했고, 해당 사안은 annual report filing delay, Hindenburg의 회계조작 주장, DOJ 조사 보도 이후 나왔다. ([Reuters][3])
...
CrowdStrike는 보안 SaaS가 반복매출이어도 운영 신뢰가 깨지면 어떻게 무너지는지 보여준다. 2024년 7월 잘못된 software update로 800만 대 이상 컴퓨터가 영향을 받았고, CrowdStrike 주가는 이후 12일 동안 32% 하락해 약 250억 달러 시가총액이 사라졌다. Delta도 outage 비용을 두고 손해배상을 추진했다. ([Reuters][4])
...
Bluebird bio는 승인된 유전자치료제를 갖고 있었지만 severe cash crunch 속에 Carlyle·SK Capital에 주당 3달러로 비상장화되기로 했다. 이 가격은 직전 종가 대비 57.4% 할인된 수준이었고, 발표 후 주가는 36% 하락했다. Reuters는 Bluebird의 gene therapy uptake가 느렸고 cash crunch가 심각했다고 보도했다. ([Reuters][5])
...
GLP-1은 대형 성장시장처럼 보였지만, Novo Nordisk는 2026년 sales와 profit이 최대 13% 감소할 수 있다고 경고했다. 원인은 미국 가격 압박, 경쟁 심화, Wegovy 등 semaglutide 제품의 realized price 하락, 특허·copycat pressure 등으로 설명됐다. ([Reuters][6])
...
데이터센터 REIT도 AI 수혜라는 이름만으로 Green을 줄 수 없다. Hindenburg Research는 Equinix가 maintenance capex를 expansion capex로 잘못 분류해 AFFO를 부풀렸다고 주장했고, Reuters는 해당 공개 후 Equinix 주가가 약 2% 하락했다고 보도했다. ([Reuters][7])

또 Equinix는 AI inference 수요를 잡기 위해 2026~2029년 연간 CAPEX를 40억~50억 달러로 높이겠다고 밝혔지만, 매출 성장 전망과 AFFO per share 성장 전망이 기대에 못 미치자 주가가 8% 하락했다. ([Reuters][8])
...
TerraUSD/Luna는 R6 디지털자산에서 절대 잊으면 안 되는 반례다. Do Kwon은 TerraUSD 생태계 붕괴와 관련해 2025년 12월 15년형을 선고받았고, AP는 이를 400억 달러 규모 fraud로 보도했다. ([AP News][9])
...
Supermicro는 이 항목의 기준 반례다. AI 서버 매출 성장으로 리레이팅됐어도, 회계 신뢰도 hard flag가 나오면 바로 4C가 된다. ([Reuters][3])
...
Novo Nordisk 사례는 GLP-1도 “거대한 시장”만으로 방어되지 않는다는 기준이다. ([Reuters][6])
...
Equinix 사례처럼 데이터센터 REIT도 capex 분류와 AFFO 무결성을 통과해야 한다. ([Reuters][7])
...
2026년 5월 12일 한국 증시는 AI 수익·세수 재분배성 발언 이후 코스피가 장중 5.1% 하락하고 종가 기준 2.3% 하락했다. 이건 crowded rally에서는 정책 이벤트가 thesis를 깨지 않아도 price-path를 강하게 흔들 수 있음을 보여준다. ([마켓워치][10])
...
Supermicro는 이 기준을 가장 잘 보여준다. 감사인이 사임하고 경영진·감사위원회 신뢰 문제를 제기하면, AI 서버 매출 성장은 더 이상 Green을 방어하지 못한다. ([Reuters][3])
...
CrowdStrike는 보안 SaaS의 핵심 반례다. 반복매출 기업도 운영 신뢰가 깨지면 RedTeam hard review로 내려가야 한다. ([Reuters][4])
...
Bluebird bio는 승인된 치료제를 갖고도 uptake·현금흐름·상업화가 안 되면 주가가 무너질 수 있음을 보여준다. ([Reuters][5])
...
Novo Nordisk처럼 구조 성장시장에서도 가격 압박과 경쟁이 수익 경로를 훼손하면 4C가 된다. ([Reuters][6])
...
TerraUSD/Luna는 이 항목의 기준 사례다. ([AP News][9])
...
Equinix Hindenburg 사례와 CAPEX/AFFO 전망 하향은 데이터센터 REIT와 AI real asset 후보에 반드시 적용해야 하는 R13 게이트다. ([Reuters][7])
...
| `DISCLOSURE_CONFIDENCE_CAP`        |     cap |        cap |        cap |        cap |       cap |      cap |    + | Stage 3 제한             |
...
8. 정책 shock은 crowded 4B 구간의 price-path를 깨는 overlay로 둔다.

9. OpenDART detail이 없거나 고객명·계약금액·용도·마진이 미공개면 disclosure confidence cap을 건다.

10. 가격경로가 틀리면 점수체계를 재보정한다.
```
...
R13 Loop 4부터 모든 case는 아래 stage date를 강제한다.
...
R13 Loop 4에서는 특히 `stage4b_date`와 `stage4c_date`를 더 중요하게 본다.
...
## 10-4. R13 Loop 4에서 가격검증 실패 시 처리
...
R13 Loop 4부터는 모든 case library에 아래 공통 필드를 강제한다.
...
5. 4B 과열 미포화
6. disclosure confidence 충분

이 여섯 개를 통과해야 한다.
```

**R13 Loop 4 점수정규화의 핵심 문장:**
...
이제 **R1~R13 Loop 4가 완료**됐다.
다음에 같은 지시가 오면 규칙대로 **R1 — 산업재·수주·인프라 Loop 5**로 돌아가서, 지금까지 쌓은 RedTeam과 가격검증 기준을 반영해 다시 더 촘촘하게 판다.
...
[5]: https://www.reuters.com/markets/deals/bluebird-bio-be-taken-private-by-carlyle-sk-capital-amid-cash-crunch-2025-02-21/?utm_source=chatgpt.com "Gene therapy maker bluebird to go private in discounted deal amid cash crunch"
[6]: https://www.reuters.com/business/novo-nordisk-warns-2026-sales-decline-2026-02-03/?utm_source=chatgpt.com "Novo Nordisk warns on profits and sales amid Trump drug price crackdown"
[7]: https://www.reuters.com/business/media-telecom/hindenburg-research-takes-short-position-data-center-operator-equinix-2024-03-20/?utm_source=chatgpt.com "Hindenburg shorts data center firm Equinix alleging inflated profit metric"
[8]: https://www.reuters.com/business/equinix-shares-fall-revenue-capital-spending-forecast-disappoint-investors-2025-06-26/?utm_source=chatgpt.com "Equinix shares fall as revenue, capital spending forecast disappoint investors"
[9]: https://apnews.com/article/0df8b30b5f3cff2954f0a08bf0a327f3?utm_source=chatgpt.com "Crypto mogul Do Kwon sentenced to 15 years in prison for $40 billion stablecoin fraud"
