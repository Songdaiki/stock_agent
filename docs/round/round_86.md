좋아. **R6 Loop 4까지 끝났으니, 이번은 R7 Loop 4 — 바이오·헬스케어·의료기기**로 넘어간다.
...
R7 = 바이오·헬스케어·의료기기
Loop 4 목표 =
CDMO capacity / CRO funding cycle / biosimilar commercialization /
GLP-1 commercial scripts / telehealth channel / gene therapy uptake /
AI medical deployment / surgical robot installed base / device safety를 완전히 분리
```
...
Intuitive Surgical은 R7에서 가장 서생원식 구조에 가까운 의료기기 성공 후보 중 하나다. 2026년 1분기 da Vinci와 Ion procedure가 전년 대비 17% 증가했고, 매출은 23% 증가한 27.7억 달러, instruments/accessories 매출은 23% 증가한 16.9억 달러였다. 2026년 procedure growth guidance도 13.5~15.5%로 올렸고, 실적 후 주가는 7.2% 상승했다. 이 구조는 **installed base → procedure growth → 소모품/기구 반복매출 → EPS/FCF visibility**가 확인되는 좋은 reference다. ([Investors][2])
...
Eli Lilly의 oral GLP-1 비만치료제 Foundayo, 즉 orforglipron은 2026년 4월 FDA 승인을 받았다. Reuters는 Foundayo가 임상에서 12~15% 체중감량을 보였고, LillyDirect에서 월 149달러로 시작하며, Novo의 Wegovy pill과 경쟁하게 된다고 보도했다. 승인 뉴스에서 Lilly 주가는 6% 올랐다. 이는 `OBESITY_GLP1_COMMERCIALIZATION`의 강한 Stage 2 후보지만, Stage 3는 처방량·보험·가격·OP/EPS가 따라와야 한다. ([Reuters][4])
...
## 4-7. Biosimilar 접근성 — `BIOSIMILAR_COMMERCIALIZATION`

Boehringer Ingelheim과 GoodRx는 Humira biosimilar를 정가 대비 92% 할인된 cash-pay 가격으로 제공하기로 했다. Boehringer의 adalimumab-adbm은 Humira와 pharmacy substitution이 가능한 FDA-approved interchangeable biosimilar지만, Axios는 Humira biosimilar 초기 demand가 느렸다고 설명했다. 즉 biosimilar는 허가와 할인만으로 충분하지 않고, PBM incentive, 보험, 의사·환자 전환, 마진 방어가 Stage 3 조건이다. ([Axios][7])
...
Novo Nordisk는 GLP-1 시장의 대표 성장기업이지만, 2026년 sales와 operating profit이 5~13% 감소할 수 있다고 경고하자 주가가 16% 하락했고 약 500억 달러 시가총액이 사라졌다. Reuters는 미국 가격 하락 압박이 핵심이라고 설명했다. 이 사례는 GLP-1 같은 거대 TAM도 가격·보험·경쟁 압박을 이기지 못하면 4B에서 4C로 내려간다는 기준이다. ([Reuters][8])
...
Novo의 16% 급락과 500억 달러 시총 증발은 GLP-1도 TAM만으로 Green 유지가 안 된다는 기준이다. ([Reuters][8])
...
Humira biosimilar는 92% discount가 있어도 adoption은 PBM·보험·처방전환 구조를 통과해야 한다. ([Axios][7])
...
Novo의 2026년 sales·operating profit 감소 경고와 주가 16% 하락은 GLP-1의 4B→4C 전환 기준이다. ([Reuters][8])
...
| `OBESITY_GLP1_COMMERCIALIZATION`     |      22 |         20 |         12 |         13 |         9 |       0 |    6 | 경쟁, 가격, 보험, 조제약, scripts                        |
| `ORAL_GLP1_MAINTENANCE_THERAPY`      |      21 |         19 |         11 |         13 |        10 |       0 |    6 | 초기 처방량, 보험, 경쟁 pill, boxed warning              |
...
Loop 4에서 가장 크게 바뀐 건 일곱 가지다.

```text
1. CDMO_US_TARIFF_HEDGE_CAPACITY를 별도 분리.
   미국 생산거점은 전략적으로 좋지만, 고객계약·가동률·OPM 전까지 Stage 3 금지.

2. ORAL_GLP1_MAINTENANCE_THERAPY를 별도 분리.
   Foundayo approval과 maintenance data는 좋지만, scripts·보험·가격이 핵심 gate다.

3. COMPOUNDED_GLP1_REGULATORY_RISK를 gate로 격상.
   Hims 사례처럼 channel이 회색이면 GLP-1 수요가 커도 4C가 된다.

4. MEDICAL_AI_EXTERNAL_VALIDATION을 별도 분리.
   AUC와 논문은 Stage 1~2이고, 병원도입·수가·반복매출이 Stage 3다.

5. SURGICAL_ROBOT_INSTALLED_BASE는 Green 후보 유지.
   installed base + procedure growth + 소모품 매출이 반복수익 구조를 만든다.

6. GENE_THERAPY_RARE_DISEASE는 더 보수적으로 유지.
   Bluebird가 승인 후 상업화 실패의 기준 반례다.

7. BIOSIMILAR_PATENT_LITIGATION을 gate로 추가.
   biosimilar는 허가보다 launch timing, PBM, 특허소송, 마진이 더 중요하다.
```
...
| `intuitive_surgical_q1_2026_procedure_growth_case` | 2026-04-22 | +7.2%, surgical robot recurring consumable success |
...
다음 순서는 **R8 — 플랫폼·콘텐츠·SW·보안 Loop 4**다.
...
[7]: https://www.axios.com/2024/07/18/goodrx-boehringer-humira-biosimilar?utm_source=chatgpt.com "GoodRx teams with Boehringer on Humira biosimilar"
