...
from e2r.cli.build_round145_r13_loop8_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10ThemePosture
from e2r.sector.round145_r13_loop8_cross_archetype_redteam import (
...
class Round145R13Loop8CrossArchetypeRedTeamTests(unittest.TestCase):
    def test_round145_targets_cover_loop8_overlays(self):
        labels = {target.target_id for target in ROUND145_OVERLAY_TARGETS}
    ...
    def test_round145_overlay_axes_match_v8_cross_archetype_score_table(self):
        axes = {axis.axis_id: axis for axis in ROUND145_OVERLAY_AXES}

        self.assertEqual(
            {axis.axis_id: axis.weight for axis in ROUND145_OVERLAY_AXES},
            {
        ...
        self.assertEqual(
            [axis.axis_id for axis in ROUND145_OVERLAY_AXES],
            [
        ...
        rows = round145_overlay_axis_rows()
        self.assertEqual(len(rows), 7)
        self.assertTrue(all(row["production_scoring_changed"] == "false" for row in rows))

    def test_stage_caps_make_round145_final_gate_explicit(self):
        rows = {row["cap_id"]: row for row in round145_stage_cap_rows()}

        self.assertEqual(len(rows), 4)
        self.assertEqual(len(ROUND145_STAGE_CAPS), 4)
    ...
    def test_round145_new_canonical_archetypes_exist(self):
        expected = (
    ...
    def test_round145_green_and_hard_gate_rules_are_explicit(self):
        structural = round145_target_for("STRUCTURAL_SUCCESS_ALIGNED")
        sector_4b = round145_target_for("STRUCTURAL_SUCCESS_BUT_4B_WATCH")
        accounting = round145_target_for("REDTEAM_ACCOUNTING_TRUST_OVERLAY")
        stablecoin = round145_target_for("STABLECOIN_CONVERTIBILITY_RISK")
        policy_success = round145_target_for("SECTOR_SUCCESS_BUT_POLICY_SHOCK_WATCH")
        policy_shock = round145_target_for("POLICY_MARKET_SHOCK_EVENT")
        circular = round145_target_for("CIRCULAR_AI_FINANCING_WATCH")
        event_contract = round145_target_for("EVENT_TO_CONTRACT_ESCALATION")
        capex_affo = round145_target_for("CAPEX_AFFO_DILUTION_RISK")
        disclosure_cap = round145_target_for("DISCLOSURE_CONFIDENCE_CAPPED")
    ...
    def test_required_round145_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round145_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND145_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_green_guardrails(self):
        ...
        self.assertEqual(len(records), len(ROUND145_CASE_CANDIDATES))
        for record in records:
            record.validate()
            self.assertEqual(record.large_sector, ROUND145_LARGE_SECTOR)
    ...
    def test_score_profile_rows_are_overlay_not_production_scoring(self):
        rows = round145_score_profile_rows()

        self.assertEqual(len(rows), len(ROUND145_OVERLAY_TARGETS))
        for row in rows:
            self.assertEqual(row["production_scoring_changed"], "false")
    ...
    def test_stage_date_and_price_fields_cover_loop8_needs(self):
        stage_rows = {row["target_id"]: row for row in round145_stage_date_rows()}
        price_fields = {row["field"] for row in round145_price_field_rows()}
    ...
    def test_summary_and_markdown_explain_loop8_validation_layer(self):
        summary = round145_summary()
        summary_md = render_round145_summary_markdown()
        gate_plan = render_round145_redteam_gate_plan_markdown()
        price_plan = render_round145_price_validation_plan_markdown()
        ...
        self.assertEqual(summary["case_candidate_count"], 43)
        ...
        self.assertIn("R13 v8 Cross-Archetype Overlay Axes", summary_md)
        self.assertIn("`eps_fcf_roe_affo_opm_bodyweight_change`: 24", summary_md)
        self.assertIn("R13 v8 Stage Caps", summary_md)
        self.assertIn("`stage3_green_all_checks_gate`: Stage 3 / requires_all_green_checks", summary_md)
        self.assertIn("Do not apply Round145 overlay symbols", gate_plan)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round145_r13_loop8_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r13_loop8_round145.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round145_r13_loop8_v8.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND145_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round145_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round145_r13_loop8_cross_archetype_redteam", text)
