...
from e2r.cli.build_round132_r13_loop7_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10ThemePosture
from e2r.sector.round132_r13_loop7_cross_archetype_redteam import (
...
class Round132R13Loop7CrossArchetypeRedTeamTests(unittest.TestCase):
    def test_round132_targets_cover_loop7_overlays(self):
        labels = {target.target_id for target in ROUND132_OVERLAY_TARGETS}
    ...
    def test_round132_overlay_axes_match_v7_cross_archetype_score_table(self):
        axes = {axis.axis_id: axis for axis in ROUND132_OVERLAY_AXES}

        self.assertEqual(
            {axis.axis_id: axis.weight for axis in ROUND132_OVERLAY_AXES},
            {
                ...
                "valuation_room_4b_margin": 10,
                "capital_discipline_leverage_fcf": 10,
                "disclosure_confidence_redteam": 10,
            },
        )
        ...
        rows = round132_overlay_axis_rows()
        self.assertEqual(len(rows), 7)
        self.assertTrue(all(row["production_scoring_changed"] == "false" for row in rows))

    def test_round132_new_canonical_archetypes_exist(self):
        expected = (
    ...
    def test_round132_green_and_hard_gate_rules_are_explicit(self):
        structural = round132_target_for("STRUCTURAL_SUCCESS_ALIGNED")
        sector_4b = round132_target_for("STRUCTURAL_SUCCESS_BUT_4B_WATCH")
        accounting = round132_target_for("REDTEAM_ACCOUNTING_TRUST_OVERLAY")
        stablecoin = round132_target_for("STABLECOIN_CONVERTIBILITY_RISK")
        policy_success = round132_target_for("SECTOR_SUCCESS_BUT_POLICY_SHOCK_WATCH")
        policy_shock = round132_target_for("POLICY_MARKET_SHOCK_EVENT")
        circular = round132_target_for("CIRCULAR_AI_FINANCING_WATCH")
        event_contract = round132_target_for("EVENT_TO_CONTRACT_ESCALATION")
        capex_affo = round132_target_for("CAPEX_AFFO_DILUTION_RISK")
        disclosure_cap = round132_target_for("DISCLOSURE_CONFIDENCE_CAPPED")
    ...
    def test_required_round132_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round132_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND132_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_green_guardrails(self):
        ...
        self.assertEqual(len(records), len(ROUND132_CASE_CANDIDATES))
        for record in records:
            record.validate()
            self.assertEqual(record.large_sector, ROUND132_LARGE_SECTOR)
    ...
    def test_score_profile_rows_are_overlay_not_production_scoring(self):
        rows = round132_score_profile_rows()

        self.assertEqual(len(rows), len(ROUND132_OVERLAY_TARGETS))
        for row in rows:
    ...
    def test_stage_date_and_price_fields_cover_loop7_needs(self):
        stage_rows = {row["target_id"]: row for row in round132_stage_date_rows()}
        price_fields = {row["field"] for row in round132_price_field_rows()}
    ...
    def test_summary_and_markdown_explain_loop7_validation_layer(self):
        summary = round132_summary()
        summary_md = render_round132_summary_markdown()
        gate_plan = render_round132_redteam_gate_plan_markdown()
        price_plan = render_round132_price_validation_plan_markdown()
        ...
        self.assertIn("R13 v7 Cross-Archetype Overlay Axes", summary_md)
        self.assertIn("`eps_fcf_roe_affo_opm_bodyweight_change`: 24", summary_md)
        self.assertIn("Do not apply Round132 overlay symbols", gate_plan)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round132_r13_loop7_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r13_loop7_round132.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round132_r13_loop7_v7.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND132_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round132_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round132_r13_loop7_cross_archetype_redteam", text)
