...
from e2r.cli.build_round167_r9_loop10_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round167_r9_loop10_mobility_transport_leisure import (
    ROUND167_CASE_CANDIDATES,
    ROUND167_HELPER_OVERLAY_TARGET_COUNT,
    ROUND167_HELPER_OVERLAY_TARGET_IDS,
    ROUND167_PRICE_FIELDS,
    ROUND167_SCORE_TARGETS,
    ROUND167_SOURCE_CANONICAL_TARGET_COUNT,
    ROUND167_SOURCE_CANONICAL_TARGET_IDS,
    render_round167_green_guardrail_markdown,
    render_round167_price_validation_plan_markdown,
    render_round167_risk_overlay_markdown,
    render_round167_summary_markdown,
    round167_base_score_axis_rows,
    round167_case_candidate_rows,
    round167_case_records,
    round167_price_field_rows,
    round167_score_profile_rows,
    round167_stage_date_rows,
    round167_summary,
    target_for,
    write_round167_r9_loop10_reports,
)


class Round167R9Loop10MobilityTransportLeisureTests(unittest.TestCase):
    def test_round167_targets_cover_r9_loop10_archetypes(self):
        labels = {target.target_id for target in ROUND167_SCORE_TARGETS}

        self.assertEqual(len(labels), 33)
        self.assertEqual(ROUND167_SOURCE_CANONICAL_TARGET_COUNT, 28)
        self.assertEqual(ROUND167_HELPER_OVERLAY_TARGET_COUNT, 5)
        self.assertTrue(set(ROUND167_SOURCE_CANONICAL_TARGET_IDS).issubset(labels))
        self.assertEqual(
            set(ROUND167_HELPER_OVERLAY_TARGET_IDS),
            {
    ...
    def test_required_round167_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round167_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND167_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_round167_guardrails(self):
        records = round167_case_records()

        self.assertEqual(len(records), len(ROUND167_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round167_weight_table(self):
        rows = {row["target_id"]: row for row in round167_score_profile_rows()}
    ...
    def test_base_score_axes_match_round167_v10_table(self):
        axis_rows = round167_base_score_axis_rows()
        rows = {row["axis_id"]: row for row in axis_rows}

        self.assertEqual(len(rows), 7)
        self.assertEqual(
            [row["axis_id"] for row in axis_rows],
            [
        ...
        self.assertEqual(rows["eps_fcf_opm_conversion"]["weight"], "22")
        self.assertIn("op_margin_target", rows["eps_fcf_opm_conversion"]["stage2_evidence"])
        self.assertIn("op_margin_cut", rows["eps_fcf_opm_conversion"]["hard_redteam"])
        self.assertEqual(rows["contract_backlog_operating_visibility"]["weight"], "20")
        self.assertIn("paid_freight_flag", rows["contract_backlog_operating_visibility"]["stage2_evidence"])
        self.assertIn("service_area_only", rows["contract_backlog_operating_visibility"]["hard_redteam"])
        self.assertEqual(rows["unit_fleet_economics"]["weight"], "18")
        self.assertIn("cost_per_mile", rows["unit_fleet_economics"]["stage2_evidence"])
        self.assertIn("repair_cost_spike", rows["unit_fleet_economics"]["hard_redteam"])
        self.assertEqual(rows["recurrence_demand_duration"]["weight"], "12")
        self.assertIn("policy_event_only", rows["recurrence_demand_duration"]["hard_redteam"])
        self.assertEqual(rows["safety_regulatory_certification_disclosure"]["weight"], "12")
        self.assertIn("part135_without_type_certification", rows["safety_regulatory_certification_disclosure"]["hard_redteam"])
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round167_stage_date_rows()}
        fields = {row["field"] for row in round167_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND167_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r9_loop10_guardrails(self):
        summary = round167_summary()
        summary_md = render_round167_summary_markdown()
        guardrails = render_round167_green_guardrail_markdown()
        overlays = render_round167_risk_overlay_markdown()
        price_plan = render_round167_price_validation_plan_markdown()

        self.assertEqual(summary["target_count"], 33)
        self.assertEqual(summary["source_canonical_target_count"], 28)
        ...
        self.assertIn("Round 167", summary_md)
        self.assertIn("source_canonical_target_count: 28", summary_md)
        self.assertIn("helper_overlay_target_count: 5", summary_md)
        self.assertIn("Do not apply R9 Loop-10 v10.0 weights", guardrails)
        self.assertIn("R9 v10 Base Score Axes", summary_md)
        self.assertIn("unit_fleet_economics", summary_md)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round167_r9_loop10_reports(
                ...
                cases_path=Path(tmp) / "cases_r9_loop10_round167.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round167_r9_loop10_v10.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND167_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round167_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round167_r9_loop10_mobility_transport_leisure", text)
