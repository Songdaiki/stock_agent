...
from e2r.cli.build_round77_r11_loop3_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round77_r11_loop3_policy_geopolitical_event import (
...
class Round77R11Loop3PolicyGeopoliticalEventTests(unittest.TestCase):
    def test_round77_targets_cover_r11_loop3_archetypes(self):
        labels = {target.target_id for target in ROUND77_SCORE_TARGETS}
    ...
    def test_required_round77_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round77_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND77_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_round77_guardrails(self):
        records = round77_case_records()

        self.assertEqual(len(records), len(ROUND77_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round77_weight_table(self):
        rows = {row["target_id"]: row for row in round77_score_profile_rows()}
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round77_stage_date_rows()}
        fields = {row["field"] for row in round77_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND77_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r11_loop3_guardrails(self):
        summary = round77_summary()
        summary_md = render_round77_summary_markdown()
        guardrails = render_round77_green_guardrail_markdown()
        caps = render_round77_event_false_positive_caps_markdown()
        price_plan = render_round77_price_validation_plan_markdown()
        ...
        self.assertIn("Round-77", summary_md)
        self.assertIn("Do not apply these R11 Loop-3 v3 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round77_r11_loop3_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r11_loop3_round77.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round77_r11_loop3_v3.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND77_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round77_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round77_r11_loop3_policy_geopolitical_event", text)
