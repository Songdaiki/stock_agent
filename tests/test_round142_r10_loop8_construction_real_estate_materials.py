...
from e2r.cli.build_round142_r10_loop8_report import build_parser
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round142_r10_loop8_construction_real_estate_materials import (
...
class Round142R10Loop8ConstructionRealEstateMaterialsTests(unittest.TestCase):
    def test_round142_targets_cover_r10_loop8_archetypes(self):
        labels = {target.target_id for target in ROUND142_SCORE_TARGETS}
    ...
    def test_required_round142_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round142_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND142_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_round142_guardrails(self):
        records = round142_case_records()

        self.assertEqual(len(records), len(ROUND142_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round142_weight_table(self):
        rows = {row["target_id"]: row for row in round142_score_profile_rows()}
    ...
    def test_base_score_axes_match_round142_v8_weight_table(self):
        rows = {row["axis_id"]: row for row in round142_base_score_axis_rows()}

        self.assertEqual(len(rows), 7)
        self.assertEqual(len(ROUND142_BASE_SCORE_AXES), 7)
        self.assertEqual(rows["eps_fcf_affo_noi_conversion"]["score_weight"], "24")
        self.assertIn("affo_per_share_growth", rows["eps_fcf_affo_noi_conversion"]["stage3_evidence"])
        self.assertIn("dividend_cut", rows["eps_fcf_affo_noi_conversion"]["hard_redteam_flags"])
        self.assertEqual(rows["asset_tenant_contract_pf_visibility"]["score_weight"], "20")
        self.assertIn("binding_lease_flag", rows["asset_tenant_contract_pf_visibility"]["stage2_evidence"])
        self.assertIn("no_acquired_assets", rows["asset_tenant_contract_pf_visibility"]["hard_redteam_flags"])
        self.assertEqual(rows["power_water_permitting_local_acceptance"]["score_weight"], "16")
        self.assertIn("water_rights_delay", rows["power_water_permitting_local_acceptance"]["hard_redteam_flags"])
        self.assertEqual(rows["cost_price_volume_mna_synergy"]["score_weight"], "14")
        self.assertIn("volume_and_price_aligned", rows["cost_price_volume_mna_synergy"]["stage3_evidence"])
        self.assertEqual(rows["leverage_capex_affo_integrity_disclosure"]["score_weight"], "12")
        self.assertIn("maintenance_capex_misclassification", rows["leverage_capex_affo_integrity_disclosure"]["hard_redteam_flags"])
    ...
    def test_stage_caps_make_round142_stage_boundaries_explicit(self):
        rows = {row["cap_id"]: row for row in round142_stage_cap_rows()}

        self.assertEqual(len(rows), 4)
        self.assertEqual(len(ROUND142_STAGE_CAPS), 4)
        self.assertEqual(rows["stage1_headline_cap"]["score_cap"], "45")
        self.assertIn("ai_data_center_label", rows["stage1_headline_cap"]["cap_triggers"])
        self.assertIn("cash_conversion_improvement", rows["stage1_headline_cap"]["release_conditions"])
        self.assertEqual(rows["stage2_cashflow_visibility_cap"]["score_cap"], "70")
        self.assertIn("affo_per_share_or_fcf_growth", rows["stage2_cashflow_visibility_cap"]["release_conditions"])
        self.assertIn("maintenance_capex_misclassification", rows["stage2_cashflow_visibility_cap"]["hard_redteam_flags"])
        self.assertEqual(rows["stage3_cross_evidence_gate"]["score_cap"], "requires_score_above_70_and_cross_evidence")
        self.assertIn("at_least_two_independent_evidence_families", rows["stage3_cross_evidence_gate"]["release_conditions"])
        self.assertIn("price_runup_before_cashflow", rows["stage4b_4c_real_asset_breaks"]["cap_triggers"])
        for row in rows.values():
            self.assertEqual(row["production_scoring_changed"], "false")

    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round142_stage_date_rows()}
        fields = {row["field"] for row in round142_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND142_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r10_loop8_guardrails(self):
        summary = round142_summary()
        summary_md = render_round142_summary_markdown()
        guardrails = render_round142_green_guardrail_markdown()
        overlays = render_round142_risk_overlay_markdown()
        price_plan = render_round142_price_validation_plan_markdown()
        ...
        self.assertIn("Round 142", summary_md)
        self.assertIn("R10 v8 Base Score Axes", summary_md)
        self.assertIn("eps_fcf_affo_noi_conversion: 24", summary_md)
        self.assertIn("R10 v8 Stage Caps", summary_md)
        self.assertIn("stage1_headline_cap: 45", summary_md)
        self.assertIn("Do not apply R10 Loop-8 v8.0 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round142_r10_reports(
                ...
                cases_path=Path(tmp) / "cases_r10_loop8_round142.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round142_r10_loop8_v8.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND142_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round142_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round142_r10_loop8_construction_real_estate_materials", text)
