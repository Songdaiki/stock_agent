...
from e2r.cli.build_round137_r5_loop8_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round137_r5_loop8_consumer_retail_brand import (
...
class Round137R5Loop8ConsumerRetailBrandTests(unittest.TestCase):
    def test_round137_targets_cover_consumer_retail_brand_loop8(self):
        labels = {target.target_id for target in ROUND137_SCORE_TARGETS}
    ...
    def test_new_loop8_canonical_archetypes_exist(self):
        expected = (
    ...
    def test_green_possible_watch_and_redteam_targets_are_separated(self):
        ...
        for target_id in green_targets:
            target = round137_target_for(target_id)
    ...
    def test_required_round137_cases_are_present_with_stage_markers(self):
        rows = {row["case_id"]: row for row in round137_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND137_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_consumer_guardrails(self):
        ...
        self.assertEqual(len(records), len(ROUND137_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round137_weight_table(self):
        rows = {row["target_id"]: row for row in round137_score_profile_rows()}
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round137_stage_date_rows()}
        fields = {row["field"] for row in round137_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND137_PRICE_FIELDS))

    def test_loop8_base_weights_stage_caps_and_alignment_are_explicit(self):
        weights = {row["component"]: row for row in round137_base_score_weight_rows()}
        caps = {row["cap_id"]: row for row in round137_stage_cap_rows()}
        alignment = {row["case_id"]: row for row in round137_score_stage_price_alignment_rows()}
        alignment_md = render_round137_score_stage_price_alignment_markdown()
    ...
    def test_summary_and_markdown_explain_round137_guardrails(self):
        summary = round137_summary()
        summary_md = render_round137_summary_markdown()
        guardrails = render_round137_green_guardrail_markdown()
        overlays = render_round137_risk_overlay_markdown()
        price_plan = render_round137_price_validation_plan_markdown()
        ...
        self.assertIn("R5 Loop 8", summary_md)
        self.assertIn("Do not apply R5 Loop-8 v8.0 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round137_r5_loop8_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r5_loop8_round137.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round137_r5_loop8_v8.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND137_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round137_pack(self):
        ...
        for relative in (
            ...
            self.assertNotIn("round137_r5_loop8_consumer_retail_brand", text)
