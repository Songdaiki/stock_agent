...
from e2r.cli.build_round163_r5_loop10_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round163_r5_loop10_consumer_retail_brand import (
...
class Round163R5Loop10ConsumerRetailBrandTests(unittest.TestCase):
    def test_round163_targets_cover_consumer_retail_brand_loop10(self):
        labels = {target.target_id for target in ROUND163_SCORE_TARGETS}
    ...
    def test_green_possible_watch_and_redteam_targets_are_separated(self):
        ...
        for target_id in green_targets:
            target = round163_target_for(target_id)
    ...
    def test_required_round163_cases_are_present_with_stage_markers(self):
        rows = {row["case_id"]: row for row in round163_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND163_CASE_CANDIDATES))
        ...
        self.assertEqual(rows["ulta_kbeauty_retail_platform_case"]["target_id"], "K_BEAUTY_RETAIL_PLATFORM")
        self.assertEqual(rows["ulta_kbeauty_retail_platform_case"]["symbol"], "ULTA")
        self.assertEqual(rows["ulta_kbeauty_retail_platform_case"]["stage2_date"], "")
        self.assertEqual(rows["ulta_kbeauty_retail_platform_case"]["price_validation_status"], "needs_exact_stage_date_backfill")
    ...
    def test_case_records_validate_and_keep_consumer_guardrails(self):
        ...
        self.assertEqual(len(records), len(ROUND163_CASE_CANDIDATES))
        ...
        self.assertIn(
            E2RArchetype.K_BEAUTY_OFFLINE_SELL_THROUGH,
            by_id["ulta_kbeauty_retail_platform_case"].secondary_archetypes,
        )
    ...
    def test_score_profile_rows_match_round163_weight_table(self):
        rows = {row["target_id"]: row for row in round163_score_profile_rows()}
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round163_stage_date_rows()}
        fields = {row["field"] for row in round163_price_field_rows()}
        ...
        for field in (
            ...
            "store_level_sales",
            "store_level_margin",
            "ulta_net_sales_growth",
            "ulta_same_store_sales_growth",
            "after_hours_share_price_change",
            "retailer_exclusive_selection_flag",
            "price_increase_flag",
        ...
        self.assertEqual(len(fields), len(ROUND163_PRICE_FIELDS))

    def test_loop10_base_weights_stage_caps_and_alignment_are_explicit(self):
        weights = {row["component"]: row for row in round163_base_score_weight_rows()}
        caps = {row["cap_id"]: row for row in round163_stage_cap_rows()}
        alignment = {row["case_id"]: row for row in round163_score_stage_price_alignment_rows()}
        alignment_md = render_round163_score_stage_price_alignment_markdown()
        ...
        self.assertEqual(weights["market_mispricing_rerating_gap"]["weight"], "8")
        self.assertEqual(weights["valuation_room_4b_runway"]["weight"], "7")
        self.assertEqual(weights["safety_regulatory_trust_disclosure_confidence"]["weight"], "12")
        ...
        self.assertEqual(alignment["ulta_kbeauty_retail_platform_case"]["score_stage"], "Stage 2")
        self.assertIn("same-store", alignment["ulta_kbeauty_retail_platform_case"]["price_path_signal"])
    ...
    def test_summary_and_markdown_explain_round163_guardrails(self):
        summary = round163_summary()
        summary_md = render_round163_summary_markdown()
        guardrails = render_round163_green_guardrail_markdown()
        overlays = render_round163_risk_overlay_markdown()
        price_plan = render_round163_price_validation_plan_markdown()

        self.assertEqual(summary["target_count"], 30)
        self.assertEqual(summary["source_canonical_target_count"], 26)
        self.assertEqual(summary["helper_overlay_target_count"], 4)
        ...
        self.assertIn("R5 Loop 10", summary_md)
        self.assertIn("Do not apply R5 Loop-10 v10.0 weights", guardrails)
        self.assertIn("RETAIL_PLATFORM_STAGE2_NOT_STAGE3", overlays)
        self.assertIn("ULTA_RETAIL_CHANNEL_STAGE2", overlays)
        ...
        self.assertIn("ulta_kbeauty_retail_platform_case", price_plan)
        self.assertIn("ULTA_KBEAUTY_RETAIL_PLATFORM_STAGE2", price_plan)

    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round163_r5_loop10_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r5_loop10_round163.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round163_r5_loop10_v10.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND163_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round163_pack(self):
        ...
        for relative in (
            ...
            self.assertNotIn("round163_r5_loop10_consumer_retail_brand", text)
