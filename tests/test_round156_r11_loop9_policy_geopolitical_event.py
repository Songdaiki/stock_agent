...
from e2r.cli.build_round156_r11_loop9_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round156_r11_loop9_policy_geopolitical_event import (
...
class Round156R11Loop9PolicyGeopoliticalEventTests(unittest.TestCase):
    def test_round156_targets_cover_r11_loop9_archetypes(self):
        labels = {target.target_id for target in ROUND156_SCORE_TARGETS}
    ...
    def test_required_round156_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round156_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND156_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_round156_guardrails(self):
        records = round156_case_records()

        self.assertEqual(len(records), len(ROUND156_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round156_weight_table(self):
        rows = {row["target_id"]: row for row in round156_score_profile_rows()}
    ...
    def test_base_score_axes_match_round156_v9_weight_table(self):
        rows = {row["axis_id"]: row for row in round156_base_score_axis_rows()}

        self.assertEqual(len(rows), 7)
        self.assertEqual(len(ROUND156_BASE_SCORE_AXES), 7)
    ...
    def test_stage_caps_make_round156_event_boundaries_explicit(self):
        rows = {row["cap_id"]: row for row in round156_stage_cap_rows()}

        self.assertEqual(len(rows), 4)
        self.assertEqual(len(ROUND156_STAGE_CAPS), 4)
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round156_stage_date_rows()}
        fields = {row["field"] for row in round156_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND156_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r11_loop9_guardrails(self):
        summary = round156_summary()
        summary_md = render_round156_summary_markdown()
        guardrails = render_round156_green_guardrail_markdown()
        caps = render_round156_event_false_positive_caps_markdown()
        price_plan = render_round156_price_validation_plan_markdown()
        ...
        self.assertIn("Round-156", summary_md)
        self.assertIn("R11 v9 Base Score Axes", summary_md)
        self.assertIn("actual_contract_budget_order_financing_visibility: 28", summary_md)
        self.assertIn("R11 v9 Stage Caps", summary_md)
        self.assertIn("stage1_event_headline_cap: Stage 1 / 40", summary_md)
        self.assertIn("Do not apply these R11 Loop-9 v9 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round156_r11_loop9_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r11_loop9_round156.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round156_r11_loop9_v9.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND156_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round156_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round156_r11_loop9_policy_geopolitical_event", text)
