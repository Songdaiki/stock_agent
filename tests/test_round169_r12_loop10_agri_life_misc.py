...
from e2r.cli.build_round169_r12_loop10_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round169_r12_loop10_agri_life_misc import (
...
class Round169R12Loop10AgriLifeMiscTests(unittest.TestCase):
    def test_round169_targets_cover_r12_loop10_archetypes_and_overlays(self):
        labels = {target.target_id for target in ROUND169_SCORE_TARGETS}

        self.assertEqual(len(labels), 30)
        self.assertEqual(ROUND169_SOURCE_CANONICAL_TARGET_COUNT, 30)
        self.assertEqual(set(ROUND169_SOURCE_CANONICAL_TARGET_IDS), labels)
        for label in (
    ...
    def test_base_score_axes_match_round169_v10_weight_table(self):
        axes = {axis.axis_id: axis for axis in ROUND169_BASE_SCORE_AXES}
        self.assertEqual(
            {
        ...
        self.assertEqual(
            [axis.axis_id for axis in ROUND169_BASE_SCORE_AXES],
            [
        ...
        rows = round169_base_score_axis_rows()
        self.assertEqual(len(rows), 7)
        self.assertTrue(all(row["production_scoring_changed"] == "false" for row in rows))

    def test_new_round169_archetypes_exist_and_gates_are_explicit(self):
        for archetype in (
        ...
        sulfuric = target_for("FERTILIZER_INPUT_COST_SULFURIC_ACID")
    ...
    def test_score_profile_rows_match_round169_weight_table(self):
        rows = {row["target_id"]: row for row in round169_score_profile_rows()}
        ...
        self.assertEqual(rows["FERTILIZER_INPUT_COST_SULFURIC_ACID"]["gate_only"], "true")
        self.assertEqual(rows["FERTILIZER_INPUT_COST_SULFURIC_ACID"]["eps_fcf"], "gate")
    ...
    def test_stage_caps_make_round169_loop10_boundaries_explicit(self):
        rows = {row["cap_id"]: row for row in round169_stage_cap_rows()}

        self.assertEqual(len(rows), 4)
        self.assertEqual(len(ROUND169_STAGE_CAPS), 4)
    ...
    def test_required_round169_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round169_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND169_CASE_CANDIDATES))
        ...
        self.assertEqual(rows["mosaic_phosphate_sulfuric_acid_squeeze_case"]["target_id"], "FERTILIZER_INPUT_COST_SULFURIC_ACID")
        self.assertEqual(rows["mosaic_phosphate_sulfuric_acid_squeeze_case"]["stage4c_date"], "2026-05-06")
    ...
    def test_case_records_validate_and_keep_round169_guardrails(self):
        records = round169_case_records()

        self.assertEqual(len(records), len(ROUND169_CASE_CANDIDATES))
        ...
        self.assertEqual(by_id["mosaic_phosphate_sulfuric_acid_squeeze_case"].rerating_result, "thesis_break")

    def test_stage_date_and_price_field_plans_are_explicit(self):
        stage_rows = {row["target_id"]: row for row in round169_stage_date_rows()}
        price_fields = {row["field"] for row in round169_price_field_rows()}
        ...
        self.assertIn("guidance_withdrawal", stage_rows["FERTILIZER_INPUT_COST_SULFURIC_ACID"]["stage4c"])
        ...
        self.assertEqual(len(price_fields), len(ROUND169_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r12_loop10_guardrails(self):
        summary = round169_summary()
        summary_md = render_round169_summary_markdown()
        guardrails = render_round169_green_guardrail_markdown()
        caps = render_round169_unit_economics_cap_markdown()
        price_plan = render_round169_price_validation_plan_markdown()
        ...
        self.assertEqual(summary["redteam_first_count"], 13)
        self.assertEqual(summary["gate_only_target_count"], 9)
        self.assertFalse(summary["production_scoring_changed"])
        self.assertIn("Round-169", summary_md)
        self.assertIn("source_canonical_target_count: 30", summary_md)
        self.assertIn("R12 v10 Base Score Axes", summary_md)
        self.assertIn("`eps_fcf_opm_conversion`: 22", summary_md)
        self.assertIn("R12 v10 Stage Caps", summary_md)
        ...
        self.assertIn("Do not apply these R12 Loop-10 v10 weights", guardrails)
        ...
        self.assertIn("fertilizer_input_cost_4c", price_plan)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round169_r12_loop10_reports(
                ...
                cases_path=Path(tmp) / "cases_r12_loop10_round169.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round169_r12_loop10_v10.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND169_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round169_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round169_r12_loop10_agri_life_misc", text)
