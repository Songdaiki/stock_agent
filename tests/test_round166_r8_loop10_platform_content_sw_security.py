...
from e2r.cli.build_round166_r8_loop10_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round166_r8_loop10_platform_content_sw_security import (
    ROUND166_CASE_CANDIDATES,
    ROUND166_HELPER_OVERLAY_TARGET_COUNT,
    ROUND166_HELPER_OVERLAY_TARGET_IDS,
    ROUND166_PRICE_FIELDS,
    ROUND166_SCORE_TARGETS,
    ROUND166_SOURCE_CANONICAL_TARGET_COUNT,
    ROUND166_SOURCE_CANONICAL_TARGET_IDS,
    render_round166_green_guardrail_markdown,
    render_round166_price_validation_plan_markdown,
    render_round166_risk_overlay_markdown,
    render_round166_summary_markdown,
    round166_base_score_axis_rows,
    round166_case_candidate_rows,
    round166_case_records,
    round166_price_field_rows,
    round166_score_profile_rows,
    round166_stage_date_rows,
    round166_summary,
    round166_target_for,
    write_round166_r8_loop10_reports,
)


class Round166R8Loop10PlatformContentSwSecurityTests(unittest.TestCase):
    def test_round166_targets_cover_r8_loop10_archetypes(self):
        labels = {target.target_id for target in ROUND166_SCORE_TARGETS}

        self.assertEqual(len(labels), 28)
        self.assertEqual(ROUND166_SOURCE_CANONICAL_TARGET_COUNT, 23)
        self.assertEqual(ROUND166_HELPER_OVERLAY_TARGET_COUNT, 5)
        self.assertTrue(set(ROUND166_SOURCE_CANONICAL_TARGET_IDS).issubset(labels))
        self.assertEqual(
            set(ROUND166_HELPER_OVERLAY_TARGET_IDS),
            {
    ...
    def test_green_possible_targets_are_repeat_revenue_guardrailed(self):
        b2b = round166_target_for("B2B_SAAS_ERP_WORKFLOW")
        enterprise = round166_target_for("ENTERPRISE_AI_ONTOLOGY_WORKFLOW")
        government = round166_target_for("GOVERNMENT_AI_PROGRAM_OF_RECORD")
        cloud = round166_target_for("CLOUD_AI_SOFTWARE_INFRA")
        edge = round166_target_for("EDGE_AI_CLOUD_INFRASTRUCTURE")
        legacy_reinforcement = round166_target_for("LEGACY_SAAS_AI_REINFORCEMENT")
        cyber = round166_target_for("CYBERSECURITY_AI_THREAT_DEMAND")
        ai_networking = round166_target_for("AI_NETWORKING_SOFTWARE_INFRA_CROSSOVER")
        ...
        self.assertIn("ai_cost", cloud.loop10_penalty_axes)
        ...
        self.assertIn("capex", edge.loop10_penalty_axes)
    ...
    def test_ai_game_and_ugc_themes_are_not_green_by_default(self):
        ai = round166_target_for("AI_SOFTWARE_APPLICATION")
        game = round166_target_for("GAME_CONTENT_IP")
        single_ip = round166_target_for("SINGLE_IP_RELEASE_EVENT_PREMIUM")
        ugc = round166_target_for("UGC_GAME_PLATFORM_SAFETY")
        nft = round166_target_for("METAVERSE_NFT_THEME")
    ...
    def test_overlay_targets_are_gate_only(self):
        for target_id in (
            ...
            target = round166_target_for(target_id)
            assert target is not None
        ...
        trust = round166_target_for("SECURITY_OPERATIONAL_TRUST_OVERLAY")
        ad_trust = round166_target_for("PLATFORM_AD_TRUST_OVERLAY")
        privacy = round166_target_for("PLATFORM_PRIVACY_YOUTH_SAFETY_OVERLAY")
        observability_risk = round166_target_for("OBSERVABILITY_GUIDANCE_RISK")
        legacy = round166_target_for("LEGACY_SAAS_AI_DISRUPTION_OVERLAY")
    ...
    def test_disclosure_confidence_cap_is_cap_only(self):
        cap = round166_target_for("DISCLOSURE_CONFIDENCE_CAP")
    ...
    def test_required_round166_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round166_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND166_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_round166_guardrails(self):
        records = round166_case_records()

        self.assertEqual(len(records), len(ROUND166_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round166_weight_table(self):
        rows = {row["target_id"]: row for row in round166_score_profile_rows()}
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round166_stage_date_rows()}
        fields = {row["field"] for row in round166_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND166_PRICE_FIELDS))

    def test_base_score_axes_match_round166_weight_table(self):
        rows = {row["axis_id"]: row for row in round166_base_score_axis_rows()}
        ordered_axis_ids = [row["axis_id"] for row in round166_base_score_axis_rows()]
    ...
    def test_summary_and_markdown_explain_r8_loop10_guardrails(self):
        summary = round166_summary()
        summary_md = render_round166_summary_markdown()
        guardrails = render_round166_green_guardrail_markdown()
        overlays = render_round166_risk_overlay_markdown()
        price_plan = render_round166_price_validation_plan_markdown()

        self.assertEqual(summary["target_count"], 28)
        self.assertEqual(summary["source_canonical_target_count"], 23)
        ...
        self.assertIn("Round 166", summary_md)
        self.assertIn("source_canonical_target_count: 23", summary_md)
        self.assertIn("helper_overlay_target_count: 5", summary_md)
        self.assertIn("Do not apply R8 Loop-10 v10.0 weights", guardrails)
        ...
        self.assertIn("R8 v10 Base Score Axes", summary_md)

    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round166_r8_loop10_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r8_loop10_round166.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round166_r8_loop10_v10.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND166_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round166_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round166_r8_loop10_platform_content_sw_security", text)
