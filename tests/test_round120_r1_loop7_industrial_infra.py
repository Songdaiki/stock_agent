...
from e2r.cli.build_round120_r1_loop7_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round120_r1_loop7_industrial_infra import (
...
class Round120R1Loop7IndustrialInfraTests(unittest.TestCase):
    def test_round120_targets_cover_r1_loop7_archetypes_and_overlays(self):
        labels = {target.target_id for target in ROUND120_SCORE_TARGETS}
    ...
    def test_new_r1_loop7_canonical_archetypes_exist(self):
        expected = (
    ...
    def test_loop7_weights_and_penalties_are_stricter_than_loop2(self):
        transformer = round120_target_for("GRID_TRANSFORMER_SHORTAGE")
        ehv = round120_target_for("GRID_EHV_TRANSFORMER_EXPORT")
        medium_voltage = round120_target_for("GRID_MEDIUM_VOLTAGE_EXPANSION")
        slot_prebuy = round120_target_for("GRID_SUPPLY_SLOT_PREBUY")
        turbine = round120_target_for("GAS_TURBINE_POWER_BACKLOG")
        capital_return = round120_target_for("POWER_EQUIPMENT_CAPITAL_RETURN")
        grid_flex = round120_target_for("DATA_CENTER_GRID_FLEXIBILITY_OVERLAY")
        defense = round120_target_for("DEFENSE_GOVERNMENT_BACKLOG")
        defense_local = round120_target_for("DEFENSE_LOCAL_PRODUCTION_PLATFORM")
        defense_shock = round120_target_for("DEFENSE_CAPITAL_ALLOCATION_SHOCK")
        defense_us_shipbuilding = round120_target_for("DEFENSE_US_SHIPBUILDING_PLATFORM")
        procurement = round120_target_for("SHIPBUILDING_PROCUREMENT_LEADTIME")
        smr = round120_target_for("NUCLEAR_SMR_GRID_POLICY")
        ppa = round120_target_for("NUCLEAR_EXISTING_PPA_RESTART")
        nuclear_grid = round120_target_for("NUCLEAR_GRID_INJECTION_RIGHTS")
        permitting = round120_target_for("DATA_CENTER_POWER_WATER_PERMITTING")
        dilution = round120_target_for("CAPITAL_ALLOCATION_DILUTION_OVERLAY")
        disclosure_cap = round120_target_for("DISCLOSURE_CONFIDENCE_CAP")
        ...
        self.assertIn("low_margin_long_term_contract", transformer.loop7_penalty_axes)
        ...
        self.assertIn("capital_allocation_shock", defense.loop7_penalty_axes)
    ...
    def test_loop7_base_score_weights_and_stage_caps_are_explicit(self):
        weights = {row["component"]: row for row in round120_base_score_weight_rows()}
        caps = {row["stage_band"]: row for row in round120_stage_cap_rows()}

        self.assertEqual(len(ROUND120_BASE_SCORE_WEIGHTS), 7)
        self.assertEqual(weights["eps_fcf_revision"]["points"], "25")
        self.assertEqual(weights["contract_backlog_visibility"]["points"], "24")
        self.assertEqual(weights["bottleneck_pricing_power"]["points"], "20")
        self.assertEqual(weights["valuation_room_4b_runway"]["points"], "8")
        self.assertEqual(weights["capital_discipline_fcf_conversion"]["points"], "6")
        self.assertEqual(len(ROUND120_STAGE_CAPS), 5)
        self.assertEqual(caps["Stage 1"]["max_score"], "45")
        self.assertIn("mou", caps["Stage 1"]["required_evidence"])
    ...
    def test_score_stage_price_alignment_table_covers_round120_core_examples(self):
        rows = {row["case_id"]: row for row in round120_score_stage_price_alignment_rows()}

        self.assertEqual(len(ROUND120_SCORE_STAGE_PRICE_ALIGNMENT), 6)
    ...
    def test_required_round120_cases_are_present_with_dates_and_alignment(self):
        rows = {row["case_id"]: row for row in round120_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND120_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_loop7_guardrails(self):
        records = round120_case_records()

        self.assertEqual(len(records), len(ROUND120_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_mark_no_production_change_and_include_loop7_penalties(self):
        rows = round120_score_profile_rows()

        self.assertEqual(len(rows), len(ROUND120_SCORE_TARGETS))
        for row in rows:
            self.assertEqual(row["large_sector"], "INDUSTRIAL_ORDERS_INFRA")
            self.assertEqual(row["production_scoring_changed"], "false")
            self.assertIn("loop7_penalty_axes", row)
        by_target = {row["target_id"]: row for row in rows}
        self.assertEqual(by_target["GRID_TRANSFORMER_SHORTAGE"]["eps_fcf"], "24")
        self.assertIn("low_margin_long_term_contract", by_target["GRID_TRANSFORMER_SHORTAGE"]["loop7_penalty_axes"])
        self.assertEqual(by_target["GRID_EHV_TRANSFORMER_EXPORT"]["information_confidence"], "6")
        self.assertIn("customer_project_delay", by_target["GRID_EHV_TRANSFORMER_EXPORT"]["loop7_penalty_axes"])
        self.assertIn("slot_cancelled", by_target["GRID_SUPPLY_SLOT_PREBUY"]["loop7_penalty_axes"])
        self.assertIn("capa_normalization", by_target["GRID_MEDIUM_VOLTAGE_EXPANSION"]["loop7_penalty_axes"])
        self.assertIn("wind_segment_loss", by_target["GAS_TURBINE_POWER_BACKLOG"]["loop7_penalty_axes"])
        ...
        self.assertIn("moratorium", by_target["DATA_CENTER_POWER_WATER_PERMITTING"]["loop7_penalty_axes"])
        self.assertEqual(by_target["DEFENSE_CAPITAL_ALLOCATION_SHOCK"]["hard_gate"], "true")
        self.assertIn("moa_only", by_target["DEFENSE_US_SHIPBUILDING_PLATFORM"]["loop7_penalty_axes"])
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round120_stage_date_rows()}
        fields = {row["field"] for row in round120_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND120_PRICE_FIELDS))

    def test_summary_and_markdown_explain_loop7_guardrails(self):
        summary = round120_summary()
        summary_md = render_round120_summary_markdown()
        guardrails = render_round120_green_guardrail_markdown()
        overlays = render_round120_loop7_risk_overlay_markdown()
        price_plan = render_round120_price_validation_plan_markdown()
        alignment = render_round120_score_stage_price_alignment_markdown()
        ...
        self.assertIn("R1 Loop 7", summary_md)
        self.assertIn("Do not apply R1 Loop-7 v7.0 weights", guardrails)
        ...
        self.assertIn("EPS/FCF 25", summary_md)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round120_r1_loop7_reports(
                ...
                cases_path=Path(tmp) / "cases_r1_loop7_round120.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round120_r1_loop7_v7.csv",
            )
            ...
            self.assertTrue(paths["loop7_risk_overlays"].exists())
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND120_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round120_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round120_r1_loop7_industrial_infra", text)
