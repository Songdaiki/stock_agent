...
from e2r.cli.build_round170_r11_loop10_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round170_r11_loop10_policy_geopolitical_event import (
...
class Round170R11Loop10PolicyGeopoliticalEventTests(unittest.TestCase):
    def test_round170_targets_cover_r11_loop10_archetypes(self):
        labels = {target.target_id for target in ROUND170_SCORE_TARGETS}

        self.assertEqual(len(labels), 24)
        self.assertEqual(ROUND170_SOURCE_CANONICAL_TARGET_COUNT, 24)
        self.assertEqual(set(ROUND170_SOURCE_CANONICAL_TARGET_IDS), labels)
        for label in (
    ...
    def test_required_round170_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round170_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND170_CASE_CANDIDATES))
        ...
        self.assertEqual(rows["china_rare_earth_partial_easing_4c_watch_case"]["target_id"], "STRATEGIC_SUPPLY_CHAIN_EXPORT_CONTROL_EVENT")
        self.assertEqual(rows["china_rare_earth_partial_easing_4c_watch_case"]["stage4c_date"], "2026-05-15")
    ...
    def test_case_records_validate_and_keep_round170_guardrails(self):
        records = round170_case_records()

        self.assertEqual(len(records), len(ROUND170_CASE_CANDIDATES))
        ...
        self.assertEqual(by_id["china_rare_earth_partial_easing_4c_watch_case"].score_price_alignment, "price_moved_without_evidence")
        self.assertEqual(by_id["china_rare_earth_partial_easing_4c_watch_case"].rerating_result, "theme_overheat")
    ...
    def test_score_profile_rows_match_round170_weight_table(self):
        rows = {row["target_id"]: row for row in round170_score_profile_rows()}
    ...
    def test_base_score_axes_match_round170_v10_weight_table(self):
        rows = {row["axis_id"]: row for row in round170_base_score_axis_rows()}

        self.assertEqual(len(rows), 7)
        self.assertEqual(len(ROUND170_BASE_SCORE_AXES), 7)
    ...
    def test_stage_caps_make_round170_event_boundaries_explicit(self):
        rows = {row["cap_id"]: row for row in round170_stage_cap_rows()}

        self.assertEqual(len(rows), 4)
        self.assertEqual(len(ROUND170_STAGE_CAPS), 4)
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round170_stage_date_rows()}
        fields = {row["field"] for row in round170_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND170_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r11_loop10_guardrails(self):
        summary = round170_summary()
        summary_md = render_round170_summary_markdown()
        guardrails = render_round170_green_guardrail_markdown()
        caps = render_round170_event_false_positive_caps_markdown()
        price_plan = render_round170_price_validation_plan_markdown()

        self.assertEqual(summary["target_count"], 24)
        self.assertEqual(summary["source_canonical_target_count"], 24)
        ...
        self.assertIn("Round-170", summary_md)
        self.assertIn("source_canonical_target_count: 24", summary_md)
        self.assertIn("R11 v10 Base Score Axes", summary_md)
        self.assertIn("actual_contract_budget_order_financing_visibility: 28", summary_md)
        self.assertIn("R11 v10 Stage Caps", summary_md)
        self.assertIn("stage1_event_headline_cap: Stage 1 / 40", summary_md)
        self.assertIn("Do not apply these R11 Loop-10 v10 weights", guardrails)
        ...
        self.assertIn("EXPORT_CONTROL_EASING_4C_WATCH", caps)
        ...
        self.assertIn("china_rare_earth_partial_easing_4c_watch_case", price_plan)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round170_r11_loop10_reports(
                ...
                cases_path=Path(tmp) / "cases_r11_loop10_round170.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round170_r11_loop10_v10.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND170_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round170_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round170_r11_loop10_policy_geopolitical_event", text)
