...
from e2r.cli.build_round154_r9_loop9_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round154_r9_loop9_mobility_transport_leisure import (
    ROUND154_CASE_CANDIDATES,
    ROUND154_PRICE_FIELDS,
    ROUND154_SCORE_TARGETS,
    render_round154_green_guardrail_markdown,
    render_round154_price_validation_plan_markdown,
    render_round154_risk_overlay_markdown,
    render_round154_summary_markdown,
    round154_base_score_axis_rows,
    round154_case_candidate_rows,
    round154_case_records,
    round154_price_field_rows,
    round154_score_profile_rows,
    round154_stage_date_rows,
    round154_summary,
    target_for,
    write_round154_r9_loop9_reports,
)


class Round154R9Loop9MobilityTransportLeisureTests(unittest.TestCase):
    def test_round154_targets_cover_r9_loop9_archetypes(self):
        labels = {target.target_id for target in ROUND154_SCORE_TARGETS}
    ...
    def test_required_round154_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round154_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND154_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_round154_guardrails(self):
        records = round154_case_records()

        self.assertEqual(len(records), len(ROUND154_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round154_weight_table(self):
        rows = {row["target_id"]: row for row in round154_score_profile_rows()}
    ...
    def test_base_score_axes_match_round154_v9_table(self):
        rows = {row["axis_id"]: row for row in round154_base_score_axis_rows()}
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round154_stage_date_rows()}
        fields = {row["field"] for row in round154_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND154_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r9_loop9_guardrails(self):
        summary = round154_summary()
        summary_md = render_round154_summary_markdown()
        guardrails = render_round154_green_guardrail_markdown()
        overlays = render_round154_risk_overlay_markdown()
        price_plan = render_round154_price_validation_plan_markdown()
        ...
        self.assertIn("Round 154", summary_md)
        self.assertIn("Do not apply R9 Loop-9 v9.0 weights", guardrails)
        self.assertIn("R9 v9 Base Score Axes", summary_md)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round154_r9_loop9_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r9_loop9_round154.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round154_r9_loop9_v9.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND154_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round154_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round154_r9_loop9_mobility_transport_leisure", text)
