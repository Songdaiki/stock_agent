...
from e2r.cli.build_round101_r9_loop5_report import build_parser
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round101_r9_loop5_mobility_transport_leisure import (
    ROUND101_CASE_CANDIDATES,
    ROUND101_PRICE_FIELDS,
    ROUND101_SCORE_TARGETS,
    render_round101_green_guardrail_markdown,
    render_round101_price_validation_plan_markdown,
    render_round101_risk_overlay_markdown,
    render_round101_summary_markdown,
    round101_case_candidate_rows,
    round101_case_records,
    round101_price_field_rows,
    round101_score_profile_rows,
    round101_stage_date_rows,
    round101_summary,
    target_for,
    write_round101_r9_loop5_reports,
)


class Round101R9Loop5MobilityTransportLeisureTests(unittest.TestCase):
    def test_round101_targets_cover_r9_loop5_archetypes(self):
        labels = {target.target_id for target in ROUND101_SCORE_TARGETS}
    ...
    def test_required_round101_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round101_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND101_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_round101_guardrails(self):
        records = round101_case_records()

        self.assertEqual(len(records), len(ROUND101_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round101_weight_table(self):
        rows = {row["target_id"]: row for row in round101_score_profile_rows()}
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round101_stage_date_rows()}
        fields = {row["field"] for row in round101_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND101_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r9_loop5_guardrails(self):
        summary = round101_summary()
        summary_md = render_round101_summary_markdown()
        guardrails = render_round101_green_guardrail_markdown()
        overlays = render_round101_risk_overlay_markdown()
        price_plan = render_round101_price_validation_plan_markdown()
        ...
        self.assertEqual(summary["redteam_first_count"], 11)
        self.assertEqual(summary["gate_only_target_count"], 7)
        self.assertFalse(summary["production_scoring_changed"])
        self.assertIn("Round 101", summary_md)
        self.assertIn("Do not apply R9 Loop-5 v5.0 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round101_r9_loop5_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r9_loop5_round101.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round101_r9_loop5_v5.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND101_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round101_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round101_r9_loop5_mobility_transport_leisure", text)
