...
from e2r.cli.build_round139_r7_loop8_report import build_parser
...
class Round139R7Loop8BiotechHealthcareDeviceTests(unittest.TestCase):
    def test_round139_targets_cover_r7_loop8_archetypes(self):
        labels = {target.target_id for target in ROUND139_SCORE_TARGETS}
    ...
    def test_cdmo_glp1_and_devices_are_green_possible_but_guardrailed(self):
        cdmo = round139_target_for("CDMO_HEALTHCARE_CONTRACT")
        glp1 = round139_target_for("OBESITY_GLP1_COMMERCIALIZATION")
        device = round139_target_for("MEDICAL_DEVICE_HEALTHCARE_EXPORT")
        implant = round139_target_for("MEDICAL_DEVICE_DENTAL_IMPLANT")
        robot = round139_target_for("SURGICAL_ROBOT_INSTALLED_BASE")
    ...
    def test_pre_revenue_ai_telehealth_and_diagnostics_are_redteam_first(self):
        for target_id in (
            ...
            target = round139_target_for(target_id)
            assert target is not None
            self.assertEqual(target.posture, Round10ThemePosture.REDTEAM_FIRST)

        gene = round139_target_for("GENE_THERAPY_RARE_DISEASE")
        ai_drug = round139_target_for("AI_DRUG_DISCOVERY_PLATFORM")
        diagnostics = round139_target_for("DIAGNOSTICS_INFECTIOUS_DISEASE")
        telehealth = round139_target_for("TELEHEALTH_BEHAVIORAL_HEALTH")
        glp1_channel = round139_target_for("GLP1_TELEHEALTH_CHANNEL")
        ai_subgroup = round139_target_for("MEDICAL_AI_SUBGROUP_GENERALIZATION_RISK")
    ...
    def test_redteam_overlays_are_gate_only(self):
        ...
        compounded = round139_target_for("COMPOUNDED_GLP1_REGULATORY_RISK")
        price_war = round139_target_for("GLP1_PRICE_WAR_OVERLAY")
        patent = round139_target_for("BIOSIMILAR_PATENT_LITIGATION")
        device = round139_target_for("DEVICE_SAFETY_COUNTERFEIT_OVERLAY")
        cro_funding = round139_target_for("CRO_FUNDING_CYCLE_OVERLAY")
        robot_mix = round139_target_for("SURGICAL_ROBOT_GLP1_PROCEDURE_MIX_OVERLAY")
    ...
    def test_disclosure_confidence_cap_is_cap_only_not_gate_only(self):
        cap = round139_target_for("DISCLOSURE_CONFIDENCE_CAP")
    ...
    def test_required_round139_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round139_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND139_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_round139_guardrails(self):
        records = round139_case_records()

        self.assertEqual(len(records), len(ROUND139_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round139_weight_table(self):
        rows = {row["target_id"]: row for row in round139_score_profile_rows()}
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round139_stage_date_rows()}
        fields = {row["field"] for row in round139_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND139_PRICE_FIELDS))

    def test_base_weights_stage_caps_and_alignment_are_reported(self):
        base_rows = {row["component"]: row for row in round139_base_score_weight_rows()}
        cap_rows = {row["cap_id"]: row for row in round139_stage_cap_rows()}
        alignment_rows = {row["case_id"]: row for row in round139_score_stage_price_alignment_rows()}
        alignment_md = render_round139_score_stage_price_alignment_markdown()
    ...
    def test_summary_and_markdown_explain_r7_loop8_guardrails(self):
        summary = round139_summary()
        summary_md = render_round139_summary_markdown()
        guardrails = render_round139_green_guardrail_markdown()
        overlays = render_round139_risk_overlay_markdown()
        price_plan = render_round139_price_validation_plan_markdown()
        ...
        self.assertIn("R7 Loop 8", summary_md)
        self.assertIn("Do not apply R7 Loop-8 v8.0 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round139_r7_loop8_reports(
                ...
                cases_path=Path(tmp) / "cases_r7_loop8_round139.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round139_r7_loop8_v8.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND139_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round139_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round139_r7_loop8_biotech_healthcare_device", text)
