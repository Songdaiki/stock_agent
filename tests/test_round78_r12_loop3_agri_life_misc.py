...
from e2r.cli.build_round78_r12_loop3_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round78_r12_loop3_agri_life_misc import (
...
class Round78R12Loop3AgriLifeMiscTests(unittest.TestCase):
    def test_round78_targets_cover_r12_loop3_archetypes_and_overlays(self):
        labels = {target.target_id for target in ROUND78_SCORE_TARGETS}
    ...
    def test_new_round78_archetypes_exist_and_gates_are_explicit(self):
        for archetype in (
    ...
    def test_score_profile_rows_match_round78_weight_table(self):
        rows = {row["target_id"]: row for row in round78_score_profile_rows()}
    ...
    def test_required_round78_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round78_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND78_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_round78_guardrails(self):
        records = round78_case_records()

        self.assertEqual(len(records), len(ROUND78_CASE_CANDIDATES))
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        stage_rows = {row["target_id"]: row for row in round78_stage_date_rows()}
        price_fields = {row["field"] for row in round78_price_field_rows()}
        ...
        self.assertEqual(len(price_fields), len(ROUND78_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r12_loop3_guardrails(self):
        summary = round78_summary()
        summary_md = render_round78_summary_markdown()
        guardrails = render_round78_green_guardrail_markdown()
        caps = render_round78_unit_economics_cap_markdown()
        price_plan = render_round78_price_validation_plan_markdown()
        ...
        self.assertIn("Round-78", summary_md)
        self.assertIn("Do not apply these R12 Loop-3 v3 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round78_r12_loop3_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r12_loop3_round78.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round78_r12_loop3_v3.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND78_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round78_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round78_r12_loop3_agri_life_misc", text)
