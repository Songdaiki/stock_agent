...
from e2r.cli.build_round131_r12_loop7_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round131_r12_loop7_agri_life_misc import (
...
class Round131R12Loop7AgriLifeMiscTests(unittest.TestCase):
    def test_round131_targets_cover_r12_loop7_archetypes_and_overlays(self):
        labels = {target.target_id for target in ROUND131_SCORE_TARGETS}
    ...
    def test_base_score_axes_match_round131_v7_weight_table(self):
        axes = {axis.axis_id: axis for axis in ROUND131_BASE_SCORE_AXES}
        self.assertEqual(
            {
        ...
        rows = round131_base_score_axis_rows()
        self.assertEqual(len(rows), 7)
        self.assertTrue(all(row["production_scoring_changed"] == "false" for row in rows))

    def test_new_round131_archetypes_exist_and_gates_are_explicit(self):
        for archetype in (
    ...
    def test_score_profile_rows_match_round131_weight_table(self):
        rows = {row["target_id"]: row for row in round131_score_profile_rows()}
    ...
    def test_required_round131_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round131_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND131_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_round131_guardrails(self):
        records = round131_case_records()

        self.assertEqual(len(records), len(ROUND131_CASE_CANDIDATES))
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        stage_rows = {row["target_id"]: row for row in round131_stage_date_rows()}
        price_fields = {row["field"] for row in round131_price_field_rows()}
        ...
        self.assertEqual(len(price_fields), len(ROUND131_PRICE_FIELDS))

    def test_summary_and_markdown_explain_r12_loop7_guardrails(self):
        summary = round131_summary()
        summary_md = render_round131_summary_markdown()
        guardrails = render_round131_green_guardrail_markdown()
        caps = render_round131_unit_economics_cap_markdown()
        price_plan = render_round131_price_validation_plan_markdown()
        ...
        self.assertIn("Round-131", summary_md)
        self.assertIn("R12 v7 Base Score Axes", summary_md)
        self.assertIn("`eps_fcf_opm_conversion`: 22", summary_md)
        self.assertIn("Do not apply these R12 Loop-7 v7 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round131_r12_loop7_reports(
                ...
                cases_path=Path(tmp) / "cases_r12_loop7_round131.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round131_r12_loop7_v7.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND131_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round131_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round131_r12_loop7_agri_life_misc", text)
