...
from e2r.cli.build_round126_r7_loop7_report import build_parser
...
class Round126R7Loop7BiotechHealthcareDeviceTests(unittest.TestCase):
    def test_round126_targets_cover_r7_loop7_archetypes(self):
        labels = {target.target_id for target in ROUND126_SCORE_TARGETS}
    ...
    def test_cdmo_glp1_and_devices_are_green_possible_but_guardrailed(self):
        cdmo = round126_target_for("CDMO_HEALTHCARE_CONTRACT")
        glp1 = round126_target_for("OBESITY_GLP1_COMMERCIALIZATION")
        device = round126_target_for("MEDICAL_DEVICE_HEALTHCARE_EXPORT")
        implant = round126_target_for("MEDICAL_DEVICE_DENTAL_IMPLANT")
        robot = round126_target_for("SURGICAL_ROBOT_INSTALLED_BASE")
    ...
    def test_pre_revenue_ai_telehealth_and_diagnostics_are_redteam_first(self):
        for target_id in (
            ...
            target = round126_target_for(target_id)
            assert target is not None
            self.assertEqual(target.posture, Round10ThemePosture.REDTEAM_FIRST)

        gene = round126_target_for("GENE_THERAPY_RARE_DISEASE")
        ai_drug = round126_target_for("AI_DRUG_DISCOVERY_PLATFORM")
        diagnostics = round126_target_for("DIAGNOSTICS_INFECTIOUS_DISEASE")
        telehealth = round126_target_for("TELEHEALTH_BEHAVIORAL_HEALTH")
        glp1_channel = round126_target_for("GLP1_TELEHEALTH_CHANNEL")
        ai_subgroup = round126_target_for("MEDICAL_AI_SUBGROUP_GENERALIZATION_RISK")
    ...
    def test_redteam_overlays_are_gate_only(self):
        ...
        compounded = round126_target_for("COMPOUNDED_GLP1_REGULATORY_RISK")
        price_war = round126_target_for("GLP1_PRICE_WAR_OVERLAY")
        patent = round126_target_for("BIOSIMILAR_PATENT_LITIGATION")
        device = round126_target_for("DEVICE_SAFETY_COUNTERFEIT_OVERLAY")
        cro_funding = round126_target_for("CRO_FUNDING_CYCLE_OVERLAY")
        robot_mix = round126_target_for("SURGICAL_ROBOT_GLP1_PROCEDURE_MIX_OVERLAY")
    ...
    def test_disclosure_confidence_cap_is_cap_only_not_gate_only(self):
        cap = round126_target_for("DISCLOSURE_CONFIDENCE_CAP")
    ...
    def test_required_round126_cases_are_present_with_stage_dates(self):
        rows = {row["case_id"]: row for row in round126_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND126_CASE_CANDIDATES))
    ...
    def test_case_records_validate_and_keep_round126_guardrails(self):
        records = round126_case_records()

        self.assertEqual(len(records), len(ROUND126_CASE_CANDIDATES))
    ...
    def test_score_profile_rows_match_round126_weight_table(self):
        rows = {row["target_id"]: row for row in round126_score_profile_rows()}
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round126_stage_date_rows()}
        fields = {row["field"] for row in round126_price_field_rows()}
        ...
        self.assertEqual(len(fields), len(ROUND126_PRICE_FIELDS))

    def test_base_weights_stage_caps_and_alignment_are_reported(self):
        base_rows = {row["component"]: row for row in round126_base_score_weight_rows()}
        cap_rows = {row["cap_id"]: row for row in round126_stage_cap_rows()}
        alignment_rows = {row["case_id"]: row for row in round126_score_stage_price_alignment_rows()}
        alignment_md = render_round126_score_stage_price_alignment_markdown()
        ...
        self.assertEqual(base_rows["safety_regulatory_disclosure_confidence"]["weight"], "12")
    ...
    def test_summary_and_markdown_explain_r7_loop7_guardrails(self):
        summary = round126_summary()
        summary_md = render_round126_summary_markdown()
        guardrails = render_round126_green_guardrail_markdown()
        overlays = render_round126_risk_overlay_markdown()
        price_plan = render_round126_price_validation_plan_markdown()
        ...
        self.assertIn("R7 Loop 7", summary_md)
        self.assertIn("Do not apply R7 Loop-7 v7.0 weights", guardrails)
    ...
    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round126_r7_loop7_reports(
                ...
                cases_path=Path(tmp) / "cases_r7_loop7_round126.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round126_r7_loop7_v7.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND126_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round126_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round126_r7_loop7_biotech_healthcare_device", text)
