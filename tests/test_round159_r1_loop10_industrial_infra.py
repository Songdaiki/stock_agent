...
from e2r.cli.build_round159_r1_loop10_report import build_parser
from e2r.sector.archetypes import E2RArchetype
from e2r.sector.case_library import load_case_library
from e2r.sector.round10_theme_tag_taxonomy import Round10LargeSector, Round10ThemePosture
from e2r.sector.round159_r1_loop10_industrial_infra import (
...
class Round159R1Loop10IndustrialInfraTests(unittest.TestCase):
    def test_round159_targets_cover_r1_loop10_archetypes_and_overlays(self):
        labels = {target.target_id for target in ROUND159_SCORE_TARGETS}
    ...
    def test_new_r1_loop10_canonical_archetypes_exist(self):
        expected = (
    ...
    def test_loop10_weights_and_penalties_are_stricter_than_loop2(self):
        transformer = round159_target_for("GRID_TRANSFORMER_SHORTAGE")
        ehv = round159_target_for("GRID_EHV_TRANSFORMER_EXPORT")
        medium_voltage = round159_target_for("GRID_MEDIUM_VOLTAGE_EXPANSION")
        slot_prebuy = round159_target_for("GRID_SUPPLY_SLOT_PREBUY")
        turbine = round159_target_for("GAS_TURBINE_POWER_BACKLOG")
        capital_return = round159_target_for("POWER_EQUIPMENT_BACKLOG_TO_FCF")
        grid_flex = round159_target_for("DATA_CENTER_GRID_FLEXIBILITY_OVERLAY")
        defense = round159_target_for("DEFENSE_GOVERNMENT_BACKLOG")
        defense_local = round159_target_for("DEFENSE_LOCAL_PRODUCTION_PLATFORM")
        defense_shock = round159_target_for("DEFENSE_CAPITAL_ALLOCATION_SHOCK")
        defense_us_shipbuilding = round159_target_for("DEFENSE_US_SHIPBUILDING_PLATFORM")
        procurement = round159_target_for("SHIPBUILDING_PROCUREMENT_LEADTIME")
        smr = round159_target_for("NUCLEAR_SMR_GRID_POLICY")
        ppa = round159_target_for("NUCLEAR_EXISTING_PPA_RESTART")
        nuclear_grid = round159_target_for("NUCLEAR_GRID_INJECTION_RIGHTS")
        permitting = round159_target_for("DATA_CENTER_POWER_WATER_PERMITTING")
        dilution = round159_target_for("CAPITAL_ALLOCATION_DILUTION_OVERLAY")
        disclosure_cap = round159_target_for("DISCLOSURE_CONFIDENCE_CAP")
        ...
        self.assertIn("low_margin_long_term_contract", transformer.loop10_penalty_axes)
        ...
        self.assertIn("capital_allocation_shock", defense.loop10_penalty_axes)
    ...
    def test_loop10_base_score_weights_and_stage_caps_are_explicit(self):
        weights = {row["component"]: row for row in round159_base_score_weight_rows()}
        caps = {row["stage_band"]: row for row in round159_stage_cap_rows()}

        self.assertEqual(len(ROUND159_BASE_SCORE_WEIGHTS), 7)
        self.assertEqual(weights["eps_fcf_margin_conversion"]["points"], "25")
        self.assertEqual(weights["contract_backlog_customer_visibility"]["points"], "22")
        self.assertEqual(weights["bottleneck_pricing_power"]["points"], "18")
        self.assertEqual(weights["capital_discipline_dilution"]["points"], "10")
        self.assertEqual(weights["market_mispricing_rerating_gap"]["points"], "9")
        self.assertEqual(weights["valuation_room_4b_runway"]["points"], "7")
        self.assertEqual(weights["disclosure_confidence_redteam"]["points"], "9")
        self.assertEqual(len(ROUND159_STAGE_CAPS), 5)
        self.assertEqual(caps["Stage 1"]["max_score"], "45")
        self.assertIn("mou_or_moa", caps["Stage 1"]["required_evidence"])
        self.assertEqual(caps["Stage 2"]["max_score"], "70")
        self.assertIn("ppa_or_slot_reservation_with_gate_status", caps["Stage 2"]["required_evidence"])
        self.assertIn("op_eps_fcf_revision", caps["Stage 3"]["required_evidence"])
        self.assertEqual(caps["Stage 4C"]["max_score"], "hard_gate")
        self.assertIn("profit_miss", caps["Stage 4C"]["required_evidence"])

    def test_score_stage_price_alignment_table_covers_round159_core_examples(self):
        rows = {row["case_id"]: row for row in round159_score_stage_price_alignment_rows()}

        self.assertEqual(len(ROUND159_SCORE_STAGE_PRICE_ALIGNMENT), 10)
        self.assertEqual(rows["ge_vernova_data_center_orders_case"]["verdict"], "score_to_stage_to_price_aligned")
        self.assertIn("4B watch", rows["ge_vernova_data_center_orders_case"]["normalization_adjustment"])
        self.assertEqual(rows["siemens_energy_fcf_buyback_case"]["verdict"], "partially_confirmed")
        self.assertEqual(rows["us_transformer_shortage_import_slots_case"]["verdict"], "bottleneck_signal_confirmed")
        self.assertEqual(rows["ls_electric_525kv_us_datacenter_transformer_case"]["detected_stage"], "Stage 2")
        self.assertEqual(rows["hyundai_rotem_morocco_rail_case"]["verdict"], "stage2_not_green_yet")
        self.assertEqual(rows["constellation_tmi_microsoft_restart_case"]["verdict"], "stage2_with_approval_gate")
        self.assertEqual(rows["hanwha_aerospace_dilution_trim_case"]["verdict"], "redteam_alignment_confirmed")
        self.assertEqual(rows["hd_hyundai_huntington_us_navy_aux_case"]["verdict"], "green_block_correct")
        self.assertEqual(rows["siemens_orders_profit_miss_case"]["verdict"], "orders_only_false_positive_contained")
        self.assertEqual(rows["oklo_smr_no_revenue_watch_case"]["verdict"], "pre_revenue_smr_green_block_correct")

    def test_required_round159_cases_are_present_with_dates_and_alignment(self):
        rows = {row["case_id"]: row for row in round159_case_candidate_rows()}

        self.assertEqual(len(rows), len(ROUND159_CASE_CANDIDATES))
        ...
        self.assertEqual(rows["siemens_energy_fcf_buyback_case"]["target_id"], "POWER_EQUIPMENT_BACKLOG_TO_FCF")
        self.assertEqual(rows["siemens_energy_fcf_buyback_case"]["stage4b_date"], "2026-05-12")
        self.assertEqual(rows["siemens_energy_record_backlog_case"]["target_id"], "POWER_EQUIPMENT_BACKLOG_TO_FCF")
        self.assertEqual(rows["siemens_orders_profit_miss_case"]["target_id"], "POWER_EQUIPMENT_BACKLOG_TO_FCF")
        self.assertEqual(rows["siemens_orders_profit_miss_case"]["case_type"], "failed_rerating")
        self.assertIn("orders_only_without_profit", rows["siemens_orders_profit_miss_case"]["red_flag_fields"])
        ...
        self.assertEqual(rows["oklo_smr_no_revenue_watch_case"]["target_id"], "NUCLEAR_SMR_GRID_POLICY")
        self.assertEqual(rows["oklo_smr_no_revenue_watch_case"]["price_validation_status"], "needs_source_date_backfill")
        self.assertIn("pre_revenue_smr", rows["oklo_smr_no_revenue_watch_case"]["red_flag_fields"])
    ...
    def test_case_records_validate_and_keep_loop10_guardrails(self):
        ...
        self.assertEqual(len(records), len(ROUND159_CASE_CANDIDATES))
        ...
        self.assertEqual(by_id["siemens_orders_profit_miss_case"].score_price_alignment, "false_positive_score")
        self.assertEqual(by_id["oklo_smr_no_revenue_watch_case"].rerating_result, "no_rerating")
    ...
    def test_score_profile_rows_mark_no_production_change_and_include_loop10_penalties(self):
        rows = round159_score_profile_rows()

        self.assertEqual(len(rows), len(ROUND159_SCORE_TARGETS))
        for row in rows:
            self.assertEqual(row["large_sector"], "INDUSTRIAL_ORDERS_INFRA")
            self.assertEqual(row["production_scoring_changed"], "false")
            self.assertIn("loop10_penalty_axes", row)
        by_target = {row["target_id"]: row for row in rows}
        self.assertEqual(by_target["GRID_TRANSFORMER_SHORTAGE"]["eps_fcf"], "24")
        self.assertIn("low_margin_long_term_contract", by_target["GRID_TRANSFORMER_SHORTAGE"]["loop10_penalty_axes"])
        self.assertEqual(by_target["GRID_EHV_TRANSFORMER_EXPORT"]["information_confidence"], "6")
        self.assertIn("customer_project_delay", by_target["GRID_EHV_TRANSFORMER_EXPORT"]["loop10_penalty_axes"])
        self.assertIn("slot_cancelled", by_target["GRID_SUPPLY_SLOT_PREBUY"]["loop10_penalty_axes"])
        self.assertIn("capa_normalization", by_target["GRID_MEDIUM_VOLTAGE_EXPANSION"]["loop10_penalty_axes"])
        self.assertIn("wind_segment_loss", by_target["GAS_TURBINE_POWER_BACKLOG"]["loop10_penalty_axes"])
        self.assertIn("buyback_cut", by_target["POWER_EQUIPMENT_BACKLOG_TO_FCF"]["stage4c_conditions"])
        self.assertEqual(by_target["DATA_CENTER_GRID_FLEXIBILITY_OVERLAY"]["eps_fcf"], "gate")
        self.assertIn("moratorium", by_target["DATA_CENTER_POWER_WATER_PERMITTING"]["loop10_penalty_axes"])
        self.assertEqual(by_target["DEFENSE_CAPITAL_ALLOCATION_SHOCK"]["hard_gate"], "true")
        self.assertIn("moa_only", by_target["DEFENSE_US_SHIPBUILDING_PLATFORM"]["loop10_penalty_axes"])
    ...
    def test_stage_date_and_price_field_plans_are_explicit(self):
        rows = {row["target_id"]: row for row in round159_stage_date_rows()}
        fields = {row["field"] for row in round159_price_field_rows()}
        ...
        self.assertIn("orders_to_fcf", rows["POWER_EQUIPMENT_BACKLOG_TO_FCF"]["stage2"])
        ...
        self.assertEqual(len(fields), len(ROUND159_PRICE_FIELDS))

    def test_summary_and_markdown_explain_loop10_guardrails(self):
        summary = round159_summary()
        summary_md = render_round159_summary_markdown()
        guardrails = render_round159_green_guardrail_markdown()
        overlays = render_round159_loop10_risk_overlay_markdown()
        price_plan = render_round159_price_validation_plan_markdown()
        alignment = render_round159_score_stage_price_alignment_markdown()
        ...
        self.assertIn("R1 Loop 10", summary_md)
        self.assertIn("Do not apply R1 Loop-10 v10.0 weights", guardrails)
        ...
        self.assertIn("EPS/FCF 25, visibility 22, bottleneck/pricing 18, capital discipline 10, mispricing 9", summary_md)
        self.assertIn("Stage 2 contracts are not Stage 3", summary_md)
        self.assertIn("Stage Caps", alignment)
        self.assertIn("GE Vernova", alignment)
        self.assertIn("Hanwha Aerospace", alignment)

    def test_report_writer_outputs_cases_and_reports(self):
        with tempfile.TemporaryDirectory() as tmp:
            paths = write_round159_r1_loop10_reports(
                output_directory=Path(tmp) / "out",
                cases_path=Path(tmp) / "cases_r1_loop10_round159.jsonl",
                score_profile_path=Path(tmp) / "score_weight_profiles_round159_r1_loop10_v10.csv",
            )
            ...
            self.assertEqual(len(load_case_library(paths["cases"])), len(ROUND159_CASE_CANDIDATES))
    ...
    def test_production_scoring_modules_do_not_import_round159_pack(self):
        for path in (
            "src/e2r/features.py",
            "src/e2r/staging.py",
            "src/e2r/red_team.py",
            ...
            self.assertNotIn("round159_r1_loop10_industrial_infra", text)
