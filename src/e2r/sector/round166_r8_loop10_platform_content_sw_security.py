"""Round-166 R8 Loop-10 platform/content/software/security pack.

Round 166 tightens the R8 platform/content/software/security pack. It separates
...
ROUND166_SOURCE_ROUND_PATH = "docs/round/round_166.md"
ROUND166_DEFAULT_OUTPUT_DIRECTORY = "output/e2r_round166_r8_loop10_platform_content_sw_security"
ROUND166_DEFAULT_CASES_PATH = "data/e2r_case_library/cases_r8_loop10_round166.jsonl"
ROUND166_DEFAULT_SCORE_PROFILE_PATH = "data/sector_taxonomy/score_weight_profiles_round166_r8_loop10_v10.csv"

ROUND166_SOURCE_CANONICAL_TARGET_IDS: tuple[str, ...] = (
...
ROUND166_HELPER_OVERLAY_TARGET_IDS: tuple[str, ...] = (
...
ROUND166_SOURCE_CANONICAL_TARGET_COUNT = len(ROUND166_SOURCE_CANONICAL_TARGET_IDS)
ROUND166_HELPER_OVERLAY_TARGET_COUNT = len(ROUND166_HELPER_OVERLAY_TARGET_IDS)


@dataclass(frozen=True)
class Round166ScoreWeightDraft:
    eps_fcf: int | str
...
@dataclass(frozen=True)
class Round166ScoreTarget:
    target_id: str
    canonical_archetype: E2RArchetype
    posture: Round10ThemePosture
    score_weight: Round166ScoreWeightDraft
...
GATE_WEIGHT = Round166ScoreWeightDraft("gate", "gate", "gate", "gate", "gate", "gate", "gate")
GATE_INFO5_WEIGHT = Round166ScoreWeightDraft("gate", "gate", "gate", "gate", "gate", "gate", 5)
CAP_WEIGHT = Round166ScoreWeightDraft("cap", "cap", "cap", "cap", "cap", "cap", "+")


ROUND166_BASE_SCORE_AXES: tuple[Round166BaseScoreAxis, ...] = (
    Round166BaseScoreAxis(
        "arr_billings_bookings_ad_revenue_contract_value",
        24,
        ("arr_growth", "billings_growth", "bookings_growth", "ad_revenue_growth", "contract_value", "guidance_raise"),
        ("repeat_revenue_growth", "renewal_strength", "net_retention_rate", "ad_arpu", "contract_renewal"),
        ("arr_miss", "guidance_miss", "bookings_cut", "revenue_miss"),
        "AI feature, user count, or ad tier launch remains Stage 1 until repeat revenue or contract value is verified.",
    ),
    Round166BaseScoreAxis(
        "recurrence_retention_workflow_lock_in",
        20,
        ("low_churn", "customer_retention", "workflow_integration", "paid_product_adoption"),
        ("workflow_lock_in", "renewal", "seat_churn_low", "customer_expansion"),
        ("seat_churn", "license_downsell", "workflow_lockin_failure", "age_verification_friction"),
        "Green needs repeated usage and workflow lock-in, not one product launch or one viral title.",
    ),
    Round166BaseScoreAxis(
        "operational_trust_legal_privacy_disclosure",
        14,
        ("disclosure_confidence_score", "privacy_risk_low", "ad_quality_clean", "operational_trust_intact"),
        ("legal_stability", "security_incident_absent", "customer_terms_verified", "ad_quality_controls_verified"),
        ("security_outage", "privacy_lawsuit", "youth_safety_lawsuit", "scam_ad_lawsuit", "copyright_lawsuit", "disclosure_confidence_low"),
        "Operational trust and legal/data quality are hard Loop 10 gates; CrowdStrike, Roblox, Meta, and Netflix-style events can cap or break Stage 3.",
    ),
    Round166BaseScoreAxis(
        "ai_cloud_security_platform_bottleneck",
        14,
        ("ai_workload_customer_count", "hyperscaler_ai_orders", "sase_firewall_demand", "frontier_model_customer_flag"),
        ("capacity_constraint", "software_security_mix", "ai_workload_attach", "security_platform_lock_in"),
        ("customer_concentration", "capex_burden", "security_demand_bubble", "order_to_revenue_delay"),
        "AI/cloud/security demand gets credit only when it creates billable bottleneck or platform lock-in.",
    ),
    Round166BaseScoreAxis(
        "opm_fcf_gross_margin_conversion",
        12,
        ("opm_improvement", "gross_margin_stable", "fcf_margin", "margin_confirmed"),
        ("fcf_conversion", "rule_of_40", "compute_cost_controlled", "gross_margin_stable"),
        ("gross_margin_pressure", "agent_cost_overrun", "margin_miss", "restructuring_cost"),
        "AI revenue is not enough if compute cost, capex, or restructuring absorbs the economics.",
    ),
    Round166BaseScoreAxis(
        "market_mispricing_rerating_gap",
        8,
        ("legacy_frame_shift", "old_saas_frame", "legacy_cdn_reframe", "ad_platform_reframe"),
        ("valuation_frame_shift", "platform_monetization", "new_repeat_revenue_frame"),
        ("narrative_fully_priced", "price_failed_after_beat", "premium_multiple_guidance_miss"),
        "Rerating room exists only when the market still uses the old frame while repeat economics are improving; Loop 10 shifts weight from narrative rerating to evidence quality.",
    ),
    Round166BaseScoreAxis(
        "valuation_room_4b_margin",
        8,
        ("valuation_room", "multiple_not_saturated", "price_path_not_overextended"),
        ("price_path_alignment", "revision_support", "valuation_still_reasonable"),
        ("valuation_saturation", "strong_results_stock_stalls", "ai_software_narrative_crowded", "short_term_price_spike"),
        "High-quality AI/platform evidence can still be 4B-watch if valuation moved first.",
    ),
)


ROUND166_SCORE_TARGETS: tuple[Round166ScoreTarget, ...] = (
    Round166ScoreTarget(
        "PLATFORM_SOFTWARE_INTERNET",
        E2RArchetype.PLATFORM_SOFTWARE_INTERNET,
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round166ScoreWeightDraft(18, 18, 7, 15, 13, 1, 5),
    ...
    Round166ScoreTarget(
        ...
        Round166ScoreWeightDraft(20, 23, 7, 15, 13, 1, 5),
    ...
    Round166ScoreTarget(
        ...
        Round10ThemePosture.GREEN_POSSIBLE,
        Round166ScoreWeightDraft(23, 23, 10, 16, 9, 0, 6),
    ...
    Round166ScoreTarget(
        "GOVERNMENT_AI_PROGRAM_OF_RECORD",
        E2RArchetype.GOVERNMENT_AI_PROGRAM_OF_RECORD,
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round166ScoreWeightDraft(23, 24, 11, 15, 10, 0, 6),
        ("government_ai_pilot", "defense_ai_software", "public_sector_ai_contract", "maven_or_defense_ai_flag"),
        ("program_of_record", "government_program_budget", "official_procurement", "long_term_contract", "total_contract_value"),
        ("recurring_government_software_revenue", "commercial_expansion", "auditable_workflow", "fcf_conversion"),
        ("government_ai_contractor_premium_overheated", "program_of_record_fully_priced"),
        ("budget_cut", "contract_cancellation", "political_risk", "auditability_failure", "ethics_review"),
        ("program_of_record", "government_program_budget", "official_procurement", "auditable_workflow", "fcf_conversion"),
        ("political_risk", "budget_cycle", "government_customer_concentration", "auditability_failure"),
        ("government_budget", "program_of_record", "auditability", "customer_concentration"),
        "Government AI becomes higher-quality only when pilot work turns into a funded program of record.",
    ),
    Round166ScoreTarget(
        "CLOUD_AI_SOFTWARE_INFRA",
        E2RArchetype.CLOUD_AI_SOFTWARE_INFRA,
        Round10ThemePosture.GREEN_POSSIBLE,
        Round166ScoreWeightDraft(22, 23, 9, 16, 10, 0, 5),
    ...
    Round166ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round166ScoreWeightDraft(20, 21, 12, 14, 9, 2, 5),
    ...
    Round166ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round166ScoreWeightDraft(20, 19, 9, 15, 12, 0, 5),
    ...
    Round166ScoreTarget(
        "LEGACY_SAAS_AI_REINFORCEMENT",
        E2RArchetype.LEGACY_SAAS_AI_REINFORCEMENT,
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round166ScoreWeightDraft(20, 21, 8, 14, 11, 0, 6),
        ...
        ("ai_arr", "ai_attach_rate", "seat_churn_low", "gross_margin_stable", "workflow_lock_in"),
        ("ai_arr_too_small", "seat_churn", "license_downsell", "gross_margin_pressure", "agent_cost_overrun"),
        ("ai_arr", "attach_rate", "seat_churn", "gross_margin", "workflow_lock_in"),
        "Legacy SaaS AI reinforcement is Watch-to-Green only when AI attach grows without seat churn or margin damage.",
    ),
    Round166ScoreTarget(
        "LEGACY_SAAS_AI_DISRUPTION_OVERLAY",
    ...
    Round166ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round166ScoreWeightDraft(21, 22, 8, 15, 10, 0, 5),
    ...
    Round166ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round166ScoreWeightDraft(19, 20, 8, 14, 13, 0, 5),
    ...
    Round166ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round166ScoreWeightDraft(18, 16, 6, 13, 10, 0, 5),
    ...
    Round166ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round166ScoreWeightDraft(10, 8, 4, 10, 6, 0, 5),
    ...
    Round166ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round166ScoreWeightDraft(17, 14, 5, 12, 8, 0, 5),
    ...
    Round166ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round166ScoreWeightDraft(16, 14, 6, 12, 9, 0, 5),
    ...
    Round166ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round166ScoreWeightDraft(18, 18, 7, 14, 8, 0, 5),
    ...
    Round166ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round166ScoreWeightDraft(20, 21, 8, 14, 11, 0, 5),
    ...
    Round166ScoreTarget(
        "CYBERSECURITY_AI_THREAT_DEMAND",
        E2RArchetype.CYBERSECURITY_AI_THREAT_DEMAND,
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round166ScoreWeightDraft(21, 21, 10, 14, 11, 0, 5),
        ("ai_ransomware", "ai_driven_cyber_threat", "security_budget_increase", "sase_firewall_demand"),
        ("billings_growth", "arr_growth", "guidance_raise", "renewal_rate", "opm_improvement"),
        ("security_platform_lock_in", "renewal_strength", "operational_trust_intact", "fcf_conversion"),
        ("ai_security_narrative_crowded", "threat_demand_fully_priced"),
        ("global_outage", "customer_lawsuit", "renewal_rate_down", "trust_damage", "security_demand_bubble"),
        ("billings_growth", "arr_growth", "renewal_strength", "operational_trust_intact", "fcf_conversion"),
        ("situational_demand_bubble", "renewal_risk", "outage", "trust_damage"),
        ("billings", "arr", "renewal", "operational_trust"),
        "AI threat demand is useful only when it becomes billings, ARR, renewal strength, and FCF.",
    ),
    Round166ScoreTarget(
        "AI_NETWORKING_SOFTWARE_INFRA_CROSSOVER",
        E2RArchetype.AI_NETWORKING_SOFTWARE_INFRA_CROSSOVER,
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round166ScoreWeightDraft(21, 21, 13, 13, 9, 2, 5),
        ("ai_networking_orders", "hyperscaler_ai_orders", "data_center_switching_orders", "silicon_optics_security_reframe"),
        ("ai_infrastructure_orders", "data_center_switching_orders", "guidance_raise", "software_security_mix", "order_to_revenue_conversion"),
        ("revenue_conversion", "opm_conversion", "fcf_conversion", "software_security_mix", "legacy_mix_stable"),
        ("ai_networking_order_premium_crowded", "orders_priced_before_margin"),
        ("restructuring_cost", "workforce_reduction", "legacy_networking_mix", "margin_conversion_unverified", "order_to_revenue_delay"),
        ("ai_infrastructure_orders", "guidance_raise", "opm_conversion", "fcf_conversion", "software_security_mix"),
        ("restructuring_cost", "legacy_networking_mix", "margin_conversion_unverified", "customer_concentration", "order_to_revenue_delay"),
        ("orders", "software_security_mix", "margin", "legacy_mix", "restructuring"),
        "AI networking crossover is Watch-to-Green only when orders convert into software/security mix, margin, and FCF.",
    ),
    Round166ScoreTarget(
        "SECURITY_IDENTITY_DEEPFAKE",
        E2RArchetype.SECURITY_IDENTITY_DEEPFAKE,
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round166ScoreWeightDraft(20, 20, 10, 14, 11, 0, 5),
    ...
    Round166ScoreTarget(
        ...
        Round10ThemePosture.REDTEAM_FIRST,
        Round166ScoreWeightDraft("gate", "gate", "gate", "gate", "gate", "gate", 6),
    ...
    Round166ScoreTarget(
        ...
        Round10ThemePosture.REDTEAM_FIRST,
        Round166ScoreWeightDraft(5, 5, 5, 6, 5, 0, 3),
...
ROUND166_CASE_CANDIDATES: tuple[Round166CaseCandidate, ...] = (
    Round166CaseCandidate(
        ...
        ("round_166.md Reuters EQT Douzone Bizon",),
    ...
    Round166CaseCandidate(
        ...
        None,
        None,
        ("record_revenue", "total_contract_value", "government_revenue_growth", "commercial_revenue_growth", "ontology_workflow_flag", "program_of_record_flag", "auditability_flag", "rule_of_40"),
        ("valuation_saturation", "government_customer_risk", "political_risk"),
        "government_and_commercial_ai_contract_success",
        "needs_price_backfill",
        ("round_166.md MarketWatch Palantir Q4 2025",),
        "AI is treated as structural only because revenue, contract value, government adoption, and commercial workflow evidence are present.",
        (E2RArchetype.ENTERPRISE_AI_ONTOLOGY_WORKFLOW, E2RArchetype.CLOUD_AI_SOFTWARE_INFRA),
    ),
    Round166CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_166.md MarketWatch Palantir Q1 2026",),
    ...
    Round166CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_166.md Barrons Akamai frontier model AI cloud deal",),
    ...
    Round166CaseCandidate(
        "akamai_ai_cloud_revenue_contribution_case",
        "EDGE_AI_CLOUD_INFRASTRUCTURE",
        "AKAM",
        "Akamai AI cloud quarterly revenue contribution estimate",
        "US",
        "success_candidate",
        None,
        date(2026, 5, 13),
        ...
        date(2026, 5, 13),
        None,
        ("edge_cloud_contract_value", "cloud_infrastructure_services_revenue", "revenue_contribution", "frontier_model_customer_flag"),
        ("customer_disclosed_flag_missing", "cloud_capex_amount_unverified", "margin_unverified", "ai_customer_concentration"),
        "edge_ai_cloud_revenue_contribution_watch",
        "needs_price_backfill",
        ("round_166.md Barrons Akamai AI cloud revenue contribution",),
        "Estimated quarterly revenue contribution improves Stage 2 evidence, but margin, capex, customer concentration, and FCF still gate promotion.",
    ),
    Round166CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_166.md Barrons Datadog observability",),
    ...
    Round166CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_166.md Barrons Dynatrace ARR guidance risk",),
    ...
    Round166CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_166.md Barrons Fortinet billings",),
        "Security software can be Watch-to-Green when billings, ARR, renewal, and operational trust are visible together.",
        (E2RArchetype.SECURITY_IDENTITY_DEEPFAKE,),
    ),
    Round166CaseCandidate(
        "cisco_ai_infrastructure_orders_case",
        "AI_NETWORKING_SOFTWARE_INFRA_CROSSOVER",
        "CSCO",
        "Cisco AI infrastructure orders and restructuring watch",
        "US",
        "success_candidate",
        None,
        date(2026, 5, 13),
        ...
        None,
        None,
        ("ai_infrastructure_orders", "hyperscaler_ai_orders", "data_center_switching_orders", "guidance_raise", "software_security_mix"),
        ("restructuring_cost", "workforce_reduction", "legacy_networking_mix", "margin_conversion_unverified"),
        "ai_networking_software_infra_crossover_stage2_restructuring_watch",
        "needs_price_backfill",
        ("round_166.md Reuters Cisco AI infrastructure orders",),
        "AI networking orders support Stage 2 only when orders convert into software/security mix, revenue, margin, and FCF after restructuring costs.",
        (E2RArchetype.AI_NETWORKING_SWITCHING_INFRA, E2RArchetype.CLOUD_AI_SOFTWARE_INFRA),
    ),
    Round166CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_166.md The Verge Netflix ad ambitions",),
    ...
    Round166CaseCandidate(
        ...
        "adtech_premium_miss",
        "needs_price_backfill",
        ("round_166.md Barron's Trade Desk revenue miss",),
    ...
    Round166CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_166.md Reuters CrowdStrike shareholder lawsuit",),
    ...
    Round166CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_166.md Reuters Kakao founder legal case",),
    ...
    Round166CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_166.md Reuters Roblox safety forecast cut",),
    ...
    Round166CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_166.md GTA VI preorder rumor",),
    ...
    Round166CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_166.md Reuters WPP ad forecast cut",),
    ...
    Round166CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_166.md Reuters Netflix Texas privacy lawsuit",),
    ...
    Round166CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_166.md Reuters Meta scam ads lawsuit",),
    ...
    Round166CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_166.md Reuters Meta youth safety trial",),
    ...
    Round166CaseCandidate(
        ...
        ("round_166.md Reuters Breakingviews SaaSpocalypse",),
    ...
    Round166CaseCandidate(
        ...
        (E2RArchetype.AI_SOFTWARE_APPLICATION, E2RArchetype.LEGACY_SAAS_AI_DISRUPTION_OVERLAY),
    ),
)


ROUND166_PRICE_FIELDS: tuple[str, ...] = (
    "case_id",
    "symbol",
    "company_name",
    "primary_archetype",
...
ROUND166_RISK_OVERLAYS: tuple[str, ...] = (
...
def round166_target_for(target_id: str) -> Round166ScoreTarget | None:
    for target in ROUND166_SCORE_TARGETS:
        if target.target_id == target_id:
            return target
    return None


def round166_case_records() -> tuple[E2RCaseRecord, ...]:
    records: list[E2RCaseRecord] = []
    for candidate in ROUND166_CASE_CANDIDATES:
        target = round166_target_for(candidate.target_id)
        if target is None:
        ...
        record = E2RCaseRecord(
            ...
            evidence_summary=(
                f"Round166 R8 Loop-10 case for {candidate.target_id}; "
...
def round166_score_profile_rows() -> tuple[dict[str, str], ...]:
...
def round166_base_score_axis_rows() -> tuple[dict[str, str], ...]:
    return tuple(
        {
        ...
        for axis in ROUND166_BASE_SCORE_AXES
    )


def round166_case_candidate_rows() -> tuple[dict[str, str], ...]:
    rows: list[dict[str, str]] = []
    for candidate in ROUND166_CASE_CANDIDATES:
        target = round166_target_for(candidate.target_id)
...
def round166_stage_date_rows() -> tuple[dict[str, str], ...]:
    return tuple(
        {
            "target_id": target.target_id,
...
def round166_price_field_rows() -> tuple[dict[str, str], ...]:
    return tuple({"field": field, "required_for_round166_backfill": "true"} for field in ROUND166_PRICE_FIELDS)


def round166_summary() -> dict[str, int | bool]:
    records = round166_case_records()
    return {
        "target_count": len(ROUND166_SCORE_TARGETS),
        "source_canonical_target_count": ROUND166_SOURCE_CANONICAL_TARGET_COUNT,
        "helper_overlay_target_count": ROUND166_HELPER_OVERLAY_TARGET_COUNT,
        "base_score_axis_count": len(ROUND166_BASE_SCORE_AXES),
        ...
        "green_possible_count": sum(1 for target in ROUND166_SCORE_TARGETS if target.posture == Round10ThemePosture.GREEN_POSSIBLE),
        "watch_yellow_first_count": sum(1 for target in ROUND166_SCORE_TARGETS if target.posture == Round10ThemePosture.WATCH_YELLOW_FIRST),
        "redteam_first_count": sum(1 for target in ROUND166_SCORE_TARGETS if target.posture == Round10ThemePosture.REDTEAM_FIRST),
        "gate_only_target_count": sum(1 for target in ROUND166_SCORE_TARGETS if target.gate_only),
        "production_scoring_changed": False,
        "case_records_are_candidate_generation_input": False,
    }


def write_round166_r8_loop10_reports(
    *,
    output_directory: str | Path = ROUND166_DEFAULT_OUTPUT_DIRECTORY,
    cases_path: str | Path = ROUND166_DEFAULT_CASES_PATH,
    score_profile_path: str | Path = ROUND166_DEFAULT_SCORE_PROFILE_PATH,
) -> dict[str, Path]:
    ...
    paths = {
        ...
        "summary": output / "round166_r8_loop10_platform_content_sw_security_summary.md",
        "case_matrix": output / "round166_r8_loop10_case_matrix.csv",
        "stage_date_plan": output / "round166_r8_loop10_stage_date_plan.csv",
        "base_score_axes": output / "round166_r8_loop10_base_score_axes.csv",
        "green_guardrails": output / "round166_r8_loop10_green_guardrails.md",
        "risk_overlays": output / "round166_r8_loop10_risk_overlays.md",
        "price_validation_plan": output / "round166_r8_loop10_price_validation_plan.md",
        "price_fields": output / "round166_r8_loop10_price_fields.csv",
    }
    _write_case_jsonl(round166_case_records(), cases)
    _write_rows(round166_score_profile_rows(), score_profiles)
    _write_rows(round166_base_score_axis_rows(), paths["base_score_axes"])
    _write_rows(round166_case_candidate_rows(), paths["case_matrix"])
    _write_rows(round166_stage_date_rows(), paths["stage_date_plan"])
    _write_rows(round166_price_field_rows(), paths["price_fields"])
    paths["summary"].write_text(render_round166_summary_markdown(), encoding="utf-8")
    paths["green_guardrails"].write_text(render_round166_green_guardrail_markdown(), encoding="utf-8")
    paths["risk_overlays"].write_text(render_round166_risk_overlay_markdown(), encoding="utf-8")
    paths["price_validation_plan"].write_text(render_round166_price_validation_plan_markdown(), encoding="utf-8")
    return paths


def render_round166_summary_markdown() -> str:
    summary = round166_summary()
    lines = [
        "# Round 166 R8 Loop-10 Platform/Content/SW/Security Summary",
        "",
        "Round 166 is calibration material only. It does not change production scoring.",
        "The source round defines 23 canonical targets; 5 older helper overlays remain for continuity and RedTeam calibration.",
        "",
        "## Counts",
    ]
    ...
    lines.extend(
        [
            ...
            "Example: Netflix ad tier reach is Stage 2 evidence, but privacy/youth-safety litigation can cap promotion until ad ARPU, churn, and compliance are verified.",
            "",
            "## R8 v10 Base Score Axes",
            "",
        ]
    )
    for axis in ROUND166_BASE_SCORE_AXES:
...
def render_round166_green_guardrail_markdown() -> str:
    lines = [
        "# Round 166 R8 Loop-10 Green Guardrails",
        "",
        "- Do not apply R8 Loop-10 v10.0 weights to production scoring yet.",
...
def render_round166_risk_overlay_markdown() -> str:
    lines = ["# Round 166 R8 Loop-10 Risk Overlays", ""]
    for overlay in ROUND166_RISK_OVERLAYS:
...
def render_round166_price_validation_plan_markdown() -> str:
    lines = [
        "# Round 166 R8 Loop-10 Price Validation Plan",
        "",
        "For every case, backfill stage prices and forward MFE/MAE before applying score-weight changes.",
        "",
        "## Priority Cases",
    ]
    for row in round166_case_candidate_rows():
    ...
    lines.extend(
        [
            ...
            "",
            ", ".join(ROUND166_PRICE_FIELDS),
        ]
    )
    return "\n".join(lines) + "\n"


def _score_price_alignment(candidate: Round166CaseCandidate) -> str:
...
def _rerating_result(candidate: Round166CaseCandidate) -> str:
    if candidate.case_type == "structural_success":
...
def _score_weight_hint(target: Round166ScoreTarget) -> dict[str, float]:
    weights = target.score_weight.as_dict()
    return {
