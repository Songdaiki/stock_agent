"""Round-124 R5 Loop-7 consumer, retail, and brand calibration pack.

Round 124 tightens R5 by separating consumer buzz from EPS/FCF bodyweight
...
ROUND124_SOURCE_ROUND_PATH = "docs/round/round_124.md"
ROUND124_DEFAULT_OUTPUT_DIRECTORY = "output/e2r_round124_r5_loop7_consumer_retail_brand"
ROUND124_DEFAULT_CASES_PATH = "data/e2r_case_library/cases_r5_loop7_round124.jsonl"
ROUND124_DEFAULT_SCORE_PROFILE_PATH = "data/sector_taxonomy/score_weight_profiles_round124_r5_loop7_v7.csv"


@dataclass(frozen=True)
class Round124ScoreWeightDraft:
    eps_fcf: int | str
...
@dataclass(frozen=True)
class Round124ScoreTarget:
    target_id: str
    canonical_archetype: E2RArchetype
    posture: Round10ThemePosture
    score_weight: Round124ScoreWeightDraft
...
@dataclass(frozen=True)
class Round124ScoreStagePriceAlignment:
    case_id: str
    score_stage: str
...
) -> Round124ScoreWeightDraft:
    return Round124ScoreWeightDraft(eps_fcf, visibility, bottleneck, mispricing, valuation, capital, confidence)
...
def _target(
    ...
    posture: Round10ThemePosture,
    weight: Round124ScoreWeightDraft,
    *,
    stage1: tuple[str, ...],
    stage2: tuple[str, ...],
    stage3: tuple[str, ...],
...
ROUND124_BASE_SCORE_WEIGHTS: tuple[Round124BaseScoreWeight, ...] = (
    Round124BaseScoreWeight(
    ...
    Round124BaseScoreWeight(
        ...
        9,
        "Recall, tariff, import review, data breach, IP, product safety, and disclosure gaps are explicit gates.",
    ),
)


ROUND124_STAGE_CAPS: tuple[Round124StageCap, ...] = (
    Round124StageCap(
...
ROUND124_SCORE_STAGE_PRICE_ALIGNMENT: tuple[Round124ScoreStagePriceAlignment, ...] = (
    Round124ScoreStagePriceAlignment(
...
ROUND124_SCORE_TARGETS: tuple[Round124ScoreTarget, ...] = (
    _target(
...
ROUND124_CASE_CANDIDATES: tuple[Round124CaseCandidate, ...] = (
    Round124CaseCandidate(
        "samyang_buldak_export_rerating_case",
        "EXPORT_RECURRING_CONSUMER",
        "003230",
        "삼양식품 Buldak 수출 리레이팅",
        ...
        ("round_124.md MarketWatch Samyang Buldak target revision",),
    ...
    Round124CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_124.md AP Samyang Denmark recall",),
    ...
    Round124CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_124.md AP Samyang Denmark partial reversal",),
    ...
    Round124CaseCandidate(
        ...
        ("round_124.md K-food hero to portfolio expansion framework",),
    ...
    Round124CaseCandidate(
        ...
        ("round_124.md Reuters K-beauty US export",),
    ...
    Round124CaseCandidate(
        ...
        ("round_124.md Reuters Olive Young US retail platform",),
    ...
    Round124CaseCandidate(
        ...
        "needs_exact_stage_date_backfill",
        ("round_124.md AP K-beauty US tariff risk",),
    ...
    Round124CaseCandidate(
        ...
        ("round_124.md K-beauty offline sell-through framework",),
    ...
    Round124CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_124.md FT APR beauty device",),
    ...
    Round124CaseCandidate(
        ...
        "missing_direct_symbol_mapping",
        ("round_124.md Vogue Medicube omnichannel",),
    ...
    Round124CaseCandidate(
        ...
        "missing_direct_symbol_mapping",
        ("round_124.md Reuters K-beauty fast beauty OEM/ODM",),
    ...
    Round124CaseCandidate(
        ...
        "needs_exact_stage_date_backfill",
        ("round_124.md Barron's Coupang data breach",),
    ...
    Round124CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_124.md Reuters Coupang supplier regulation",),
    ...
    Round124CaseCandidate(
        ...
        "needs_source_date_and_price_backfill",
        ("round_124.md Coway rental model reference",),
    ...
    Round124CaseCandidate(
        ...
        "needs_exact_stage_date_backfill",
        ("round_124.md Barron's Whirlpool dividend suspension",),
    ...
    Round124CaseCandidate(
        ...
        ("round_124.md Reuters Shein Temu litigation",),
    ...
    Round124CaseCandidate(
        ...
        "needs_exact_stage_date_backfill",
        ("round_124.md FT France EU Shein Temu crackdown",),
...
ROUND124_PRICE_FIELDS: tuple[str, ...] = (
...
def round124_target_for(target_id: str) -> Round124ScoreTarget | None:
...
def round124_case_records() -> tuple[E2RCaseRecord, ...]:
    records: list[E2RCaseRecord] = []
    for candidate in ROUND124_CASE_CANDIDATES:
        target = round124_target_for(candidate.target_id)
        if target is None:
        ...
        record = E2RCaseRecord(
            ...
            evidence_summary=(
                f"Round124 R5 Loop-7 case for {candidate.target_id}; "
...
def round124_score_profile_rows() -> tuple[dict[str, str], ...]:
...
def round124_case_candidate_rows() -> tuple[dict[str, str], ...]:
    rows: list[dict[str, str]] = []
    for candidate in ROUND124_CASE_CANDIDATES:
        target = round124_target_for(candidate.target_id)
...
def round124_stage_date_rows() -> tuple[dict[str, str], ...]:
    return tuple(
        {
            "target_id": target.target_id,
...
def round124_price_field_rows() -> tuple[dict[str, str], ...]:
    return tuple({"field": field, "required_for_round124_backfill": "true"} for field in ROUND124_PRICE_FIELDS)


def round124_base_score_weight_rows() -> tuple[dict[str, str], ...]:
    return tuple(
        {
        ...
        for row in ROUND124_BASE_SCORE_WEIGHTS
    )


def round124_stage_cap_rows() -> tuple[dict[str, str], ...]:
    return tuple(
        {
            "cap_id": row.cap_id,
        ...
        for row in ROUND124_STAGE_CAPS
    )


def round124_score_stage_price_alignment_rows() -> tuple[dict[str, str], ...]:
    return tuple(
        {
            "case_id": row.case_id,
        ...
        for row in ROUND124_SCORE_STAGE_PRICE_ALIGNMENT
    )


def round124_summary() -> dict[str, int | bool]:
    records = round124_case_records()
    return {
        "target_count": len(ROUND124_SCORE_TARGETS),
        "case_candidate_count": len(records),
        "base_score_component_count": len(ROUND124_BASE_SCORE_WEIGHTS),
        "stage_cap_count": len(ROUND124_STAGE_CAPS),
        "score_stage_price_alignment_count": len(ROUND124_SCORE_STAGE_PRICE_ALIGNMENT),
        ...
        "green_possible_count": sum(1 for target in ROUND124_SCORE_TARGETS if target.posture == Round10ThemePosture.GREEN_POSSIBLE),
        "watch_yellow_first_count": sum(1 for target in ROUND124_SCORE_TARGETS if target.posture == Round10ThemePosture.WATCH_YELLOW_FIRST),
        "redteam_first_count": sum(1 for target in ROUND124_SCORE_TARGETS if target.posture == Round10ThemePosture.REDTEAM_FIRST),
        "gate_only_target_count": sum(1 for target in ROUND124_SCORE_TARGETS if target.gate_only),
        "production_scoring_changed": False,
        "case_records_are_candidate_generation_input": False,
    }


def write_round124_r5_loop7_reports(
    *,
    output_directory: str | Path = ROUND124_DEFAULT_OUTPUT_DIRECTORY,
    cases_path: str | Path = ROUND124_DEFAULT_CASES_PATH,
    score_profile_path: str | Path = ROUND124_DEFAULT_SCORE_PROFILE_PATH,
) -> dict[str, Path]:
    ...
    paths = {
        ...
        "summary": output / "round124_r5_loop7_consumer_retail_brand_summary.md",
        "case_matrix": output / "round124_r5_loop7_case_matrix.csv",
        "stage_date_plan": output / "round124_r5_loop7_stage_date_plan.csv",
        "green_guardrails": output / "round124_r5_loop7_green_guardrails.md",
        "risk_overlays": output / "round124_r5_loop7_risk_overlays.md",
        "price_validation_plan": output / "round124_r5_loop7_price_validation_plan.md",
        "price_fields": output / "round124_r5_loop7_price_fields.csv",
        "base_score_weights": output / "round124_r5_loop7_base_score_weights.csv",
        "stage_caps": output / "round124_r5_loop7_stage_caps.csv",
        "score_stage_price_alignment": output / "round124_r5_loop7_score_stage_price_alignment.csv",
        "score_stage_price_alignment_md": output / "round124_r5_loop7_score_stage_price_alignment.md",
    }
    _write_case_jsonl(round124_case_records(), cases)
    _write_rows(round124_score_profile_rows(), score_profiles)
    _write_rows(round124_case_candidate_rows(), paths["case_matrix"])
    _write_rows(round124_stage_date_rows(), paths["stage_date_plan"])
    _write_rows(round124_price_field_rows(), paths["price_fields"])
    _write_rows(round124_base_score_weight_rows(), paths["base_score_weights"])
    _write_rows(round124_stage_cap_rows(), paths["stage_caps"])
    _write_rows(round124_score_stage_price_alignment_rows(), paths["score_stage_price_alignment"])
    paths["summary"].write_text(render_round124_summary_markdown(), encoding="utf-8")
    paths["green_guardrails"].write_text(render_round124_green_guardrail_markdown(), encoding="utf-8")
    paths["risk_overlays"].write_text(render_round124_risk_overlay_markdown(), encoding="utf-8")
    paths["price_validation_plan"].write_text(render_round124_price_validation_plan_markdown(), encoding="utf-8")
...
def render_round124_summary_markdown() -> str:
    summary = round124_summary()
    lines = [
        "# Round-124 R5 Loop-7 Consumer / Retail / Brand Summary",
        "",
        f"- source_round: `{ROUND124_SOURCE_ROUND_PATH}`",
        ...
        "## Interpretation",
        "",
        "- R5 Loop 7 says brand buzz and recurring FCF are different things.",
...
def render_round124_green_guardrail_markdown() -> str:
    lines = [
    ...
    for target in ROUND124_SCORE_TARGETS:
        lines.append(
            ...
            f"{', '.join(target.green_conditions) or 'not_applicable'} | {', '.join(target.loop7_penalty_axes)} |"
        )
    lines.extend(
        [
            "",
...
def render_round124_risk_overlay_markdown() -> str:
    lines = [
        "# Round-124 R5 Loop-7 Risk Overlays",
        "",
...
def render_round124_price_validation_plan_markdown() -> str:
    lines = [
        "# Round-124 R5 Loop-7 Price Validation Plan",
        "",
        "## Method",
        "",
    ...
    for row in round124_case_candidate_rows():
...
def render_round124_score_stage_price_alignment_markdown() -> str:
    lines = [
        "# Round-124 R5 Loop-7 Score / Stage / Price Alignment",
        "",
        "Round 124 checks whether consumer, retail, and brand score interpretation matches the observed price and operating path.",
...
def _score_price_alignment(candidate: Round124CaseCandidate) -> str:
...
def _rerating_result(candidate: Round124CaseCandidate) -> str:
...
def _score_weight_hint(target: Round124ScoreTarget) -> dict[str, float]:
    weights = target.score_weight.as_dict()
    return {
...
__all__ = [
    ...
    "ROUND124_BASE_SCORE_WEIGHTS",
    "ROUND124_SCORE_TARGETS",
    "ROUND124_SCORE_STAGE_PRICE_ALIGNMENT",
    "ROUND124_STAGE_CAPS",
    "Round124BaseScoreWeight",
    "Round124CaseCandidate",
    "Round124ScoreStagePriceAlignment",
