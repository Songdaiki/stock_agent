"""Round-159 R1 Loop-10 industrial orders and infrastructure pack.

Round 159 returns to R1 after the R13 Loop-9 RedTeam pass. It tightens the
industrial/order/infrastructure sector around the exact Loop-10 question:
orders and infrastructure headlines matter only when contract quality,
backlog, margin, EPS/FCF conversion, capital discipline, approval gates,
and price-path validation line up. The 15 source-round target archetypes are
kept alongside prior helper risk overlays because items like grid injection
rights, data-center permitting, or dilution are not independent sectors, but
they are still required gates.
...
ROUND159_SOURCE_ROUND_PATH = "docs/round/round_159.md"
ROUND159_DEFAULT_OUTPUT_DIRECTORY = "output/e2r_round159_r1_loop10_industrial_infra"
ROUND159_DEFAULT_CASES_PATH = "data/e2r_case_library/cases_r1_loop10_round159.jsonl"
ROUND159_DEFAULT_SCORE_PROFILE_PATH = "data/sector_taxonomy/score_weight_profiles_round159_r1_loop10_v10.csv"


@dataclass(frozen=True)
class Round159ScoreWeightDraft:
    eps_fcf: int | str
...
@dataclass(frozen=True)
class Round159ScoreTarget:
    target_id: str
    canonical_archetype: E2RArchetype
    posture: Round10ThemePosture
    score_weight: Round159ScoreWeightDraft
...
@dataclass(frozen=True)
class Round159ScoreStagePriceAlignment:
    case_id: str
    detected_stage: str
...
ROUND159_BASE_SCORE_WEIGHTS: tuple[Round159BaseScoreWeight, ...] = (
    Round159BaseScoreWeight("eps_fcf_margin_conversion", 25, "up", "Stage 3 promotion depends on order/backlog becoming OP/EPS/FCF and margin conversion."),
    Round159BaseScoreWeight("contract_backlog_customer_visibility", 22, "hold_but_detail_required", "Contract amount, period, customer, delivery, backlog, PPA, and slot reservation define Stage 2."),
    Round159BaseScoreWeight("bottleneck_pricing_power", 18, "up_with_price_path_check", "Transformer lead time, turbine slot, grid bottleneck, and defense capacity matter when they support pricing and margin."),
    Round159BaseScoreWeight("capital_discipline_dilution", 10, "up", "FCF-funded buyback/dividend adds quality; dilution, FSS correction, and unclear CAPEX become hard RedTeam."),
    Round159BaseScoreWeight("market_mispricing_rerating_gap", 9, "cap_after_consensus", "Old industrial frame must still be wrong, but crowded AI power or defense consensus lowers room."),
    Round159BaseScoreWeight("valuation_room_4b_runway", 7, "cap_after_rerating", "GE Vernova-style rerating needs explicit 4B watch after the market recognizes the power-equipment theme."),
    Round159BaseScoreWeight("disclosure_confidence_redteam", 9, "up", "OpenDART list-only, MoU-only, PPA without approval, profit miss, no-revenue SMR, or missing detail caps Stage 3."),
)


ROUND159_STAGE_CAPS: tuple[Round159StageCap, ...] = (
    Round159StageCap(
        "Stage 1",
        "45",
...
ROUND159_SCORE_STAGE_PRICE_ALIGNMENT: tuple[Round159ScoreStagePriceAlignment, ...] = (
    Round159ScoreStagePriceAlignment(
...
ROUND159_SCORE_TARGETS: tuple[Round159ScoreTarget, ...] = (
    Round159ScoreTarget(
        "GRID_TRANSFORMER_SHORTAGE",
        E2RArchetype.GRID_TRANSFORMER_SHORTAGE,
        Round10ThemePosture.GREEN_POSSIBLE,
        Round159ScoreWeightDraft(24, 25, 24, 12, 10, 1, 5),
        ...
        "Loop 10 strengthens transformer shortage, but Green still requires contract amount, duration, delivery, margin, and EPS conversion.",
    ),
    Round159ScoreTarget(
        "GRID_EHV_TRANSFORMER_EXPORT",
        E2RArchetype.GRID_EHV_TRANSFORMER_EXPORT,
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round159ScoreWeightDraft(24, 25, 24, 13, 10, 1, 6),
    ...
    Round159ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round159ScoreWeightDraft(22, 24, 22, 12, 10, 1, 5),
    ...
    Round159ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round159ScoreWeightDraft(22, 23, 20, 12, 10, 1, 5),
    ...
    Round159ScoreTarget(
        ...
        Round10ThemePosture.GREEN_POSSIBLE,
        Round159ScoreWeightDraft(22, 23, 19, 13, 10, 0, 5),
    ...
    Round159ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round159ScoreWeightDraft(21, 22, 18, 12, 10, 0, 5),
    ...
    Round159ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round159ScoreWeightDraft(22, 22, 18, 12, 9, 4, 5),
    ...
    Round159ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round159ScoreWeightDraft("gate", "gate", "gate", "gate", "gate", "gate", "gate"),
    ...
    Round159ScoreTarget(
        ...
        Round10ThemePosture.GREEN_POSSIBLE,
        Round159ScoreWeightDraft(20, 24, 18, 13, 12, 1, 5),
    ...
    Round159ScoreTarget(
        ...
        Round10ThemePosture.GREEN_POSSIBLE,
        Round159ScoreWeightDraft(21, 24, 17, 14, 14, 3, 5),
    ...
    Round159ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round159ScoreWeightDraft(21, 23, 16, 14, 13, 2, 5),
    ...
    Round159ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round159ScoreWeightDraft(16, 16, 12, 13, 9, 1, 5),
    ...
    Round159ScoreTarget(
        ...
        Round10ThemePosture.GREEN_POSSIBLE,
        Round159ScoreWeightDraft(21, 22, 18, 13, 13, 1, 5),
    ...
    Round159ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round159ScoreWeightDraft(16, 17, 11, 13, 10, 1, 5),
    ...
    Round159ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round159ScoreWeightDraft(20, 22, 12, 14, 11, 1, 5),
    ...
    Round159ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round159ScoreWeightDraft(19, 23, 13, 14, 12, 2, 5),
    ...
    Round159ScoreTarget(
        ...
        Round10ThemePosture.REDTEAM_FIRST,
        Round159ScoreWeightDraft(13, 12, 10, 13, 7, 1, 5),
    ...
    Round159ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round159ScoreWeightDraft(10, 8, 8, 10, 7, 0, 4),
    ...
    Round159ScoreTarget(
        ...
        Round10ThemePosture.REDTEAM_FIRST,
        Round159ScoreWeightDraft("cap", "cap", "cap", "cap", "cap", "cap", "+"),
...
ROUND159_CASE_CANDIDATES: tuple[Round159CaseCandidate, ...] = (
    Round159CaseCandidate(
        "us_transformer_shortage_import_slots_case",
        "GRID_TRANSFORMER_SHORTAGE",
        ...
        ("round_159.md Reuters US transformer shortage",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md LS Electric 525kV US datacenter transformer case placeholder",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md Reuters ABB medium-voltage equipment investment",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md Reuters Siemens Energy cash flow and buyback",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md Siemens Energy record backlog context",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md Reuters Siemens orders strong but profit miss",),
        "Loop 10 separates strong orders from E2R quality: if sales/profit miss and margin conversion fail, backlog cannot create Green.",
        (E2RArchetype.DISCLOSURE_CONFIDENCE_CAP,),
    ),
    Round159CaseCandidate(
        "ge_vernova_data_center_orders_case",
        ...
        "needs_price_backfill",
        ("round_159.md WSJ GE Vernova orders/backlog",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md Reuters GE Vernova power backlog / turbine context",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md Reuters EIA US power demand record path",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md arXiv AI data-center flexibility",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md Guardian Perth data-center withdrawal",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md data-center moratorium reference",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md Seattle data-center moratorium watch",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md arXiv data-center public water systems",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md Reuters Romania K9 contract",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md Reuters Hanwha Europe land arms sales",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md Reuters Hanwha Aerospace trims capital increase plan",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md Reuters HD Hyundai Heavy / Huntington Ingalls MoA",),
    ...
    Round159CaseCandidate(
        ...
        ("round_159.md AP Hanwha Ocean China sanctions",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md arXiv shipyard procurement lead-time prediction",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md Reuters Hyundai Rotem Morocco rail order",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md Reuters Meta Constellation nuclear PPA",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md Reuters Three Mile Island restart decision",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md Reuters Three Mile Island restart grid rights gate",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md NuScale UAMPS CFPP cancellation",),
    ...
    Round159CaseCandidate(
        ...
        "needs_source_date_backfill",
        ("round_159.md Oklo pre-revenue SMR watch case",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md reconstruction policy rule",),
    ...
    Round159CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_159.md Neom policy/infrastructure rule",),
...
ROUND159_PRICE_FIELDS: tuple[str, ...] = (
    "case_id",
    "symbol",
    "company_name",
    "primary_archetype",
...
def round159_target_for(target_id: str) -> Round159ScoreTarget | None:
    for target in ROUND159_SCORE_TARGETS:
        if target.target_id == target_id:
            return target
    return None


def round159_case_records() -> tuple[E2RCaseRecord, ...]:
    records: list[E2RCaseRecord] = []
    for candidate in ROUND159_CASE_CANDIDATES:
        target = round159_target_for(candidate.target_id)
        if target is None:
        ...
        record = E2RCaseRecord(
            ...
            evidence_summary=(
                f"Round159 R1 Loop-10 case for {candidate.target_id}; "
            ...
            score_price_alignment=_round159_score_price_alignment(candidate),
            rerating_result=_round159_rerating_result(candidate),
            ...
            green_guardrails=(
                ...
                "project_delay_capital_allocation_shock_low_margin_mro_smr_false_green_are_loop10_penalties",
                *target.red_flags,
            ),
...
def round159_score_profile_rows() -> tuple[dict[str, str], ...]:
...
def round159_case_candidate_rows() -> tuple[dict[str, str], ...]:
    rows: list[dict[str, str]] = []
    for candidate in ROUND159_CASE_CANDIDATES:
        target = round159_target_for(candidate.target_id)
...
def round159_stage_date_rows() -> tuple[dict[str, str], ...]:
    return tuple(
        {
            "target_id": target.target_id,
...
def round159_price_field_rows() -> tuple[dict[str, str], ...]:
    return tuple({"field": field, "required_for_round159_backfill": "true"} for field in ROUND159_PRICE_FIELDS)


def round159_base_score_weight_rows() -> tuple[dict[str, str], ...]:
    return tuple(weight.as_row() for weight in ROUND159_BASE_SCORE_WEIGHTS)


def round159_stage_cap_rows() -> tuple[dict[str, str], ...]:
    return tuple(cap.as_row() for cap in ROUND159_STAGE_CAPS)


def round159_score_stage_price_alignment_rows() -> tuple[dict[str, str], ...]:
    return tuple(row.as_row() for row in ROUND159_SCORE_STAGE_PRICE_ALIGNMENT)


def round159_summary() -> dict[str, int | bool]:
    records = round159_case_records()
    return {
        "target_count": len(ROUND159_SCORE_TARGETS),
        "case_candidate_count": len(records),
        "base_score_component_count": len(ROUND159_BASE_SCORE_WEIGHTS),
        "stage_cap_count": len(ROUND159_STAGE_CAPS),
        "score_stage_price_alignment_count": len(ROUND159_SCORE_STAGE_PRICE_ALIGNMENT),
        ...
        "green_possible_count": sum(1 for target in ROUND159_SCORE_TARGETS if target.posture == Round10ThemePosture.GREEN_POSSIBLE),
        "watch_yellow_first_count": sum(1 for target in ROUND159_SCORE_TARGETS if target.posture == Round10ThemePosture.WATCH_YELLOW_FIRST),
        "redteam_first_count": sum(1 for target in ROUND159_SCORE_TARGETS if target.posture == Round10ThemePosture.REDTEAM_FIRST),
        "hard_gate_target_count": sum(1 for target in ROUND159_SCORE_TARGETS if target.hard_gate),
        "production_scoring_changed": False,
        "case_records_are_candidate_generation_input": False,
    }


def write_round159_r1_loop10_reports(
    *,
    output_directory: str | Path = ROUND159_DEFAULT_OUTPUT_DIRECTORY,
    cases_path: str | Path = ROUND159_DEFAULT_CASES_PATH,
    score_profile_path: str | Path = ROUND159_DEFAULT_SCORE_PROFILE_PATH,
) -> dict[str, Path]:
    ...
    paths = {
        ...
        "summary": output / "round159_r1_loop10_industrial_infra_summary.md",
        "case_matrix": output / "round159_r1_loop10_case_matrix.csv",
        "stage_date_plan": output / "round159_r1_loop10_stage_date_plan.csv",
        "green_guardrails": output / "round159_r1_loop10_green_guardrails.md",
        "loop10_risk_overlays": output / "round159_r1_loop10_risk_overlays.md",
        "price_validation_plan": output / "round159_r1_loop10_price_validation_plan.md",
        "price_fields": output / "round159_r1_loop10_price_fields.csv",
        "base_score_weights": output / "round159_r1_loop10_base_score_weights.csv",
        "stage_caps": output / "round159_r1_loop10_stage_caps.csv",
        "score_stage_price_alignment": output / "round159_r1_loop10_score_stage_price_alignment.csv",
        "score_stage_price_alignment_md": output / "round159_r1_loop10_score_stage_price_alignment.md",
    }
    _write_case_jsonl(round159_case_records(), cases)
    _write_rows(round159_score_profile_rows(), score_profiles)
    _write_rows(round159_case_candidate_rows(), paths["case_matrix"])
    _write_rows(round159_stage_date_rows(), paths["stage_date_plan"])
    _write_rows(round159_price_field_rows(), paths["price_fields"])
    _write_rows(round159_base_score_weight_rows(), paths["base_score_weights"])
    _write_rows(round159_stage_cap_rows(), paths["stage_caps"])
    _write_rows(round159_score_stage_price_alignment_rows(), paths["score_stage_price_alignment"])
    paths["summary"].write_text(render_round159_summary_markdown(), encoding="utf-8")
    paths["green_guardrails"].write_text(render_round159_green_guardrail_markdown(), encoding="utf-8")
    paths["loop10_risk_overlays"].write_text(render_round159_loop10_risk_overlay_markdown(), encoding="utf-8")
    paths["price_validation_plan"].write_text(render_round159_price_validation_plan_markdown(), encoding="utf-8")
    paths["score_stage_price_alignment_md"].write_text(render_round159_score_stage_price_alignment_markdown(), encoding="utf-8")
    return paths


def render_round159_summary_markdown() -> str:
    summary = round159_summary()
    lines = [
        "# Round-159 R1 Loop-10 Industrial Orders / Infrastructure Summary",
        "",
        f"- source_round: `{ROUND159_SOURCE_ROUND_PATH}`",
        "- large_sector: `INDUSTRIAL_ORDERS_INFRA`",
        "- loop: `R1 Loop 10 / v10.0`",
        ...
        "## Interpretation",
        "",
        "- R1 Loop 10 narrows order/backlog candidates after the R13 Loop-9 RedTeam pass.",
        ...
        "- Loop 10 makes score-to-stage-to-price validation explicit: Stage 2 contracts are not Stage 3 until OP/EPS/FCF, margin, and price path line up.",
        "- Loop 10 base score weights are EPS/FCF 25, visibility 22, bottleneck/pricing 18, capital discipline 10, mispricing 9, valuation room 7, disclosure confidence 9.",
        "- Loop 10 keeps EHV transformer export, backlog-to-FCF conversion, grid-flexibility context, nuclear restart approval gates, and data-center power/water gates.",
...
def render_round159_green_guardrail_markdown() -> str:
    lines = [
        "# Round-159 R1 Loop-10 Green Guardrails",
        "",
    ...
    for target in ROUND159_SCORE_TARGETS:
        lines.append(
            ...
            f"{', '.join(target.green_conditions)} | {', '.join(target.loop10_penalty_axes)} |"
        )
    lines.extend(
        [
            "",
            "## What Not To Change",
            "",
            "- Do not apply R1 Loop-10 v10.0 weights to production scoring yet.",
...
def render_round159_loop10_risk_overlay_markdown() -> str:
    lines = [
        "# Round-159 R1 Loop-10 Risk Overlays",
        "",
...
def render_round159_price_validation_plan_markdown() -> str:
    lines = [
    ...
    for row in round159_case_candidate_rows():
...
def render_round159_score_stage_price_alignment_markdown() -> str:
    lines = [
        "# Round-159 R1 Loop-10 Score -> Stage -> Price Alignment",
        "",
        "## Base Score Weights",
        "",
    ...
    for row in ROUND159_BASE_SCORE_WEIGHTS:
    ...
    for cap in ROUND159_STAGE_CAPS:
        lines.append(
    ...
    for row in ROUND159_SCORE_STAGE_PRICE_ALIGNMENT:
        lines.append(
    ...
    lines.extend(
        [
            ...
            "## Interpretation",
            "",
            "- GE Vernova is the cleanest Loop-10 example where score, stage, and short-term price reaction align, but it also turns on 4B-watch.",
...
def _round159_score_price_alignment(candidate: Round159CaseCandidate) -> str:
...
def _round159_rerating_result(candidate: Round159CaseCandidate) -> str:
...
__all__ = [
    "ROUND159_CASE_CANDIDATES",
    "ROUND159_BASE_SCORE_WEIGHTS",
    "ROUND159_DEFAULT_CASES_PATH",
    "ROUND159_DEFAULT_OUTPUT_DIRECTORY",
    "ROUND159_DEFAULT_SCORE_PROFILE_PATH",
    "ROUND159_PRICE_FIELDS",
    "ROUND159_SCORE_TARGETS",
    "ROUND159_SCORE_STAGE_PRICE_ALIGNMENT",
    "ROUND159_STAGE_CAPS",
    "Round159CaseCandidate",
    "Round159BaseScoreWeight",
    "Round159ScoreStagePriceAlignment",
    "Round159ScoreTarget",
    "Round159ScoreWeightDraft",
    "Round159StageCap",
    "render_round159_green_guardrail_markdown",
    "render_round159_loop10_risk_overlay_markdown",
    "render_round159_price_validation_plan_markdown",
    "render_round159_score_stage_price_alignment_markdown",
    "render_round159_summary_markdown",
    "round159_base_score_weight_rows",
    "round159_case_candidate_rows",
    "round159_case_records",
    "round159_price_field_rows",
    "round159_score_profile_rows",
    "round159_score_stage_price_alignment_rows",
    "round159_stage_cap_rows",
    "round159_stage_date_rows",
    "round159_summary",
    "round159_target_for",
    "write_round159_r1_loop10_reports",
]
