"""Round-77 R11 Loop-3 policy/geopolitical/disaster/event pack.

Round 77 tightens the R11 event-risk pack from Round 64. The rule stays simple:
...
ROUND77_SOURCE_ROUND_PATH = "docs/round/round_77.md"
ROUND77_DEFAULT_OUTPUT_DIRECTORY = "output/e2r_round77_r11_loop3_policy_geopolitical_event"
ROUND77_DEFAULT_CASES_PATH = "data/e2r_case_library/cases_r11_loop3_round77.jsonl"
ROUND77_DEFAULT_SCORE_PROFILE_PATH = "data/sector_taxonomy/score_weight_profiles_round77_r11_loop3_v3.csv"


@dataclass(frozen=True)
class Round77ScoreWeightDraft:
    eps_fcf: int | str
...
@dataclass(frozen=True)
class Round77ScoreTarget:
    target_id: str
    canonical_archetype: E2RArchetype
    posture: Round10ThemePosture
    score_weight: Round77ScoreWeightDraft
...
ROUND77_SCORE_TARGETS: tuple[Round77ScoreTarget, ...] = (
    Round77ScoreTarget(
        "NORTH_KOREA_POLICY_EVENT",
        E2RArchetype.NORTH_KOREA_POLICY_EVENT,
        Round10ThemePosture.REDTEAM_FIRST,
        Round77ScoreWeightDraft(4, 3, 5, 8, 4, 0, 3),
    ...
    Round77ScoreTarget(
        ...
        Round77ScoreWeightDraft(12, 11, 8, 10, 8, 0, 4),
    ...
    Round77ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round77ScoreWeightDraft(10, 6, 7, 8, 6, 0, 4),
    ...
    Round77ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round77ScoreWeightDraft(13, 14, 11, 10, 8, 0, 5),
    ...
    Round77ScoreTarget(
        ...
        Round10ThemePosture.REDTEAM_FIRST,
        Round77ScoreWeightDraft(13, 9, 8, 8, 6, 0, 5),
    ...
    Round77ScoreTarget(
        ...
        Round10ThemePosture.REDTEAM_FIRST,
        Round77ScoreWeightDraft(19, 5, 5, 5, 5, 0, 5),
    ...
    Round77ScoreTarget(
        ...
        Round10ThemePosture.REDTEAM_FIRST,
        Round77ScoreWeightDraft(5, 4, 5, 5, 5, 0, 3),
    ...
    Round77ScoreTarget(
        ...
        Round10ThemePosture.REDTEAM_FIRST,
        Round77ScoreWeightDraft(7, 6, 6, 8, 6, 0, 3),
    ...
    Round77ScoreTarget(
        ...
        Round10ThemePosture.REDTEAM_FIRST,
        Round77ScoreWeightDraft(5, 5, 5, 8, 5, 0, 3),
    ...
    Round77ScoreTarget(
        ...
        Round10ThemePosture.REDTEAM_FIRST,
        Round77ScoreWeightDraft(8, 5, 5, 6, 5, 0, 4),
    ...
    Round77ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round77ScoreWeightDraft(15, 14, 8, 10, 8, 0, 5),
    ...
    Round77ScoreTarget(
        ...
        Round10ThemePosture.REDTEAM_FIRST,
        Round77ScoreWeightDraft("gate", "gate", "gate", "gate", "gate", "gate", "gate"),
...
ROUND77_CASE_CANDIDATES: tuple[Round77CaseCandidate, ...] = (
    Round77CaseCandidate(
    ...
    Round77CaseCandidate(
        ...
        None,
        None,
        ("project_financing", "financing_amount", "telecom_infrastructure", "participating_company"),
    ...
    Round77CaseCandidate(
        ...
        None,
        None,
        ("heatwave_event", "cooling_demand", "grid_stress", "peak_load_increase_estimate"),
    ...
    Round77CaseCandidate(
        ...
        None,
        None,
        ("vpp_program", "battery_program_capacity", "demand_response_program", "grid_stress"),
...
ROUND77_PRICE_FIELDS: tuple[str, ...] = (
    "case_id",
    "symbol",
    "company_name",
    "primary_archetype",
...
def target_for(target_id: str) -> Round77ScoreTarget | None:
    for target in ROUND77_SCORE_TARGETS:
        if target.target_id == target_id:
            return target
    return None


def round77_case_records() -> tuple[E2RCaseRecord, ...]:
    records: list[E2RCaseRecord] = []
    for candidate in ROUND77_CASE_CANDIDATES:
        ...
        record = E2RCaseRecord(
            ...
            evidence_summary=(
                f"Round77 R11 Loop-3 case for {candidate.target_id}; "
...
def round77_score_profile_rows() -> tuple[dict[str, str], ...]:
...
def round77_case_candidate_rows() -> tuple[dict[str, str], ...]:
    rows: list[dict[str, str]] = []
    for candidate in ROUND77_CASE_CANDIDATES:
        target = target_for(candidate.target_id)
...
def round77_stage_date_rows() -> tuple[dict[str, str], ...]:
    return tuple(
        {
            "target_id": target.target_id,
...
def round77_price_field_rows() -> tuple[dict[str, str], ...]:
    return tuple({"field": field, "required_for_round77_backfill": "true"} for field in ROUND77_PRICE_FIELDS)


def round77_summary() -> dict[str, int | bool]:
    records = round77_case_records()
    return {
        "target_count": len(ROUND77_SCORE_TARGETS),
        ...
        "green_possible_count": sum(1 for target in ROUND77_SCORE_TARGETS if target.posture == Round10ThemePosture.GREEN_POSSIBLE),
        "watch_yellow_first_count": sum(1 for target in ROUND77_SCORE_TARGETS if target.posture == Round10ThemePosture.WATCH_YELLOW_FIRST),
        "redteam_first_count": sum(1 for target in ROUND77_SCORE_TARGETS if target.posture == Round10ThemePosture.REDTEAM_FIRST),
        "gate_only_target_count": sum(1 for target in ROUND77_SCORE_TARGETS if target.gate_only),
        "production_scoring_changed": False,
        "case_records_are_candidate_generation_input": False,
    }


def write_round77_r11_loop3_reports(
    *,
    output_directory: str | Path = ROUND77_DEFAULT_OUTPUT_DIRECTORY,
    cases_path: str | Path = ROUND77_DEFAULT_CASES_PATH,
    score_profile_path: str | Path = ROUND77_DEFAULT_SCORE_PROFILE_PATH,
) -> dict[str, Path]:
    ...
    paths = {
        ...
        "summary": output / "round77_r11_loop3_policy_geopolitical_event_summary.md",
        "case_matrix": output / "round77_r11_loop3_case_matrix.csv",
        "stage_date_plan": output / "round77_r11_loop3_stage_date_plan.csv",
        "green_guardrails": output / "round77_r11_loop3_green_guardrails.md",
        "event_false_positive_caps": output / "round77_r11_loop3_event_false_positive_caps.md",
        "price_validation_plan": output / "round77_r11_loop3_price_validation_plan.md",
        "price_fields": output / "round77_r11_loop3_price_fields.csv",
    }
    _write_case_jsonl(round77_case_records(), cases)
    _write_rows(round77_score_profile_rows(), score_profiles)
    _write_rows(round77_case_candidate_rows(), paths["case_matrix"])
    _write_rows(round77_stage_date_rows(), paths["stage_date_plan"])
    _write_rows(round77_price_field_rows(), paths["price_fields"])
    paths["summary"].write_text(render_round77_summary_markdown(), encoding="utf-8")
    paths["green_guardrails"].write_text(render_round77_green_guardrail_markdown(), encoding="utf-8")
    paths["event_false_positive_caps"].write_text(render_round77_event_false_positive_caps_markdown(), encoding="utf-8")
    paths["price_validation_plan"].write_text(render_round77_price_validation_plan_markdown(), encoding="utf-8")
    return paths


def render_round77_summary_markdown() -> str:
    summary = round77_summary()
    lines = [
        "# Round-77 R11 Loop-3 Policy / Geopolitical / Disaster / Event Summary",
        "",
        f"- source_round: `{ROUND77_SOURCE_ROUND_PATH}`",
        ...
        "## Interpretation",
        "",
        "- R11 Loop-3 is mostly a false-positive defense pack.",
...
def render_round77_green_guardrail_markdown() -> str:
    lines = [
        "# Round-77 R11 Loop-3 Green Guardrails",
        "",
    ...
    lines.extend(
        [
            ...
            "## What Not To Change",
            "",
            "- Do not apply these R11 Loop-3 v3 weights to production scoring yet.",
...
def render_round77_event_false_positive_caps_markdown() -> str:
    lines = [
        "# Round-77 R11 Loop-3 Event False-Positive Caps",
        "",
...
def render_round77_price_validation_plan_markdown() -> str:
    lines = [
    ...
    for row in round77_case_candidate_rows():
...
def _score_price_alignment(candidate: Round77CaseCandidate) -> str:
...
def _rerating_result(candidate: Round77CaseCandidate) -> str:
...
__all__ = [
    "ROUND77_CASE_CANDIDATES",
    "ROUND77_DEFAULT_CASES_PATH",
    "ROUND77_DEFAULT_OUTPUT_DIRECTORY",
    "ROUND77_DEFAULT_SCORE_PROFILE_PATH",
    "ROUND77_PRICE_FIELDS",
    "ROUND77_SCORE_TARGETS",
    "ROUND77_SOURCE_ROUND_PATH",
    "Round77CaseCandidate",
    "Round77ScoreTarget",
    "Round77ScoreWeightDraft",
    "render_round77_event_false_positive_caps_markdown",
    "render_round77_green_guardrail_markdown",
    "render_round77_price_validation_plan_markdown",
    "render_round77_summary_markdown",
    "round77_case_candidate_rows",
    "round77_case_records",
    "round77_price_field_rows",
    "round77_score_profile_rows",
    "round77_stage_date_rows",
    "round77_summary",
    "target_for",
    "write_round77_r11_loop3_reports",
]
