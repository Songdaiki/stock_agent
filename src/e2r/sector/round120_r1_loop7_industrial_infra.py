"""Round-120 R1 Loop-7 industrial orders and infrastructure pack.

Round 120 returns to R1 after the R13 Loop-6 RedTeam pass. It narrows the
...
ROUND120_SOURCE_ROUND_PATH = "docs/round/round_120.md"
ROUND120_DEFAULT_OUTPUT_DIRECTORY = "output/e2r_round120_r1_loop7_industrial_infra"
ROUND120_DEFAULT_CASES_PATH = "data/e2r_case_library/cases_r1_loop7_round120.jsonl"
ROUND120_DEFAULT_SCORE_PROFILE_PATH = "data/sector_taxonomy/score_weight_profiles_round120_r1_loop7_v7.csv"


@dataclass(frozen=True)
class Round120ScoreWeightDraft:
    eps_fcf: int | str
...
@dataclass(frozen=True)
class Round120ScoreTarget:
    target_id: str
    canonical_archetype: E2RArchetype
    posture: Round10ThemePosture
    score_weight: Round120ScoreWeightDraft
...
class Round120BaseScoreWeight:
    ...
    def as_row(self) -> dict[str, str]:
        return {
            ...
            "loop7_direction": self.loop7_direction,
            "reason": self.reason,
        }


@dataclass(frozen=True)
class Round120StageCap:
    stage_band: str
    max_score: str
...
@dataclass(frozen=True)
class Round120ScoreStagePriceAlignment:
    case_id: str
    detected_stage: str
...
ROUND120_BASE_SCORE_WEIGHTS: tuple[Round120BaseScoreWeight, ...] = (
    Round120BaseScoreWeight("eps_fcf_revision", 25, "up", "Stage 3 promotion depends on order/backlog becoming OP/EPS/FCF."),
    Round120BaseScoreWeight("contract_backlog_visibility", 24, "up", "Contract amount, customer, delivery, and backlog are the Stage 2 core."),
    Round120BaseScoreWeight("bottleneck_pricing_power", 20, "hold", "Transformer, gas turbine, slot, and CAPA bottlenecks remain central."),
    Round120BaseScoreWeight("market_mispricing_gap", 12, "hold", "Old industrial frame must still be wrong."),
    Round120BaseScoreWeight("valuation_room_4b_runway", 8, "cap_after_rerating", "GE Vernova-style winners need 4B watch after large rerating."),
    Round120BaseScoreWeight("capital_discipline_fcf_conversion", 6, "up", "Siemens is a positive FCF/capital return example; Hanwha dilution is a negative example."),
    Round120BaseScoreWeight("information_confidence_disclosure_detail", 5, "up", "OpenDART list-only or MoU-only evidence caps Stage 3."),
)


ROUND120_STAGE_CAPS: tuple[Round120StageCap, ...] = (
    Round120StageCap(
        "Stage 1",
        "45",
        ("industry_news", "policy_news", "mou", "macro_shortage_without_company_contract"),
        ("us_transformer_shortage_import_slots_case", "hd_hyundai_huntington_us_navy_aux_case", "ukraine_reconstruction_policy_case"),
        "Green blocked until company-level contract/backlog detail appears.",
    ),
    Round120StageCap(
        "Stage 2",
        "70",
        ("contract_amount", "customer", "period_or_delivery_schedule", "backlog_or_guidance"),
        ("ls_electric_525kv_us_datacenter_transformer_case", "hyundai_rotem_morocco_rail_case"),
    ...
    Round120StageCap(
        "Stage 4C",
        "hard_gate",
        ("dilution", "contract_cancel_or_delay", "low_margin", "permitting_or_policy_break", "accounting_or_disclosure_shock"),
        ("hanwha_aerospace_dilution_trim_case", "nuscale_uamps_smr_cancel_case", "perth_data_center_withdrawal_case"),
        "Hard RedTeam overrides high order/backlog score.",
    ),
)


ROUND120_SCORE_STAGE_PRICE_ALIGNMENT: tuple[Round120ScoreStagePriceAlignment, ...] = (
    Round120ScoreStagePriceAlignment(
...
ROUND120_SCORE_TARGETS: tuple[Round120ScoreTarget, ...] = (
    Round120ScoreTarget(
        "GRID_TRANSFORMER_SHORTAGE",
        E2RArchetype.GRID_TRANSFORMER_SHORTAGE,
        Round10ThemePosture.GREEN_POSSIBLE,
        Round120ScoreWeightDraft(24, 25, 24, 12, 10, 1, 5),
        ...
        "Loop 7 strengthens transformer shortage, but Green still requires contract amount, duration, delivery, margin, and EPS conversion.",
    ),
    Round120ScoreTarget(
        "GRID_EHV_TRANSFORMER_EXPORT",
        E2RArchetype.GRID_EHV_TRANSFORMER_EXPORT,
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round120ScoreWeightDraft(24, 25, 24, 13, 10, 1, 6),
    ...
    Round120ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round120ScoreWeightDraft(22, 24, 22, 12, 10, 1, 5),
    ...
    Round120ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round120ScoreWeightDraft(22, 23, 20, 12, 10, 1, 5),
    ...
    Round120ScoreTarget(
        ...
        Round10ThemePosture.GREEN_POSSIBLE,
        Round120ScoreWeightDraft(22, 23, 19, 13, 10, 0, 5),
    ...
    Round120ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round120ScoreWeightDraft(21, 22, 18, 12, 10, 0, 5),
    ...
    Round120ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round120ScoreWeightDraft(22, 22, 18, 12, 9, 4, 5),
    ...
    Round120ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round120ScoreWeightDraft("gate", "gate", "gate", "gate", "gate", "gate", "gate"),
    ...
    Round120ScoreTarget(
        ...
        Round10ThemePosture.GREEN_POSSIBLE,
        Round120ScoreWeightDraft(20, 24, 18, 13, 12, 1, 5),
    ...
    Round120ScoreTarget(
        ...
        Round10ThemePosture.GREEN_POSSIBLE,
        Round120ScoreWeightDraft(21, 24, 17, 14, 14, 3, 5),
    ...
    Round120ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round120ScoreWeightDraft(21, 23, 16, 14, 13, 2, 5),
    ...
    Round120ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round120ScoreWeightDraft(16, 16, 12, 13, 9, 1, 5),
    ...
    Round120ScoreTarget(
        ...
        Round10ThemePosture.GREEN_POSSIBLE,
        Round120ScoreWeightDraft(21, 22, 18, 13, 13, 1, 5),
    ...
    Round120ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round120ScoreWeightDraft(16, 17, 11, 13, 10, 1, 5),
    ...
    Round120ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round120ScoreWeightDraft(20, 22, 12, 14, 11, 1, 5),
    ...
    Round120ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round120ScoreWeightDraft(19, 23, 13, 14, 12, 2, 5),
    ...
    Round120ScoreTarget(
        ...
        Round10ThemePosture.REDTEAM_FIRST,
        Round120ScoreWeightDraft(13, 12, 10, 13, 7, 1, 5),
    ...
    Round120ScoreTarget(
        ...
        Round10ThemePosture.WATCH_YELLOW_FIRST,
        Round120ScoreWeightDraft(10, 8, 8, 10, 7, 0, 4),
    ...
    Round120ScoreTarget(
        ...
        Round10ThemePosture.REDTEAM_FIRST,
        Round120ScoreWeightDraft("cap", "cap", "cap", "cap", "cap", "cap", "+"),
...
ROUND120_CASE_CANDIDATES: tuple[Round120CaseCandidate, ...] = (
    Round120CaseCandidate(
        ...
        ("round_120.md Reuters US transformer shortage",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md LS Electric 525kV US datacenter transformer case placeholder",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md Reuters ABB medium-voltage equipment investment",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md Reuters Siemens Energy cash flow and buyback",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md Siemens Energy record backlog context",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md WSJ GE Vernova orders/backlog",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md Reuters GE Vernova power backlog / turbine context",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md Reuters EIA US power demand record path",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md arXiv AI data-center flexibility",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md Guardian Perth data-center withdrawal",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md data-center moratorium reference",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md Seattle data-center moratorium watch",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md arXiv data-center public water systems",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md Reuters Romania K9 contract",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md Reuters Hanwha Europe land arms sales",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md Reuters Hanwha Aerospace trims capital increase plan",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md Reuters HD Hyundai Heavy / Huntington Ingalls MoA",),
    ...
    Round120CaseCandidate(
        ...
        ("round_120.md AP Hanwha Ocean China sanctions",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md arXiv shipyard procurement lead-time prediction",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md Reuters Hyundai Rotem Morocco rail order",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md Reuters Meta Constellation nuclear PPA",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md Reuters Three Mile Island restart decision",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md Reuters Three Mile Island restart grid rights gate",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md NuScale UAMPS CFPP cancellation",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md reconstruction policy rule",),
    ...
    Round120CaseCandidate(
        ...
        "needs_price_backfill",
        ("round_120.md Neom policy/infrastructure rule",),
...
ROUND120_PRICE_FIELDS: tuple[str, ...] = (
    "case_id",
    "symbol",
    "company_name",
    "primary_archetype",
...
def round120_target_for(target_id: str) -> Round120ScoreTarget | None:
    for target in ROUND120_SCORE_TARGETS:
        if target.target_id == target_id:
            return target
    return None


def round120_case_records() -> tuple[E2RCaseRecord, ...]:
    records: list[E2RCaseRecord] = []
    for candidate in ROUND120_CASE_CANDIDATES:
        target = round120_target_for(candidate.target_id)
        if target is None:
        ...
        record = E2RCaseRecord(
            ...
            evidence_summary=(
                f"Round120 R1 Loop-7 case for {candidate.target_id}; "
            ...
            score_price_alignment=_round120_score_price_alignment(candidate),
            rerating_result=_round120_rerating_result(candidate),
            ...
            green_guardrails=(
                ...
                "project_delay_capital_allocation_shock_low_margin_mro_smr_false_green_are_loop7_penalties",
                *target.red_flags,
            ),
...
def round120_score_profile_rows() -> tuple[dict[str, str], ...]:
        ...
        rows.append(
            {
                ...
                "loop7_penalty_axes": "|".join(target.loop7_penalty_axes),
...
def round120_case_candidate_rows() -> tuple[dict[str, str], ...]:
    rows: list[dict[str, str]] = []
    for candidate in ROUND120_CASE_CANDIDATES:
        target = round120_target_for(candidate.target_id)
...
def round120_stage_date_rows() -> tuple[dict[str, str], ...]:
    return tuple(
        {
            "target_id": target.target_id,
...
def round120_price_field_rows() -> tuple[dict[str, str], ...]:
    return tuple({"field": field, "required_for_round120_backfill": "true"} for field in ROUND120_PRICE_FIELDS)


def round120_base_score_weight_rows() -> tuple[dict[str, str], ...]:
    return tuple(weight.as_row() for weight in ROUND120_BASE_SCORE_WEIGHTS)


def round120_stage_cap_rows() -> tuple[dict[str, str], ...]:
    return tuple(cap.as_row() for cap in ROUND120_STAGE_CAPS)


def round120_score_stage_price_alignment_rows() -> tuple[dict[str, str], ...]:
    return tuple(row.as_row() for row in ROUND120_SCORE_STAGE_PRICE_ALIGNMENT)


def round120_summary() -> dict[str, int | bool]:
    records = round120_case_records()
    return {
        "target_count": len(ROUND120_SCORE_TARGETS),
        "case_candidate_count": len(records),
        "base_score_component_count": len(ROUND120_BASE_SCORE_WEIGHTS),
        "stage_cap_count": len(ROUND120_STAGE_CAPS),
        "score_stage_price_alignment_count": len(ROUND120_SCORE_STAGE_PRICE_ALIGNMENT),
        ...
        "green_possible_count": sum(1 for target in ROUND120_SCORE_TARGETS if target.posture == Round10ThemePosture.GREEN_POSSIBLE),
        "watch_yellow_first_count": sum(1 for target in ROUND120_SCORE_TARGETS if target.posture == Round10ThemePosture.WATCH_YELLOW_FIRST),
        "redteam_first_count": sum(1 for target in ROUND120_SCORE_TARGETS if target.posture == Round10ThemePosture.REDTEAM_FIRST),
        "hard_gate_target_count": sum(1 for target in ROUND120_SCORE_TARGETS if target.hard_gate),
        "production_scoring_changed": False,
        "case_records_are_candidate_generation_input": False,
    }


def write_round120_r1_loop7_reports(
    *,
    output_directory: str | Path = ROUND120_DEFAULT_OUTPUT_DIRECTORY,
    cases_path: str | Path = ROUND120_DEFAULT_CASES_PATH,
    score_profile_path: str | Path = ROUND120_DEFAULT_SCORE_PROFILE_PATH,
) -> dict[str, Path]:
    ...
    paths = {
        ...
        "summary": output / "round120_r1_loop7_industrial_infra_summary.md",
        "case_matrix": output / "round120_r1_loop7_case_matrix.csv",
        "stage_date_plan": output / "round120_r1_loop7_stage_date_plan.csv",
        "green_guardrails": output / "round120_r1_loop7_green_guardrails.md",
        "loop7_risk_overlays": output / "round120_r1_loop7_risk_overlays.md",
        "price_validation_plan": output / "round120_r1_loop7_price_validation_plan.md",
        "price_fields": output / "round120_r1_loop7_price_fields.csv",
        "base_score_weights": output / "round120_r1_loop7_base_score_weights.csv",
        "stage_caps": output / "round120_r1_loop7_stage_caps.csv",
        "score_stage_price_alignment": output / "round120_r1_loop7_score_stage_price_alignment.csv",
        "score_stage_price_alignment_md": output / "round120_r1_loop7_score_stage_price_alignment.md",
    }
    _write_case_jsonl(round120_case_records(), cases)
    _write_rows(round120_score_profile_rows(), score_profiles)
    _write_rows(round120_case_candidate_rows(), paths["case_matrix"])
    _write_rows(round120_stage_date_rows(), paths["stage_date_plan"])
    _write_rows(round120_price_field_rows(), paths["price_fields"])
    _write_rows(round120_base_score_weight_rows(), paths["base_score_weights"])
    _write_rows(round120_stage_cap_rows(), paths["stage_caps"])
    _write_rows(round120_score_stage_price_alignment_rows(), paths["score_stage_price_alignment"])
    paths["summary"].write_text(render_round120_summary_markdown(), encoding="utf-8")
    paths["green_guardrails"].write_text(render_round120_green_guardrail_markdown(), encoding="utf-8")
    paths["loop7_risk_overlays"].write_text(render_round120_loop7_risk_overlay_markdown(), encoding="utf-8")
    paths["price_validation_plan"].write_text(render_round120_price_validation_plan_markdown(), encoding="utf-8")
    paths["score_stage_price_alignment_md"].write_text(render_round120_score_stage_price_alignment_markdown(), encoding="utf-8")
    return paths


def render_round120_summary_markdown() -> str:
    summary = round120_summary()
    lines = [
        "# Round-120 R1 Loop-7 Industrial Orders / Infrastructure Summary",
        "",
        f"- source_round: `{ROUND120_SOURCE_ROUND_PATH}`",
        ...
        "## Interpretation",
        "",
        "- R1 Loop 7 narrows order/backlog candidates after the R13 Loop-6 RedTeam pass.",
        ...
        "- Loop 7 makes score-to-stage-to-price validation explicit: Stage 2 contracts are not Stage 3 until OP/EPS/FCF, margin, and price path line up.",
        "- Loop 7 base score weights are EPS/FCF 25, visibility 24, bottleneck/pricing 20, mispricing 12, valuation room 8, capital discipline 6, information confidence 5.",
        "- Loop 7 keeps EHV transformer export, power-equipment capital return, grid-flexibility context, and data-center power/water gates.",
...
def render_round120_green_guardrail_markdown() -> str:
    lines = [
    ...
    for target in ROUND120_SCORE_TARGETS:
        lines.append(
            ...
            f"{', '.join(target.green_conditions)} | {', '.join(target.loop7_penalty_axes)} |"
        )
    lines.extend(
        [
            "",
...
def render_round120_loop7_risk_overlay_markdown() -> str:
    lines = [
        "# Round-120 R1 Loop-7 Risk Overlays",
        "",
...
def render_round120_price_validation_plan_markdown() -> str:
    lines = [
        "# Round-120 R1 Loop-7 Price Validation Plan",
        "",
        "## Method",
        "",
    ...
    for row in round120_case_candidate_rows():
...
def render_round120_score_stage_price_alignment_markdown() -> str:
    lines = [
    ...
    for row in ROUND120_BASE_SCORE_WEIGHTS:
        lines.append(f"| `{row.component}` | {row.points} | {row.loop7_direction} | {row.reason} |")
    lines.extend(
        [
            "",
            "## Stage Caps",
    ...
    for cap in ROUND120_STAGE_CAPS:
        lines.append(
    ...
    for row in ROUND120_SCORE_STAGE_PRICE_ALIGNMENT:
        lines.append(
...
def _round120_score_price_alignment(candidate: Round120CaseCandidate) -> str:
...
def _round120_rerating_result(candidate: Round120CaseCandidate) -> str:
...
__all__ = [
    "ROUND120_CASE_CANDIDATES",
    "ROUND120_BASE_SCORE_WEIGHTS",
    "ROUND120_DEFAULT_CASES_PATH",
    "ROUND120_DEFAULT_OUTPUT_DIRECTORY",
    "ROUND120_DEFAULT_SCORE_PROFILE_PATH",
    "ROUND120_PRICE_FIELDS",
    "ROUND120_SCORE_TARGETS",
    "ROUND120_SCORE_STAGE_PRICE_ALIGNMENT",
    "ROUND120_STAGE_CAPS",
    "Round120CaseCandidate",
    "Round120BaseScoreWeight",
    "Round120ScoreStagePriceAlignment",
    "Round120ScoreTarget",
    "Round120ScoreWeightDraft",
    "Round120StageCap",
    "render_round120_green_guardrail_markdown",
    "render_round120_loop7_risk_overlay_markdown",
    "render_round120_price_validation_plan_markdown",
    "render_round120_score_stage_price_alignment_markdown",
    "render_round120_summary_markdown",
    "round120_base_score_weight_rows",
    "round120_case_candidate_rows",
    "round120_case_records",
    "round120_price_field_rows",
    "round120_score_profile_rows",
    "round120_score_stage_price_alignment_rows",
    "round120_stage_cap_rows",
    "round120_stage_date_rows",
    "round120_summary",
    "round120_target_for",
    "write_round120_r1_loop7_reports",
]
